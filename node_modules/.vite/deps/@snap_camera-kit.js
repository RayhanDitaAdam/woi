...
//#endregion
//#region node_modules/.pnpm/@snap+ts-inject@0.3.2/node_modules/@snap/ts-inject/dist/esm/memoize.js
function isMemoized(fn) {
	return typeof fn === "function" && typeof fn.delegate === "function";
}
function memoize$1(thisArg, delegate) {
	let memo;
	const memoized = (...args) => {
...
//#endregion
//#region node_modules/.pnpm/@snap+ts-inject@0.3.2/node_modules/@snap/ts-inject/dist/esm/entries.js
var entries$1 = (o) => Object.entries(o);
//#endregion
//#region node_modules/.pnpm/@snap+ts-inject@0.3.2/node_modules/@snap/ts-inject/dist/esm/Injectable.js
function Injectable(token, dependenciesOrFn, maybeFn) {
	...
	if (!fn) throw new TypeError("[Injectable] Received invalid arguments. The factory function must be either the second or third argument.");
	if (fn.length !== dependencies.length) throw new TypeError(`[Injectable] Function arity does not match the number of dependencies. Function has arity ${fn.length}, but ${dependencies.length} dependencies were specified.\nDependencies: ${JSON.stringify(dependencies)}`);
...
function ConcatInjectable(token, dependenciesOrFn, maybeFn) {
	const dependencies = Array.isArray(dependenciesOrFn) ? dependenciesOrFn : [];
	const fn = typeof dependenciesOrFn === "function" ? dependenciesOrFn : maybeFn;
	if (!fn) throw new TypeError("[ConcatInjectable] Received invalid arguments. The factory function must be either the second or third argument.");
	if (fn.length !== dependencies.length) throw new TypeError(`[Injectable] Function arity does not match the number of dependencies. Function has arity ${fn.length}, but ${dependencies.length} dependencies were specified.\nDependencies: ${JSON.stringify(dependencies)}`);
...
//#endregion
//#region node_modules/.pnpm/@snap+ts-inject@0.3.2/node_modules/@snap/ts-inject/dist/esm/PartialContainer.js
var PartialContainer = class PartialContainer {
	injectables;
	...
	getFactories(parent) {
		...
		return factories = Object.fromEntries(entries$1(this.injectables).map(([token, fn]) => [token, memoize$1(parent, () => fn(...fn.dependencies.map((t) => {
			return t === token ? parent.get(t) : factories[t] ? factories[t]() : parent.get(t);
		})))]));
	}
	getTokens() {
...
//#endregion
//#region node_modules/.pnpm/@snap+ts-inject@0.3.2/node_modules/@snap/ts-inject/dist/esm/Container.js
...
var Container = class Container {
	...
	static fromObject(services) {
		return entries$1(services).reduce((container, [token, value]) => container.providesValue(token, value), new Container({}));
	}
	factories;
	constructor(factories) {
		...
		for (const k in factories) {
			...
			if (isMemoized(fn)) {
				...
				fn.thisArg = this;
			} else memoizedFactories[k] = memoize$1(this, fn);
		}
	...
	get(token) {
		...
		if (!factory) throw new Error(`[Container::get] Could not find Service for Token "${String(token)}". This should've caused a compile-time error. If the Token is 'undefined', check all your calls to the Injectable function. Make sure you define dependencies using string literals or string constants that are definitely initialized before the call to Injectable.`);
		return factory();
	}
	run(fnOrContainer) {
		if (fnOrContainer instanceof PartialContainer) {
			const runnableContainer = this.provides(fnOrContainer);
			for (const token of fnOrContainer.getTokens()) runnableContainer.get(token);
		} else this.provides(fnOrContainer).get(fnOrContainer.token);
		return this;
	}
	...
	providesService(fn) {
		...
		const factory = memoize$1(this, function() {
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/common/copyDefinedProperties.js
function copyDefinedProperties(obj) {
	return Object.fromEntries(Object.entries(obj).filter(([_, value]) => value !== void 0));
}
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/configurationOverrides.js
...
function defineWindowField(propertyToOverride) {
	...
	Object.defineProperty(window, `${windowFieldPrefix}${propertyToOverride}`, {
		...
		set(value) {
			...
			if (Object.values(storedOverrides).every((value) => typeof value === "undefined")) sessionStorage.removeItem(windowFieldPrefix);
			else sessionStorage.setItem(windowFieldPrefix, JSON.stringify(storedOverrides));
		},
		enumerable: false,
		configurable: true
	});
...
function getConfigurationOverrides() {
	if (!sessionStorage) return void 0;
...
function innerFrom(input) {
	...
	if (input != null) {
		...
		if (isPromise$1(input)) return fromPromise(input);
...
function scheduled(input, scheduler) {
	if (input != null) {
		...
		if (isPromise$1(input)) return schedulePromise(input, scheduler);
...
//#endregion
//#region node_modules/.pnpm/rxjs@7.8.2/node_modules/rxjs/dist/esm5/internal/firstValueFrom.js
...
//#endregion
//#region node_modules/.pnpm/rxjs@7.8.2/node_modules/rxjs/dist/esm5/internal/observable/never.js
var NEVER = new Observable(noop);
//#endregion
//#region node_modules/.pnpm/rxjs@7.8.2/node_modules/rxjs/dist/esm5/internal/util/argsOrArgArray.js
...
//#endregion
//#region node_modules/.pnpm/rxjs@7.8.2/node_modules/rxjs/dist/esm5/internal/observable/race.js
...
//#endregion
//#region node_modules/.pnpm/rxjs@7.8.2/node_modules/rxjs/dist/esm5/internal/operators/buffer.js
...
//#endregion
//#region node_modules/.pnpm/rxjs@7.8.2/node_modules/rxjs/dist/esm5/internal/operators/scanInternals.js
...
//#endregion
//#region node_modules/.pnpm/rxjs@7.8.2/node_modules/rxjs/dist/esm5/internal/operators/combineLatest.js
...
//#endregion
//#region node_modules/.pnpm/rxjs@7.8.2/node_modules/rxjs/dist/esm5/internal/operators/combineLatestWith.js
...
//#endregion
//#region node_modules/.pnpm/rxjs@7.8.2/node_modules/rxjs/dist/esm5/internal/operators/debounceTime.js
...
//#endregion
//#region node_modules/.pnpm/rxjs@7.8.2/node_modules/rxjs/dist/esm5/internal/operators/exhaustMap.js
...
//#endregion
//#region node_modules/.pnpm/rxjs@7.8.2/node_modules/rxjs/dist/esm5/internal/operators/raceWith.js
function raceWith() {
	var otherSources = [];
	for (var _i = 0; _i < arguments.length; _i++) otherSources[_i] = arguments[_i];
...
//#endregion
//#region node_modules/.pnpm/rxjs@7.8.2/node_modules/rxjs/dist/esm5/internal/operators/retry.js
...
//#endregion
//#region node_modules/.pnpm/rxjs@7.8.2/node_modules/rxjs/dist/esm5/internal/operators/scan.js
...
//#endregion
//#region node_modules/.pnpm/rxjs@7.8.2/node_modules/rxjs/dist/esm5/internal/operators/withLatestFrom.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/common/entries.js
var entries = (o) => Object.entries(o);
var fromEntries$1 = (entries) => Object.fromEntries(entries);
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/logger/logger.js
...
function mapLogger(logger) {
	if (!logger || logger === "noop") return noopLogger;
	if (logger === "console") return console;
	for (const level of Object.keys(logLevelMap)) if (typeof logger[level] !== "function") throw new Error(`Logger method '${level}' is not available on the provided logger instance.`);
	return logger;
}
var noopLogger = {
	error: () => {},
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/configuration.js
...
var createCameraKitConfigurationFactory = (configuration) => {
	...
	if (overrides) console.warn("Configuration overrides applied", overrides);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/namedErrors.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/benchmark/webglUtils.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/benchmark/benchmarkGflops.js
...
function prepareBenchmark(gl) {
	...
	if (!buffer) throw new Error("Failed to create WebGLBuffer.");
	...
	setUniform4f(gl, program, "v0", [
		1.15,
		1.23,
		1.47,
		1.84
	]);
	setUniform4f(gl, program, "v1", [
		1.65,
		1.22,
		1.69,
		1.04
	]);
	setUniform4f(gl, program, "v2", [
		1.05,
		1.3,
		1.55,
		1.23
...
function runBenchmark(gl, program) {
	return __awaiter(this, void 0, void 0, function* () {
		...
		while (true) {
			...
			if (loopCount < maxLoopCount) loopCount += remainingBudgetMs < duration ? 10 : .6 * loopCount * remainingBudgetMs / duration;
		}
		return Math.max(...flops) * 256 * width * height / 1e6;
	});
}
var gflopsClusterCenters = new Map([
	[34, 1],
	[134, 2],
	[385, 3],
	[783, 4],
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/benchmark/estimateLensPerformanceCluster.js
var findNearest = (n, arr) => arr.reduce((a, b) => Math.abs(a - n) <= Math.abs(b - n) ? a : b);
...
function estimateLensPerformance() {
	var _a;
	return __awaiter(this, void 0, void 0, function* () {
		const gl = document.createElement("canvas").getContext("webgl2", webglContextAttributes);
		...
		return {
			cluster: (_a = gflopsClusterCenters.get(nearestGflopsClusterCenter)) !== null && _a !== void 0 ? _a : 0,
			benchmarks: [gflops],
			webglRendererInfo
		};
	});
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/handlers/arrayBufferParsingHandler.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/handlers/HandlerChainBuilder.js
var HandlerChainBuilder = class HandlerChainBuilder {
	constructor(inner) {
		this.inner = inner;
	}
	get handler() {
	...
	map(outer) {
		const outerHandler = (req, metadata) => {
			...
			const maybeAbort = () => {
				...
				abort.abort();
				(_a = metadata === null || metadata === void 0 ? void 0 : metadata.signal) === null || _a === void 0 || _a.removeEventListener("abort", maybeAbort);
			};
			(_a = metadata === null || metadata === void 0 ? void 0 : metadata.signal) === null || _a === void 0 || _a.addEventListener("abort", maybeAbort);
			const outerResponse = outer(new Proxy(this.inner, { apply: (target, thisArg, args) => {
				const [req, metadata] = args;
				if (metadata === null || metadata === void 0 ? void 0 : metadata.isSideEffect) innerCompleted = true;
				const abortListeners = [];
				signal.addEventListener = new Proxy(signal.addEventListener, { apply: (target, thisArg, args) => {
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/handlers/retryingHandler.js
var logger$28 = getLogger("retryingHandler");
var logRetry$1 = (responseOrError, sleep) => {
	logger$28.warn(`Retrying handler got failed response:`, responseOrError, `Waited ${sleep} millis, attempting retry now.`);
};
var sleep$1 = (millis) => new Promise((resolve) => setTimeout(resolve, millis));
var randomInRange = (min, max) => Math.round(Math.random() * (max - min) + min);
...
var createRetryingHandler = (options = {}) => {
	...
	const { backoffMultiple, baseSleep, maxSleep, maxRetries, retryPredicate } = Object.assign(Object.assign({}, defaultOptions$3), definedOptions);
		...
		const attemptFn = (priorSleep, retryCount) => __awaiter(void 0, void 0, void 0, function* () {
			...
			try {
				...
				if (retryCount < maxRetries && retryPredicate(response, retryCount)) {
					...
					if ((_a = metadata === null || metadata === void 0 ? void 0 : metadata.signal) === null || _a === void 0 ? void 0 : _a.aborted) return response;
					logRetry$1(response, nextSleep);
					return attemptFn(nextSleep, retryCount + 1);
				}
				return response;
			} catch (error) {
				if (!(error instanceof Error)) throw new Error(`Invalid type caught by retrying handler. Handlers may only throw Errors. Got ${JSON.stringify(error)}`);
				if (error.name === "AbortError") throw error;
				if (retryCount < maxRetries && retryPredicate(error, retryCount)) {
					const nextSleep = yield jitterSleep(priorSleep);
					if ((_b = metadata === null || metadata === void 0 ? void 0 : metadata.signal) === null || _b === void 0 ? void 0 : _b.aborted) throw error;
					logRetry$1(error, nextSleep);
					return attemptFn(nextSleep, retryCount + 1);
				}
				throw error;
			}
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/handlers/noCorsRetryingFetchHandler.js
var logger$27 = getLogger("noCorsRetryingFetchHandler");
var logRetry = (error) => {
	logger$27.warn(`NoCorsRetrying handler got failed response:`, error, `Retrying request with {mode: "no-cors"}.`);
};
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/common/typeguards.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/handlers/timeoutHandler.js
...
var defaultOptions$2 = {
	createError: (request) => {
		...
		return /* @__PURE__ */ new Error(`Request ${destination} timed out by client timeout handler.`);
	},
	timeout: 30 * 1e3
};
var createTimeoutHandler = (options = {}) => {
	const definedOptions = copyDefinedProperties(options);
	const { createError, timeout } = Object.assign(Object.assign({}, defaultOptions$2), definedOptions);
	return (next) => (req, meta) => Promise.race([next(req, meta), sleep(timeout).then(() => Promise.reject(createError(req, meta)))]);
};
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/handlers/defaultFetchHandler.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/handlers/utils.js
function withRequestPriority(request, lowPriority) {
	if (lowPriority) return Object.assign(Object.assign({}, request), { priority: "low" });
	return request;
}
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens/assets/remoteMediaAssetLoaderFactory.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/namespace.js
var Namespace;
(function(Namespace) {
...
//#endregion
//#region node_modules/.pnpm/@bufbuild+protobuf@2.10.2/node_modules/@bufbuild/protobuf/dist/esm/wire/varint.js
/**
...
//#endregion
//#region node_modules/.pnpm/@bufbuild+protobuf@2.10.2/node_modules/@bufbuild/protobuf/dist/esm/proto-int64.js
/**
...
//#endregion
//#region node_modules/.pnpm/@bufbuild+protobuf@2.10.2/node_modules/@bufbuild/protobuf/dist/esm/wire/text-encoding.js
...
//#endregion
//#region node_modules/.pnpm/@bufbuild+protobuf@2.10.2/node_modules/@bufbuild/protobuf/dist/esm/wire/binary-encoding.js
/**
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/google/protobuf/any.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/camera_kit/v3/business_events.js
var CameraKitFlavor$1;
...
})(CameraKitFlavor$1 || (CameraKitFlavor$1 = {}));
var CameraKitConnectivityType$1;
...
})(CameraKitConnectivityType$1 || (CameraKitConnectivityType$1 = {}));
var CameraKitEnvironment$1;
...
})(CameraKitEnvironment$1 || (CameraKitEnvironment$1 = {}));
...
var ExtensionEventBase = {
	encode(message, writer = new BinaryWriter()) {
		if (message.extensionName !== "") writer.uint32(10).string(message.extensionName);
		if (message.extensionVersion !== "") writer.uint32(18).string(message.extensionVersion);
		if (message.deviceCluster !== "0") writer.uint32(24).int64(message.deviceCluster);
		if (message.cameraKitVersion !== "") writer.uint32(34).string(message.cameraKitVersion);
		if (message.lensCoreVersion !== "") writer.uint32(42).string(message.lensCoreVersion);
		if (message.deviceModel !== "") writer.uint32(50).string(message.deviceModel);
		if (message.cameraKitFlavor !== 0) writer.uint32(56).int32(message.cameraKitFlavor);
		if (message.appId !== "") writer.uint32(66).string(message.appId);
		if (message.deviceConnectivity !== 0) writer.uint32(72).int32(message.deviceConnectivity);
		if (message.sessionId !== "") writer.uint32(82).string(message.sessionId);
		if (message.cameraKitEnvironment !== 0) writer.uint32(88).int32(message.cameraKitEnvironment);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/google/protobuf/timestamp.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/camera_kit/v3/legal_prompt.js
...
var LegalPrompt = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.documents) LegalDocument.encode(v, writer.uint32(10).fork()).join();
		if (message.disabled !== false) writer.uint32(16).bool(message.disabled);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var LegalDocument = {
	encode(message, writer = new BinaryWriter()) {
		...
		if (message.webUrl !== "") writer.uint32(18).string(message.webUrl);
		if (message.version !== "") writer.uint32(26).string(message.version);
		if (message.timestamp !== void 0) Timestamp.encode(toTimestamp$4(message.timestamp), writer.uint32(34).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				...
				case 4:
					if (tag !== 34) break;
					message.timestamp = fromTimestamp$4(Timestamp.decode(reader, reader.uint32()));
					continue;
			}
			if ((tag & 7) === 4 || tag === 0) break;
			reader.skip(tag & 7);
...
function toTimestamp$4(date) {
	return {
		seconds: Math.trunc(date.getTime() / 1e3).toString(),
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/camera_kit/v3/lens.js
...
var Lens = {
	encode(message, writer = new BinaryWriter()) {
		...
		Object.entries(message.vendorData).forEach(([key, value]) => {
			Lens_VendorDataEntry.encode({
				key,
				value
			}, writer.uint32(26).fork()).join();
		});
		if (message.content !== void 0) Content.encode(message.content, writer.uint32(34).fork()).join();
		if (message.isThirdParty !== false) writer.uint32(40).bool(message.isThirdParty);
		if (message.cameraFacingPreference !== 0) writer.uint32(48).int32(message.cameraFacingPreference);
		for (const v of message.featureMetadata) Any.encode(v, writer.uint32(58).fork()).join();
		if (message.lensCreator !== void 0) LensCreator.encode(message.lensCreator, writer.uint32(66).fork()).join();
		if (message.scannable !== void 0) Scannable.encode(message.scannable, writer.uint32(74).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				...
				case 3: {
					...
					if (entry3.value !== void 0) message.vendorData[entry3.key] = entry3.value;
					continue;
				}
				case 4:
					if (tag !== 34) break;
...
var Content = {
	encode(message, writer = new BinaryWriter()) {
		if (message.lnsUrl !== "") writer.uint32(10).string(message.lnsUrl);
		if (message.lnsSha256 !== "") writer.uint32(18).string(message.lnsSha256);
		if (message.iconUrl !== "") writer.uint32(26).string(message.iconUrl);
		if (message.preview !== void 0) Preview.encode(message.preview, writer.uint32(34).fork()).join();
		for (const v of message.assetManifest) LensAssetManifestItem.encode(v, writer.uint32(42).fork()).join();
		if (message.defaultHintId !== "") writer.uint32(50).string(message.defaultHintId);
		Object.entries(message.hintTranslations).forEach(([key, value]) => {
			Content_HintTranslationsEntry.encode({
				key,
				value
			}, writer.uint32(58).fork()).join();
		});
		if (message.lnsUrlBolt !== "") writer.uint32(66).string(message.lnsUrlBolt);
		if (message.iconUrlBolt !== "") writer.uint32(74).string(message.iconUrlBolt);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				...
				case 7: {
					...
					if (entry7.value !== void 0) message.hintTranslations[entry7.key] = entry7.value;
					continue;
				}
				case 8:
					if (tag !== 66) break;
...
var LensAssetManifestItem = {
	encode(message, writer = new BinaryWriter()) {
		if (message.type !== 0) writer.uint32(8).int32(message.type);
		if (message.id !== "") writer.uint32(18).string(message.id);
		if (message.requestTiming !== 0) writer.uint32(24).int32(message.requestTiming);
		if (message.assetUrl !== "") writer.uint32(34).string(message.assetUrl);
		if (message.assetChecksum !== "") writer.uint32(42).string(message.assetChecksum);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var Preview = {
	encode(message, writer = new BinaryWriter()) {
		...
		if (message.imageSequenceSize !== 0) writer.uint32(16).int32(message.imageSequenceSize);
		if (message.imageSequenceWebpUrlPattern !== "") writer.uint32(26).string(message.imageSequenceWebpUrlPattern);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var LensCreator = {
	encode(message, writer = new BinaryWriter()) {
		if (message.displayName !== "") writer.uint32(10).string(message.displayName);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var Scannable = {
	encode(message, writer = new BinaryWriter()) {
		if (message.snapcodeImageUrl !== "") writer.uint32(10).string(message.snapcodeImageUrl);
		if (message.snapcodeDeeplink !== "") writer.uint32(18).string(message.snapcodeDeeplink);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/camera_kit/v3/operational_metrics.js
...
var OperationalMetric = {
	encode(message, writer = new BinaryWriter()) {
		...
		if (message.timestamp !== void 0) Timestamp.encode(toTimestamp$3(message.timestamp), writer.uint32(18).fork()).join();
	...
	decode(input, length) {
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				...
				case 2:
					if (tag !== 18) break;
					message.timestamp = fromTimestamp$3(Timestamp.decode(reader, reader.uint32()));
					continue;
				case 3:
					if (tag !== 24) break;
					message.metric = {
						$case: "count",
						count: reader.uint64().toString()
					};
					continue;
				case 4:
					if (tag !== 32) break;
					message.metric = {
						$case: "latencyMillis",
						latencyMillis: reader.uint64().toString()
					};
					continue;
				case 5:
					if (tag !== 40) break;
					message.metric = {
						$case: "histogram",
						histogram: reader.int64().toString()
					};
					continue;
			}
			if ((tag & 7) === 4 || tag === 0) break;
	...
	fromPartial(object) {
		...
		if (((_c = object.metric) === null || _c === void 0 ? void 0 : _c.$case) === "count" && ((_d = object.metric) === null || _d === void 0 ? void 0 : _d.count) !== void 0 && ((_e = object.metric) === null || _e === void 0 ? void 0 : _e.count) !== null) message.metric = {
			$case: "count",
			count: object.metric.count
		};
		if (((_f = object.metric) === null || _f === void 0 ? void 0 : _f.$case) === "latencyMillis" && ((_g = object.metric) === null || _g === void 0 ? void 0 : _g.latencyMillis) !== void 0 && ((_h = object.metric) === null || _h === void 0 ? void 0 : _h.latencyMillis) !== null) message.metric = {
			$case: "latencyMillis",
			latencyMillis: object.metric.latencyMillis
		};
		if (((_j = object.metric) === null || _j === void 0 ? void 0 : _j.$case) === "histogram" && ((_k = object.metric) === null || _k === void 0 ? void 0 : _k.histogram) !== void 0 && ((_l = object.metric) === null || _l === void 0 ? void 0 : _l.histogram) !== null) message.metric = {
			$case: "histogram",
			histogram: object.metric.histogram
		};
		return message;
	}
};
...
var OperationalMetricsBundle = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.metrics) OperationalMetric.encode(v, writer.uint32(10).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/camera_kit/v3/ranking.js
...
var RankingData = {
	encode(message, writer = new BinaryWriter()) {
		if (message.sessionId !== "") writer.uint32(10).string(message.sessionId);
		if (message.locale !== "") writer.uint32(18).string(message.locale);
		if (message.osType !== 0) writer.uint32(24).int32(message.osType);
		if (message.connectivityType !== 0) writer.uint32(32).int32(message.connectivityType);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/camera_kit/v3/service.js
...
var GetGroupRequest = {
	encode(message, writer = new BinaryWriter()) {
		if (message.id !== "") writer.uint32(10).string(message.id);
		if (message.rankingData !== void 0) RankingData.encode(message.rankingData, writer.uint32(18).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var GetGroupResponse = {
	encode(message, writer = new BinaryWriter()) {
		if (message.id !== "") writer.uint32(10).string(message.id);
		for (const v of message.lenses) Lens.encode(v, writer.uint32(18).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var GetGroupLensRequest = {
	encode(message, writer = new BinaryWriter()) {
		if (message.lensId !== "") writer.uint32(10).string(message.lensId);
...
var GetGroupLensResponse = {
	encode(message, writer = new BinaryWriter()) {
		if (message.lens !== void 0) Lens.encode(message.lens, writer.uint32(10).fork()).join();
		if (message.groupId !== "") writer.uint32(18).string(message.groupId);
		return writer;
	},
	decode(input, length) {
...
var BatchGetGroupLensRequest = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.getRequests) GetGroupLensRequest.encode(v, writer.uint32(10).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var BatchGetGroupLensResponse = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.getResponses) GetGroupLensResponse.encode(v, writer.uint32(10).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var GetPlaceholderConfigRequest = {
	...
	fromPartial(_) {
		return createBaseGetPlaceholderConfigRequest();
	}
};
...
var GetPlaceholderConfigResponse = {
	encode(message, writer = new BinaryWriter()) {
		Object.entries(message.configs).forEach(([key, value]) => {
			GetPlaceholderConfigResponse_ConfigsEntry.encode({
				key,
				value
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				case 1: {
					...
					if (entry1.value !== void 0) message.configs[entry1.key] = entry1.value;
					continue;
				}
			}
			if ((tag & 7) === 4 || tag === 0) break;
...
var GetInitializationConfigRequest = {
	...
	fromPartial(_) {
		return createBaseGetInitializationConfigRequest();
	}
};
...
var GetInitializationConfigResponse = {
	encode(message, writer = new BinaryWriter()) {
		if (message.appVendorUuidOptIn !== false) writer.uint32(8).bool(message.appVendorUuidOptIn);
		if (message.watermarkEnabled !== false) writer.uint32(16).bool(message.watermarkEnabled);
		if (message.childrenProtectionActRestricted !== false) writer.uint32(24).bool(message.childrenProtectionActRestricted);
		if (message.legalPrompt !== void 0) LegalPrompt.encode(message.legalPrompt, writer.uint32(34).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var SetOperationalMetricsRequest = {
	encode(message, writer = new BinaryWriter()) {
		if (message.metrics !== void 0) OperationalMetricsBundle.encode(message.metrics, writer.uint32(10).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var SetOperationalMetricsResponse = {
	...
	fromPartial(_) {
		return createBaseSetOperationalMetricsResponse();
	}
};
...
var SetBusinessEventsRequest = {
	encode(message, writer = new BinaryWriter()) {
		if (message.batchEvents !== void 0) Any.encode(message.batchEvents, writer.uint32(10).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var SetBusinessEventsResponse = {
	...
	fromPartial(_) {
		return createBaseSetBusinessEventsResponse();
	}
};
function createBaseSetExtensionBusinessEventsRequest() {
	return {
		events: [],
		extensionEventBase: void 0
	};
}
var SetExtensionBusinessEventsRequest = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.events) Any.encode(v, writer.uint32(10).fork()).join();
		if (message.extensionEventBase !== void 0) ExtensionEventBase.encode(message.extensionEventBase, writer.uint32(18).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var SetExtensionBusinessEventsResponse = {
	...
	fromPartial(_) {
		return createBaseSetExtensionBusinessEventsResponse();
	}
};
	...
	methods: {
		getGroup: {
			...
			options: {
				...
				_unknownFields: { 578365826: [new Uint8Array([
					43,
					18,
					41,
					47,
...
var MetricsDefinition = {
	...
	methods: {
		setOperationalMetrics: {
			...
			responseStream: false,
			options: { _unknownFields: { 578365826: [new Uint8Array([
				63,
				34,
				58,
				47,
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/common/result.js
var import_grpc_web_client_umd = (/* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(e, t) {
		...
		return e = {
				...
				(function(e, t) {
					...
					function(e, t, r) {
						...
						t.BrowserHeaders = function() {
							function e(e, t) {
								void 0 === e && (e = {}), void 0 === t && (t = { splitValues: !1 });
								var r, o = this;
							...
							return e.prototype.appendFromString = function(e) {
								for (var t = e.split("\r\n"), r = 0; r < t.length; r++) {
									var n = t[r], o = n.indexOf(":");
									if (o > 0) {
										var s = n.substring(0, o).trim(), i = n.substring(o + 1).trim();
										this.append(s, i);
									}
								}
								...
								var r = this, o = n.normalizeName(e);
					...
					function(e, t, r) {
						"use strict";
						Object.defineProperty(t, "__esModule", { value: !0 }), t.iterateHeaders = function(e, t) {
			...
			617: function(e, t, r) {
				"use strict";
				Object.defineProperty(t, "__esModule", { value: !0 }), t.ChunkParser = t.ChunkType = t.encodeASCII = t.decodeASCII = void 0;
				var n, o = r(65);
				function s(e) {
					return 9 === (t = e) || 10 === t || 13 === t || e >= 32 && e <= 126;
					var t;
				}
				function i(e) {
					for (var t = 0; t !== e.length; ++t) if (!s(e[t])) throw new Error("Metadata is not valid (printable) ASCII");
				...
				t.decodeASCII = i, t.encodeASCII = function(e) {
					for (var t = new Uint8Array(e.length), r = 0; r !== e.length; ++r) {
						...
						if (!s(n)) throw new Error("Metadata contains invalid ASCII");
						t[r] = n;
					}
					return t;
				}, function(e) {
				...
				t.ChunkParser = function() {
					...
					return e.prototype.parse = function(e, t) {
						...
						if (null == this.buffer) this.buffer = e, this.position = 0;
							...
							var p = this.buffer.byteLength - this.position, h = new Uint8Array(p + e.byteLength), f = c(this.buffer, this.position);
							h.set(f, 0);
							var l = new Uint8Array(e);
							h.set(l, p), this.buffer = h, this.position = 0;
						}
						for (;;) {
							if (!d(this.buffer, this.position, 5)) return s;
							var g = c(this.buffer, this.position, this.position + 5), b = new DataView(g.buffer, g.byteOffset, g.byteLength), y = u(b);
							if (!d(this.buffer, this.position, 5 + y)) return s;
							var v = c(this.buffer, this.position + 5, this.position + 5 + y);
							if (this.position += 5 + y, a(b)) return s.push({
								chunkType: n.TRAILERS,
								trailers: (r = v, new o.Metadata(i(r)))
							}), s;
							s.push({
								chunkType: n.MESSAGE,
								data: v
			...
			8: function(e, t) {
				"use strict";
				var r;
				Object.defineProperty(t, "__esModule", { value: !0 }), t.httpStatusToCode = t.Code = void 0, function(e) {
					...
					switch (e) {
						case 0: return r.Internal;
						case 200: return r.OK;
						case 400: return r.InvalidArgument;
						case 401: return r.Unauthenticated;
						case 403: return r.PermissionDenied;
						case 404: return r.NotFound;
						case 409: return r.Aborted;
						case 412: return r.FailedPrecondition;
						case 429: return r.ResourceExhausted;
						case 499: return r.Canceled;
						case 500: return r.Unknown;
						case 501: return r.Unimplemented;
						case 503: return r.Unavailable;
						case 504: return r.DeadlineExceeded;
					...
					return e.prototype.createTransport = function() {
						...
						if (this.closed) this.props.debug && i.debug("grpc.onChunk received after request was closed - ignoring");
							...
							try {
								...
								this.props.debug && i.debug("onChunk.parsing error", e, e.message), this.rawOnError(s.Code.Internal, "parsing error: " + e.message);
								return;
							}
						...
						if (this.props.debug && i.debug("grpc.onEnd"), this.closed) this.props.debug && i.debug("grpc.onEnd received after request was closed - ignoring");
							...
							if (null !== e) {
								var t = this.responseTrailers.get("grpc-message"), r = this.decodeGRPCStatus(t[0]);
							...
							var n = c(this.responseHeaders), o = this.responseHeaders.get("grpc-message");
			...
			346: function(e, t) {
				"use strict";
				Object.defineProperty(t, "__esModule", { value: !0 }), t.debug = void 0, t.debug = function() {
			...
			607: function(e, t, r) {
				"use strict";
				Object.defineProperty(t, "__esModule", { value: !0 }), t.grpc = void 0;
				var n, o = r(418), s = r(57), i = r(229), a = r(540), u = r(210), d = r(859), c = r(8), p = r(938), h = r(35), f = r(934);
				(n = t.grpc || (t.grpc = {})).setDefaultTransport = s.setDefaultTransportFactory, n.CrossBrowserHttpTransport = d.CrossBrowserHttpTransport, n.FetchReadableStreamTransport = i.FetchReadableStreamTransport, n.XhrTransport = u.XhrTransport, n.WebsocketTransport = a.WebsocketTransport, n.Code = c.Code, n.Metadata = o.BrowserHeaders, n.client = function(e, t) {
					return f.client(e, t);
				}, n.invoke = p.invoke, n.unary = h.unary;
			},
			938: function(e, t, r) {
				"use strict";
				Object.defineProperty(t, "__esModule", { value: !0 }), t.invoke = void 0;
				var n = r(934);
				t.invoke = function(e, t) {
					...
					return t.onHeaders && r.onHeaders(t.onHeaders), t.onMessage && r.onMessage(t.onMessage), t.onEnd && r.onEnd(t.onEnd), r.start(t.metadata), r.send(t.request), r.finishSend(), { close: function() {
						r.close();
					} };
				};
			},
			...
			57: function(e, t, r) {
				"use strict";
				Object.defineProperty(t, "__esModule", { value: !0 }), t.makeDefaultTransport = t.setDefaultTransportFactory = void 0;
				var n = r(859), o = function(e) {
					return n.CrossBrowserHttpTransport({ withCredentials: !1 })(e);
				};
			...
			229: function(e, t, r) {
				...
				Object.defineProperty(t, "__esModule", { value: !0 }), t.detectFetchSupport = t.FetchReadableStreamTransport = void 0;
				var o = r(65), s = r(346);
				...
				var i = function() {
					...
					return e.prototype.pump = function(e, t) {
						var r = this;
						if (this.reader = e, this.cancelled) return this.options.debug && s.debug("Fetch.pump.cancel at first pump"), void this.reader.cancel().catch((function(e) {
							r.options.debug && s.debug("Fetch.pump.reader.cancel exception", e);
						}));
						...
						var e = this;
						this.cancelled ? this.options.debug && s.debug("Fetch.cancel already cancelled") : (this.cancelled = !0, this.controller ? (this.options.debug && s.debug("Fetch.cancel.controller.abort"), this.controller.abort()) : this.options.debug && s.debug("Fetch.cancel.missing abort controller"), this.reader ? (this.options.debug && s.debug("Fetch.cancel.reader.cancel"), this.reader.cancel().catch((function(t) {
							e.options.debug && s.debug("Fetch.cancel.reader.cancel exception", t);
			...
			859: function(e, t, r) {
				"use strict";
				Object.defineProperty(t, "__esModule", { value: !0 }), t.CrossBrowserHttpTransport = void 0;
				var n = r(229), o = r(210);
				t.CrossBrowserHttpTransport = function(e) {
					if (n.detectFetchSupport()) {
						var t = { credentials: e.withCredentials ? "include" : "same-origin" };
						return n.FetchReadableStreamTransport(t);
					}
					return o.XhrTransport({ withCredentials: e.withCredentials });
				};
			},
			210: function(e, t, r) {
				"use strict";
				...
				Object.defineProperty(t, "__esModule", { value: !0 }), t.stringToArrayBuffer = t.MozChunkedArrayBufferXHR = t.XHR = t.XhrTransport = void 0;
			...
			849: function(e, t) {
				...
				Object.defineProperty(t, "__esModule", { value: !0 }), t.detectXHROverrideMimeTypeSupport = t.detectMozXHRSupport = t.xhrSupportsResponseType = void 0, t.xhrSupportsResponseType = o, t.detectMozXHRSupport = function() {
			...
			540: function(e, t, r) {
				"use strict";
				Object.defineProperty(t, "__esModule", { value: !0 }), t.WebsocketTransport = void 0;
				...
				t.WebsocketTransport = function() {
					return function(e) {
						return function(e) {
							...
							function u(e) {
								if (e === n.FINISH_SEND) t.send(i);
								else {
									var r = e, o = new Int8Array(r.byteLength + 1);
			...
			35: function(e, t, r) {
				"use strict";
				Object.defineProperty(t, "__esModule", { value: !0 }), t.unary = void 0;
				var n = r(65), o = r(934);
				t.unary = function(e, t) {
					...
					var r = null, s = null, i = o.client(e, {
						host: t.host,
						transport: t.transport,
						debug: t.debug
					});
					...
					})), i.start(t.metadata), i.send(t.request), i.finishSend(), { close: function() {
						i.close();
					} };
				};
			},
			882: function(e, t) {
				"use strict";
				Object.defineProperty(t, "__esModule", { value: !0 }), t.frameRequest = void 0, t.frameRequest = function(e) {
					var t = e.serializeBinary(), r = new ArrayBuffer(t.byteLength + 5);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/common/memoize.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/environment.js
var environment_default = { PACKAGE_VERSION: "1.13.0" };
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lensCoreWasmVersions.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/platform/platformInfo.js
...
function parseDeviceModel(userAgent) {
	...
	if (userAgentWithModel) return userAgentWithModel[1].trim();
	const userAgentWithModel2 = userAgent.match(/\(([^;]+);/);
	if (userAgentWithModel2) return userAgentWithModel2[1].trim();
	return "unknown";
}
function parseOrigin() {
	...
	for (let origin of possibleOrigins) try {
...
function parseOSName(userAgent) {
	...
	for (const [match, platform] of knownPlatforms.entries()) if (normalizedUserAgent.includes(match)) return platform;
	return "unknown";
}
function parseOSVersion(userAgent) {
	const versionMatch = userAgent.match(/\s([\d][\d_.]*[\d])(;|\)|\s)/);
	if (versionMatch != null) return versionMatch[1].replace(/_/g, ".");
	return "";
}
...
var getPlatformInfo = memoize(function getPlatformInfo() {
	var _a, _b, _c;
	...
	const fullLocale = ((_a = navigator.languages) !== null && _a !== void 0 ? _a : []).map((lang, index) => {
		return `${lang};q=${Math.max(0, (10 - index) / 10).toFixed(1)}`;
	}).join(", ") || locale;
	return {
		sdkShortVersion,
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/platform/cameraKitUserAgent.js
var getCameraKitUserAgent = memoize(function getCameraKitUserAgent() {
	var _a;
	...
	return `CameraKitWeb/${sdkShortVersion} ${userAgentFlavor === "release" ? "" : "DEBUG "}(${deviceModel}; ${osName} ${osVersion}) ${browser.brand}/${browser.version} Core/${lensCore.version} AppId/${origin}`;
});
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/handlers/headersModifyingFetchHandler.js
var createHeadersModifyingFetchHandler = (modifyHeaders) => (next) => (input, init) => {
	const modifiedHeaders = modifyHeaders(init && init.headers ? new Headers(init.headers) : typeof input === "string" ? new Headers() : input.headers);
	return next(input, Object.assign(Object.assign({}, init), { headers: modifiedHeaders }));
};
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/handlers/cameraKitServiceFetchHandlerFactory.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/common/errorHelpers.js
...
function ensureError(error) {
	...
	try {
		return /* @__PURE__ */ new Error(`Non-Error type exception thrown. Serialized error value: ${JSON.stringify(error)}`);
	} catch (_) {
		return /* @__PURE__ */ new Error("Non-Error type exception thrown. Original error value could not be serialized.");
	}
}
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/events/TypedCustomEvent.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/events/TypedEventTarget.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/common/time.js
var getTimeMs = () => performance.now();
var convertDaysToSeconds = (days) => days * 24 * 60 * 60;
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/operational/Metric.js
var nameDelimiter = "_";
...
function serializeMetricDimensions(dimensions) {
	...
	return `${dimensionDelimiter}${Array.from(Object.entries(dimensions)).map((d) => d.join(dimensionDelimiter)).join(dimensionDelimiter)}`;
}
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/operational/Timer.js
var Timer = class Timer extends Metric {
	constructor(name, dimensions = {}) {
	...
	measure(nameOrDimensions, maybeDimensions) {
		...
		const measure = {
			name: joinMetricNames([this.name, name]),
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/handlers/requestStateEmittingHandler.js
var requestId = 0;
...
var dispatchRequestStarted = (requestStateEventTarget, data) => {
	const started = Object.assign(Object.assign({}, data), {
		requestId: requestId++,
		timer: new Timer("download_latency")
	});
...
var createRequestStateEmittingHandler = (requestStateEventTarget) => (next) => ([request, dimensions], metadata) => __awaiter(void 0, void 0, void 0, function* () {
	...
	try {
		...
		dispatchRequestCompleted(requestStateEventTarget, {
			...
			dimensions,
			status,
			sizeByte: safeParseInt(response.headers.get("content-length"))
		});
		return response;
	} catch (error) {
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/clients/grpcHandler.js
...
var grpcHandlerFactory = Injectable("grpcHandlerFactory", [
	configurationToken,
	cameraKitServiceFetchHandlerFactory.token,
	requestStateEventTargetFactory.token
], (configuration, fetchHandler, requestStateEventTarget) => {
	...
	const transport = (options) => {
		...
		return {
			sendMessage(msgBytes) {
				...
				(isMetricsGrpcMethod(options.methodDefinition.methodName) ? fetchHandler(options.url, requestInit) : fetchHandlerWithMetrics([options.url, metricsDimensions], requestInit)).then((response) => {
					options.onHeaders(new import_grpc_web_client_umd.grpc.Metadata(response.headers), response.status);
	...
	return (request) => __awaiter(void 0, void 0, void 0, function* () {
		return new Promise((resolve) => {
			import_grpc_web_client_umd.grpc.unary({
				...
				host,
				onEnd: (response) => {
					if (isUnaryOutputOk(response)) resolve(Ok(response));
					else resolve(Err(response));
				},
				transport
			});
		});
...
function isUnaryOutputOk(value) {
	return value.status === import_grpc_web_client_umd.grpc.Code.OK;
}
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/clients/createTsProtoClient.js
...
function createTsProtoClient(serviceDefinition, handler) {
	return fromEntries$1(entries(serviceDefinition.methods).map(([methodName, methodDefinition]) => {
		return [methodName, (request) => __awaiter(this, void 0, void 0, function* () {
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/common/unionBy.js
var import_browser_headers_umd = (/* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function webpackUniversalModuleDefinition(root, factory) {
		...
		return (function(modules) {
			...
			(function(module$3, exports$2, __webpack_require__) {
				...
				exports$2.BrowserHeaders = function() {
					function BrowserHeaders(init, options) {
						...
						this.headersMap = {};
						if (init) {
							if (typeof Headers !== "undefined" && init instanceof Headers) util_1.getHeaderKeys(init).forEach(function(key) {
								util_1.getHeaderValues(init, key).forEach(function(value) {
									if (options.splitValues) _this.append(key, util_1.splitHeaderValue(value));
									else _this.append(key, value);
								});
							});
							else if (isBrowserHeaders(init)) init.forEach(function(key, values) {
								_this.append(key, values);
							});
							else if (typeof Map !== "undefined" && init instanceof Map) init.forEach(function(value, key) {
								_this.append(key, value);
							});
							else if (typeof init === "string") this.appendFromString(init);
							else if (typeof init === "object") Object.getOwnPropertyNames(init).forEach(function(key) {
								var values = init[key];
								if (Array.isArray(values)) values.forEach(function(value) {
									_this.append(key, value);
								});
								else _this.append(key, values);
							});
						}
					}
					...
					BrowserHeaders.prototype.delete = function(key, value) {
						...
						if (value === void 0) delete this.headersMap[normalizedKey];
						else {
							var existing = this.headersMap[normalizedKey];
							if (existing) {
								var index = existing.indexOf(value);
								if (index >= 0) existing.splice(index, 1);
								if (existing.length === 0) delete this.headersMap[normalizedKey];
							}
						}
					};
					BrowserHeaders.prototype.append = function(key, value) {
						var _this = this;
						var normalizedKey = util_1.normalizeName(key);
						if (!Array.isArray(this.headersMap[normalizedKey])) this.headersMap[normalizedKey] = [];
						if (Array.isArray(value)) value.forEach(function(arrayValue) {
							_this.headersMap[normalizedKey].push(util_1.normalizeValue(arrayValue));
						});
						else this.headersMap[normalizedKey].push(util_1.normalizeValue(value));
					};
					BrowserHeaders.prototype.set = function(key, value) {
						...
						} else this.headersMap[normalizedKey] = [util_1.normalizeValue(value)];
					};
					BrowserHeaders.prototype.has = function(key, value) {
						var keyArray = this.headersMap[util_1.normalizeName(key)];
						if (!Array.isArray(keyArray)) return false;
						if (value !== void 0) {
					...
					BrowserHeaders.prototype.get = function(key) {
						...
						if (values !== void 0) return values.concat();
						return [];
					};
					...
					BrowserHeaders.prototype.toHeaders = function() {
						if (typeof Headers !== "undefined") {
							...
							return headers_1;
						} else throw new Error("Headers class is not defined");
					};
			...
			(function(module$4, exports$3, __webpack_require__) {
				...
				exports$3.BrowserHeaders = __webpack_require__(0).BrowserHeaders;
			}),
			(function(module$5, exports$4, __webpack_require__) {
				...
				exports$4.iterateHeaders = iterateHeaders;
				...
				exports$4.iterateHeadersKeys = iterateHeadersKeys;
			}),
			(function(module$6, exports$5, __webpack_require__) {
				...
				exports$5.normalizeName = normalizeName;
				function normalizeValue(value) {
					if (typeof value !== "string") value = String(value);
					return value;
				}
				exports$5.normalizeValue = normalizeValue;
				function getHeaderValues(headersAsNative, key) {
					var headers = toWindowHeaders(headersAsNative);
					if (headers instanceof Headers && headers.getAll) return headers.getAll(key);
					var getValue = headers.get(key);
					if (getValue && typeof getValue === "string") return [getValue];
					return getValue;
				}
				exports$5.getHeaderValues = getHeaderValues;
				...
				function getHeaderKeys(headersAsNative) {
					...
					var asMap = {};
					var keys = [];
					if (headers.keys) iterateHeaders_1.iterateHeadersKeys(headers, function(key) {
						if (!asMap[key]) {
							asMap[key] = true;
							keys.push(key);
						}
					});
					else if (headers.forEach) headers.forEach(function(_, key) {
						if (!asMap[key]) {
							asMap[key] = true;
							keys.push(key);
						}
					});
					else iterateHeaders_1.iterateHeaders(headers, function(entry) {
						var key = entry[0];
						if (!asMap[key]) {
							asMap[key] = true;
							keys.push(key);
				...
				exports$5.getHeaderKeys = getHeaderKeys;
				function splitHeaderValue(str) {
					var values = [];
					str.split(", ").forEach(function(commaSpaceValue) {
				...
				exports$5.splitHeaderValue = splitHeaderValue;
			})
		]);
	});
})))();
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/common/ruid.js
var Ruid_Type;
(function(Ruid_Type) {
...
var Ruid = {
	encode(message, writer = new BinaryWriter()) {
		if (message.type !== 0) writer.uint32(8).int32(message.type);
		if (message.stringValue !== "") writer.uint32(18).string(message.stringValue);
		if (message.loggingIdValue !== "") writer.uint32(26).string(message.loggingIdValue);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/google/protobuf/wrappers.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/benchmark.js
...
var BenchmarkValue = {
	encode(message, writer = new BinaryWriter()) {
		if (message.intValue !== void 0) writer.uint32(8).int32(message.intValue);
		if (message.longValue !== void 0) writer.uint32(16).int64(message.longValue);
		if (message.boolValue !== void 0) writer.uint32(24).bool(message.boolValue);
		if (message.floatValue !== void 0) writer.uint32(37).float(message.floatValue);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var BenchmarkRequest = {
	encode(message, writer = new BinaryWriter()) {
		...
		for (const v of message.benchmarkNames) writer.int32(v);
		writer.join();
		if (message.expirationTimestamp !== void 0) Timestamp.encode(toTimestamp$2(message.expirationTimestamp), writer.uint32(18).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
			...
			switch (tag >>> 3) {
				case 1:
					...
					if (tag === 10) {
						...
						while (reader.pos < end2) message.benchmarkNames.push(reader.int32());
						continue;
					}
					break;
				case 2:
					if (tag !== 18) break;
					message.expirationTimestamp = fromTimestamp$2(Timestamp.decode(reader, reader.uint32()));
					continue;
			}
			if ((tag & 7) === 4 || tag === 0) break;
			reader.skip(tag & 7);
...
var BenchmarkResult = {
	encode(message, writer = new BinaryWriter()) {
		if (message.name !== 0) writer.uint32(8).int32(message.name);
		if (message.value !== void 0) BenchmarkValue.encode(message.value, writer.uint32(18).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/config_request.js
...
var ConfigTargetingRequest = {
	encode(message, writer = new BinaryWriter()) {
		if (message.configResultsEtag !== "") writer.uint32(10).string(message.configResultsEtag);
		writer.uint32(18).fork();
		for (const v of message.namespaces) writer.int32(v);
		writer.join();
		if (message.ghostId !== "") writer.uint32(26).string(message.ghostId);
		if (message.countryCode !== "") writer.uint32(34).string(message.countryCode);
		if (message.screenWidth !== 0) writer.uint32(40).int32(message.screenWidth);
		if (message.screenHeight !== 0) writer.uint32(48).int32(message.screenHeight);
		if (message.connectivity !== void 0) Connectivity.encode(message.connectivity, writer.uint32(58).fork()).join();
		if (message.maxVideoWidthPx !== 0) writer.uint32(64).int32(message.maxVideoWidthPx);
		if (message.maxVideoHeightPx !== 0) writer.uint32(72).int32(message.maxVideoHeightPx);
		for (const v of message.benchmarkResults) BenchmarkResult.encode(v, writer.uint32(82).fork()).join();
		if (message.deltaSync !== false) writer.uint32(88).bool(message.deltaSync);
		if (message.userAgent !== "") writer.uint32(98).string(message.userAgent);
		if (message.triggerEventType !== 0) writer.uint32(104).int32(message.triggerEventType);
		if (message.appState !== 0) writer.uint32(112).int32(message.appState);
		if (message.locale !== "") writer.uint32(122).string(message.locale);
		if (message.deviceId !== "") writer.uint32(130).string(message.deviceId);
		if (message.userId !== "") writer.uint32(138).string(message.userId);
		if (message.clientIp !== "") writer.uint32(146).string(message.clientIp);
		if (message.isUnAuthorized !== false) writer.uint32(152).bool(message.isUnAuthorized);
		if (message.appLocale !== "") writer.uint32(162).string(message.appLocale);
		if (message.instrumentation !== 0) writer.uint32(168).int32(message.instrumentation);
		if (message.lastSuccessfulSync !== "0") writer.uint32(176).int64(message.lastSuccessfulSync);
		if (message.isLogout !== false) writer.uint32(184).bool(message.isLogout);
		if (message.packageInstaller !== "") writer.uint32(194).string(message.packageInstaller);
		if (message.syncTriggerBlizzardSessionId !== "") writer.uint32(202).string(message.syncTriggerBlizzardSessionId);
		if (message.syncExecutionBlizzardSessionId !== "") writer.uint32(210).string(message.syncExecutionBlizzardSessionId);
		if (message.cofSyncTriggerDelayFromStartupMs !== 0) writer.uint32(216).int32(message.cofSyncTriggerDelayFromStartupMs);
		if (message.cofSyncExecutionDelayFromStartupMs !== 0) writer.uint32(224).int32(message.cofSyncExecutionDelayFromStartupMs);
		if (message.syncTriggerTime !== "0") writer.uint32(232).int64(message.syncTriggerTime);
		if (message.decoderEncoderAvailability !== void 0) DecoderEncoderAvailablity.encode(message.decoderEncoderAvailability, writer.uint32(242).fork()).join();
		if (message.snapkitAppId !== "") writer.uint32(250).string(message.snapkitAppId);
		if (message.lenscoreVersion !== 0) writer.uint32(256).int32(message.lenscoreVersion);
		if (message.ruid !== void 0) Ruid.encode(message.ruid, writer.uint32(266).fork()).join();
		for (const v of message.configNames) writer.uint32(274).string(v);
		if (message.includeTestUserTreatments !== false) writer.uint32(288).bool(message.includeTestUserTreatments);
		if (message.disableExposureLogging !== false) writer.uint32(296).bool(message.disableExposureLogging);
		if (message.lensClusterOrig4 !== 0) writer.uint32(304).int32(message.lensClusterOrig4);
		if (message.clientId !== "") writer.uint32(314).string(message.clientId);
		if (message.propertyOverrides !== void 0) PropertyOverrides.encode(message.propertyOverrides, writer.uint32(322).fork()).join();
		if (message.forLockscreenMode !== false) writer.uint32(328).bool(message.forLockscreenMode);
		if (message.osBuildVersion !== "") writer.uint32(338).string(message.osBuildVersion);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				...
				case 2:
					...
					if (tag === 18) {
						const end2 = reader.uint32() + reader.pos;
						while (reader.pos < end2) message.namespaces.push(reader.int32());
						continue;
					}
					break;
				case 3:
...
var PropertyOverrides = {
	encode(message, writer = new BinaryWriter()) {
		if (message.isWeb !== false) writer.uint32(8).bool(message.isWeb);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var Connectivity = {
	encode(message, writer = new BinaryWriter()) {
		if (message.networkType !== 0) writer.uint32(8).int32(message.networkType);
		if (message.isMetered !== void 0) BoolValue.encode({ value: message.isMetered }, writer.uint32(18).fork()).join();
		if (message.isRoaming !== void 0) BoolValue.encode({ value: message.isRoaming }, writer.uint32(26).fork()).join();
		if (message.carrier !== "") writer.uint32(34).string(message.carrier);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var DecoderEncoderAvailablity = {
	encode(message, writer = new BinaryWriter()) {
		if (message.isSnapVp9DecoderAvailable !== false) writer.uint32(8).bool(message.isSnapVp9DecoderAvailable);
		if (message.isSnapAv1DecoderAvailable !== false) writer.uint32(16).bool(message.isSnapAv1DecoderAvailable);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/common/value.js
...
var MapRecords = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.records) MapRecord.encode(v, writer.uint32(10).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var StringArray = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.value) writer.uint32(10).string(v);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var Value = {
	encode(message, writer = new BinaryWriter()) {
		if (message.intValue !== void 0) writer.uint32(8).int32(message.intValue);
		if (message.longValue !== void 0) writer.uint32(16).int64(message.longValue);
		if (message.floatValue !== void 0) writer.uint32(29).float(message.floatValue);
		if (message.doubleValue !== void 0) writer.uint32(73).double(message.doubleValue);
		if (message.boolValue !== void 0) writer.uint32(32).bool(message.boolValue);
		if (message.stringValue !== void 0) writer.uint32(42).string(message.stringValue);
		if (message.anyValue !== void 0) Any.encode(message.anyValue, writer.uint32(50).fork()).join();
		if (message.mapValue !== void 0) MapRecords.encode(message.mapValue, writer.uint32(58).fork()).join();
		if (message.intPairValue !== void 0) writer.uint32(65).fixed64(message.intPairValue);
		if (message.stringArrayValue !== void 0) StringArray.encode(message.stringArrayValue, writer.uint32(82).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/config_result.js
...
var ConfigResult = {
	encode(message, writer = new BinaryWriter()) {
		if (message.configId !== "") writer.uint32(10).string(message.configId);
		if (message.value !== void 0) Value.encode(message.value, writer.uint32(18).fork()).join();
		if (message.targetingExpression !== void 0) ClientTargetingExpression.encode(message.targetingExpression, writer.uint32(26).fork()).join();
		if (message.ttlSeconds !== "0") writer.uint32(32).int64(message.ttlSeconds);
		if (message.configRuleUuidBytes.length !== 0) writer.uint32(42).bytes(message.configRuleUuidBytes);
		if (message.priority !== 0) writer.uint32(48).int32(message.priority);
		if (message.namespace !== 0) writer.uint32(56).int32(message.namespace);
		if (message.studyName !== "") writer.uint32(66).string(message.studyName);
		if (message.experimentId !== 0) writer.uint32(72).int32(message.experimentId);
		if (message.delete !== false) writer.uint32(80).bool(message.delete);
		writer.uint32(90).fork();
		for (const v of message.servePlatforms) writer.int32(v);
		writer.join();
		if (message.sequenceId !== 0) writer.uint32(96).int32(message.sequenceId);
		if (message.ruidType !== 0) writer.uint32(104).int32(message.ruidType);
		if (message.segmentOrdinal !== 0) writer.uint32(112).int32(message.segmentOrdinal);
		if (message.generatedFromAbAllowlists !== false) writer.uint32(120).bool(message.generatedFromAbAllowlists);
		if (message.internalFields !== void 0) ConfigResult_InternalFields.encode(message.internalFields, writer.uint32(130).fork()).join();
		if (message.configIntId !== 0) writer.uint32(149).sfixed32(message.configIntId);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				...
				case 11:
					...
					if (tag === 90) {
						const end2 = reader.uint32() + reader.pos;
						while (reader.pos < end2) message.servePlatforms.push(reader.int32());
						continue;
					}
					break;
				case 12:
...
var ConfigResult_InternalFields = {
	encode(message, writer = new BinaryWriter()) {
		if (message.configBitmapIndex !== 0) writer.uint32(8).int32(message.configBitmapIndex);
		if (message.configResultBitmapIndex !== 0) writer.uint32(16).int32(message.configResultBitmapIndex);
		if (message.hasServerPropertiesOnly !== false) writer.uint32(24).bool(message.hasServerPropertiesOnly);
		if (message.globalPriority !== 0) writer.uint32(32).int32(message.globalPriority);
		for (const v of message.sequenceIds) ConfigResult_InternalFields_SequenceIdCandidate.encode(v, writer.uint32(42).fork()).join();
		if (message.studySegmentOrdinal !== 0) writer.uint32(48).int32(message.studySegmentOrdinal);
		if (message.experimentGuid !== "0") writer.uint32(56).uint64(message.experimentGuid);
		if (message.isAbStudyStatusCompleted !== false) writer.uint32(64).bool(message.isAbStudyStatusCompleted);
		if (message.gradualRolloutId !== "") writer.uint32(74).string(message.gradualRolloutId);
		if (message.gradualRolloutIsControl !== false) writer.uint32(80).bool(message.gradualRolloutIsControl);
		if (message.gradualRolloutIsDeleted !== false) writer.uint32(88).bool(message.gradualRolloutIsDeleted);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
	...
	fromPartial(object) {
		...
		message.sequenceIds = ((_e = object.sequenceIds) === null || _e === void 0 ? void 0 : _e.map((e) => ConfigResult_InternalFields_SequenceIdCandidate.fromPartial(e))) || [];
...
var ConfigResult_InternalFields_SequenceIdCandidate = {
	encode(message, writer = new BinaryWriter()) {
		if (message.targetingExpression !== void 0) ClientTargetingExpression.encode(message.targetingExpression, writer.uint32(10).fork()).join();
		if (message.sequenceId !== 0) writer.uint32(16).int32(message.sequenceId);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var ClientTargetingExpression = {
	encode(message, writer = new BinaryWriter()) {
		if (message.operator !== 0) writer.uint32(8).int32(message.operator);
		for (const v of message.children) ClientTargetingExpression.encode(v, writer.uint32(18).fork()).join();
		if (message.property !== 0) writer.uint32(24).int32(message.property);
		if (message.predicateOperator !== 0) writer.uint32(32).int32(message.predicateOperator);
		if (message.value !== void 0) Value.encode(message.value, writer.uint32(42).fork()).join();
		if (message.propertyMetadata !== void 0) ClientTargetingExpression_PropertyMetadata.encode(message.propertyMetadata, writer.uint32(50).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var ClientTargetingExpression_PropertyMetadata = {
	encode(message, writer = new BinaryWriter()) {
		if (message.itemId !== 0) writer.uint32(8).int32(message.itemId);
		if (message.signalToHash !== 0) writer.uint32(16).int32(message.signalToHash);
		if (message.abNamespaceForHashing !== "") writer.uint32(26).string(message.abNamespaceForHashing);
		if (message.abSeedForHashing !== "") writer.uint32(34).string(message.abSeedForHashing);
		if (message.ruidType !== void 0) writer.uint32(40).int32(message.ruidType);
		if (message.cofRolloutSeedForHashing !== "") writer.uint32(50).string(message.cofRolloutSeedForHashing);
		if (message.adsNonBudgetSegmentLayer !== 0) writer.uint32(56).int32(message.adsNonBudgetSegmentLayer);
		if (message.adsBudgetAbMacrostateName !== "") writer.uint32(66).string(message.adsBudgetAbMacrostateName);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/config_response.js
function createBaseConfigTargetingResponseDebugData() {
	return { abResultChecksumInput: "" };
}
var ConfigTargetingResponseDebugData = {
	encode(message, writer = new BinaryWriter()) {
		if (message.abResultChecksumInput !== "") writer.uint32(10).string(message.abResultChecksumInput);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var ConfigTargetingResponse = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.configResults) ConfigResult.encode(v, writer.uint32(10).fork()).join();
		if (message.configResultsEtag !== "") writer.uint32(18).string(message.configResultsEtag);
		if (message.benchmarkRequest !== void 0) BenchmarkRequest.encode(message.benchmarkRequest, writer.uint32(26).fork()).join();
		if (message.fullResults !== false) writer.uint32(32).bool(message.fullResults);
		if (message.abResultChecksum !== 0) writer.uint32(40).int32(message.abResultChecksum);
		if (message.iso3166Alpha2CountryCodeFromRequestIp !== "") writer.uint32(50).string(message.iso3166Alpha2CountryCodeFromRequestIp);
		if (message.debugData !== void 0) ConfigTargetingResponseDebugData.encode(message.debugData, writer.uint32(58).fork()).join();
		if (message.cofGrapheneContext.length !== 0) writer.uint32(106).bytes(message.cofGrapheneContext);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/debug_info.js
var AbStep;
(function(AbStep) {
...
var AbStepInfo = {
	encode(message, writer = new BinaryWriter()) {
		if (message.abStep !== 0) writer.uint32(8).int32(message.abStep);
		if (message.info !== "") writer.uint32(18).string(message.info);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var CofStepInfo = {
	encode(message, writer = new BinaryWriter()) {
		if (message.cofStep !== 0) writer.uint32(8).int32(message.cofStep);
		if (message.info !== "") writer.uint32(18).string(message.info);
		return writer;
	},
	decode(input, length) {
...
var DebugTrace = {
	encode(message, writer = new BinaryWriter()) {
		if (message.lastCofStep !== void 0) CofStepInfo.encode(message.lastCofStep, writer.uint32(10).fork()).join();
		if (message.lastAbStep !== void 0) AbStepInfo.encode(message.lastAbStep, writer.uint32(18).fork()).join();
		if (message.budgetGroupId !== 0) writer.uint32(24).uint32(message.budgetGroupId);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/content/media_reference.js
...
(function(VideoDescription_MediaQualityType) {
	...
	VideoDescription_MediaQualityType[VideoDescription_MediaQualityType["MEDIA_QUALITY_TYPE_LEVEL_MAX"] = 5e3] = "MEDIA_QUALITY_TYPE_LEVEL_MAX";
...
var VideoDescription = {
	encode(message, writer = new BinaryWriter()) {
		if (message.mediaQualityType !== 0) writer.uint32(16).int32(message.mediaQualityType);
		if (message.videoPlaybackType !== 0) writer.uint32(8).int32(message.videoPlaybackType);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var MediaReference = {
	encode(message, writer = new BinaryWriter()) {
		if (message.mediaListId !== "0") writer.uint32(48).int64(message.mediaListId);
		if (message.claimBehavior !== 0) writer.uint32(56).int32(message.claimBehavior);
		if (message.url !== "") writer.uint32(18).string(message.url);
		if (message.contentObject.length !== 0) writer.uint32(26).bytes(message.contentObject);
		if (message.localContentKey !== "") writer.uint32(74).string(message.localContentKey);
		if (message.localCacheKey !== "") writer.uint32(82).string(message.localCacheKey);
		if (message.videoDescription !== void 0) VideoDescription.encode(message.videoDescription, writer.uint32(42).fork()).join();
		if (message.mediaType !== 0) writer.uint32(64).int32(message.mediaType);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/snapdoc/capture_characteristics.js
...
var CaptureCharacteristics = {
	encode(message, writer = new BinaryWriter()) {
		if (message.flashOn !== false) writer.uint32(8).bool(message.flashOn);
		if (message.frontFacing !== false) writer.uint32(16).bool(message.frontFacing);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/snapdoc/media_id.js
function createBaseMediaId() {
	return { mediaListId: "0" };
}
var MediaId = {
	encode(message, writer = new BinaryWriter()) {
		if (message.mediaListId !== "0") writer.uint32(8).int64(message.mediaListId);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/snapdoc/media.js
...
var AudioMetadata = {
	encode(message, writer = new BinaryWriter()) {
		if (message.codecFormat !== 0) writer.uint32(8).int32(message.codecFormat);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var VideoMetadata = {
	encode(message, writer = new BinaryWriter()) {
		if (message.codecFormat !== 0) writer.uint32(8).int32(message.codecFormat);
		if (message.containerFormat !== 0) writer.uint32(16).int32(message.containerFormat);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var MediaMetadata = {
	encode(message, writer = new BinaryWriter()) {
		if (message.orientation !== 0) writer.uint32(24).int32(message.orientation);
		if (message.dimensions !== void 0) MediaMetadata_MediaDimensions.encode(message.dimensions, writer.uint32(42).fork()).join();
		if (message.mediaDurationMs !== 0) writer.uint32(120).uint32(message.mediaDurationMs);
		if (message.mediaId !== void 0) MediaId.encode(message.mediaId, writer.uint32(146).fork()).join();
		if (message.encryptionInfoV1 !== void 0) MediaMetadata_MediaEncryptionInfo.encode(message.encryptionInfoV1, writer.uint32(34).fork()).join();
		if (message.encryptionInfoV2 !== void 0) MediaMetadata_MediaEncryptionInfo.encode(message.encryptionInfoV2, writer.uint32(154).fork()).join();
		if (message.masterKeyEncryptedEncryptionInfo !== void 0) MediaMetadata_MediaEncryptionInfo.encode(message.masterKeyEncryptedEncryptionInfo, writer.uint32(250).fork()).join();
		if (message.assetType !== 0) writer.uint32(160).int32(message.assetType);
		if (message.captureCharacteristics !== void 0) CaptureCharacteristics.encode(message.captureCharacteristics, writer.uint32(170).fork()).join();
		if (message.mediaOriginCamera !== void 0) MediaOriginCamera.encode(message.mediaOriginCamera, writer.uint32(210).fork()).join();
		if (message.mediaOriginExternalStorage !== void 0) MediaOriginExternalStorage.encode(message.mediaOriginExternalStorage, writer.uint32(218).fork()).join();
		if (message.mediaOriginPixy !== void 0) MediaOriginPixy.encode(message.mediaOriginPixy, writer.uint32(226).fork()).join();
		if (message.mediaOriginSpectacles !== void 0) MediaOriginSpectacles.encode(message.mediaOriginSpectacles, writer.uint32(234).fork()).join();
		if (message.mediaOriginAi !== void 0) MediaOriginAI.encode(message.mediaOriginAi, writer.uint32(242).fork()).join();
		if (message.dreamsMetadata !== void 0) DreamsMetadata.encode(message.dreamsMetadata, writer.uint32(258).fork()).join();
		for (const v of message.additionalOrigins) MediaOrigin.encode(v, writer.uint32(266).fork()).join();
		if (message.displayOrientation !== 0) writer.uint32(176).int32(message.displayOrientation);
		if (message.audio !== void 0) AudioMetadata.encode(message.audio, writer.uint32(186).fork()).join();
		if (message.image !== void 0) ImageMetadata.encode(message.image, writer.uint32(194).fork()).join();
		if (message.video !== void 0) VideoMetadata.encode(message.video, writer.uint32(202).fork()).join();
		if (message.type !== 0) writer.uint32(16).int32(message.type);
		if (message.legacyMediaSource !== void 0) MediaMetadata_LegacyMediaSource.encode(message.legacyMediaSource, writer.uint32(50).fork()).join();
		if (message.contentDescriptor !== void 0) writer.uint32(58).bytes(message.contentDescriptor);
		if (message.mediaUrl !== "") writer.uint32(74).string(message.mediaUrl);
		if (message.hasSound !== false) writer.uint32(96).bool(message.hasSound);
		if (message.zipped !== false) writer.uint32(104).bool(message.zipped);
		if (message.frontFacing !== false) writer.uint32(112).bool(message.frontFacing);
		if (message.mediaReference !== void 0) MediaReference.encode(message.mediaReference, writer.uint32(138).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var MediaMetadata_MediaDimensions = {
	encode(message, writer = new BinaryWriter()) {
		if (message.width !== 0) writer.uint32(8).uint32(message.width);
		if (message.height !== 0) writer.uint32(16).uint32(message.height);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
function createBaseMediaMetadata_LegacyMediaSource() {
	return {
		sourceMediaId: "",
		directDownloadUrl: void 0
	};
}
var MediaMetadata_LegacyMediaSource = {
	encode(message, writer = new BinaryWriter()) {
		if (message.sourceMediaId !== "") writer.uint32(10).string(message.sourceMediaId);
		if (message.directDownloadUrl !== void 0) MediaMetadata_LegacyMediaSource_LegacyDirectDownloadUrl.encode(message.directDownloadUrl, writer.uint32(18).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var MediaMetadata_LegacyMediaSource_LegacyDirectDownloadUrl = {
	encode(message, writer = new BinaryWriter()) {
		...
		if (message.expirySeconds !== "0") writer.uint32(16).uint64(message.expirySeconds);
		if (message.type !== 0) writer.uint32(24).int32(message.type);
		if (message.region !== "") writer.uint32(34).string(message.region);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var MediaOrigin = {
	encode(message, writer = new BinaryWriter()) {
		if (message.mediaOriginCamera !== void 0) MediaOriginCamera.encode(message.mediaOriginCamera, writer.uint32(10).fork()).join();
		if (message.mediaOriginExternalStorage !== void 0) MediaOriginExternalStorage.encode(message.mediaOriginExternalStorage, writer.uint32(18).fork()).join();
		if (message.mediaOriginPixy !== void 0) MediaOriginPixy.encode(message.mediaOriginPixy, writer.uint32(26).fork()).join();
		if (message.mediaOriginSpectacles !== void 0) MediaOriginSpectacles.encode(message.mediaOriginSpectacles, writer.uint32(34).fork()).join();
		if (message.mediaOriginAi !== void 0) MediaOriginAI.encode(message.mediaOriginAi, writer.uint32(42).fork()).join();
		if (message.dreamsMetadata !== void 0) DreamsMetadata.encode(message.dreamsMetadata, writer.uint32(50).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var MediaOriginCamera = {
	...
	fromPartial(_) {
		return createBaseMediaOriginCamera();
	}
};
...
var MediaOriginPixy = {
	...
	fromPartial(_) {
		return createBaseMediaOriginPixy();
	}
};
...
var MediaOriginSpectacles = {
	...
	fromPartial(_) {
		return createBaseMediaOriginSpectacles();
	}
};
function createBaseMediaOriginExternalStorage() {
	return {
		creationTimestamp: "0",
		mediaSource: 0,
		insecureFileMetadata: void 0,
		originalLocalIds: []
	};
}
var MediaOriginExternalStorage = {
	encode(message, writer = new BinaryWriter()) {
		if (message.creationTimestamp !== "0") writer.uint32(8).uint64(message.creationTimestamp);
		if (message.mediaSource !== 0) writer.uint32(16).int32(message.mediaSource);
		if (message.insecureFileMetadata !== void 0) InsecureFileMetadata.encode(message.insecureFileMetadata, writer.uint32(42).fork()).join();
		for (const v of message.originalLocalIds) writer.uint32(50).string(v);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var InsecureFileMetadata = {
	encode(message, writer = new BinaryWriter()) {
		writer.uint32(10).fork();
		for (const v of message.lensId) writer.uint64(v);
		writer.join();
		if (message.musicTrackId !== "0") writer.uint32(16).uint64(message.musicTrackId);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				case 1:
					...
					if (tag === 10) {
						const end2 = reader.uint32() + reader.pos;
						while (reader.pos < end2) message.lensId.push(reader.uint64().toString());
						continue;
					}
					break;
				case 2:
...
var MediaOriginAI = {
	...
	fromPartial(_) {
		return createBaseMediaOriginAI();
	}
};
...
var DreamsMetadata = {
	encode(message, writer = new BinaryWriter()) {
		if (message.packId !== "") writer.uint32(10).string(message.packId);
		if (message.templateId !== "") writer.uint32(18).string(message.templateId);
		writer.uint32(26).fork();
		for (const v of message.contentType) writer.int32(v);
		writer.join();
		return writer;
	},
	decode(input, length) {
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				...
				case 3:
					...
					if (tag === 26) {
						const end2 = reader.uint32() + reader.pos;
						while (reader.pos < end2) message.contentType.push(reader.int32());
						continue;
					}
					break;
			}
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/billboard_provided_signals.js
...
var BillboardSignals = {
	encode(message, writer = new BinaryWriter()) {
		if (message.emailVerified !== false) writer.uint32(8).bool(message.emailVerified);
		if (message.phoneVerified !== false) writer.uint32(16).bool(message.phoneVerified);
		if (message.notificationPermGranted !== false) writer.uint32(24).bool(message.notificationPermGranted);
		if (message.contactPermGranted !== false) writer.uint32(32).bool(message.contactPermGranted);
		if (message.mutualFriendsCount !== 0) writer.uint32(40).int32(message.mutualFriendsCount);
		if (message.groupsCount !== 0) writer.uint32(48).int32(message.groupsCount);
		if (message.hasNonTeamSnapchatConversations !== false) writer.uint32(56).bool(message.hasNonTeamSnapchatConversations);
		if (message.emailSet !== false) writer.uint32(64).bool(message.emailSet);
		if (message.bitmojiSet !== false) writer.uint32(72).bool(message.bitmojiSet);
		if (message.hasAddFriendsRequest !== false) writer.uint32(80).bool(message.hasAddFriendsRequest);
		if (message.appLaunchFromPush !== false) writer.uint32(88).bool(message.appLaunchFromPush);
		if (message.appLaunchType !== 0) writer.uint32(96).int32(message.appLaunchType);
		if (message.campaignLocalImpressionCount !== 0) writer.uint32(104).int32(message.campaignLocalImpressionCount);
		if (message.campaignLocalLastImpressionTimeSecsToNow !== "0") writer.uint32(112).int64(message.campaignLocalLastImpressionTimeSecsToNow);
		if (message.campaignLocalContinuousDismissCount !== 0) writer.uint32(120).int32(message.campaignLocalContinuousDismissCount);
		if (message.fstNumInAppWarnings !== 0) writer.uint32(128).int32(message.fstNumInAppWarnings);
		if (message.fstShouldChangePassword !== false) writer.uint32(136).bool(message.fstShouldChangePassword);
		if (message.audioRecordPermGranted !== false) writer.uint32(144).bool(message.audioRecordPermGranted);
		if (message.mutualFriendsWithBirthdayCount !== 0) writer.uint32(152).int32(message.mutualFriendsWithBirthdayCount);
		if (message.campaignLocalDismissCount !== 0) writer.uint32(160).int32(message.campaignLocalDismissCount);
		if (message.memoriesCount !== 0) writer.uint32(168).int32(message.memoriesCount);
		if (message.campaignLocalFirstImpressionTimeSecsToNow !== "0") writer.uint32(176).int64(message.campaignLocalFirstImpressionTimeSecsToNow);
		if (message.campaignLocalClickCount !== 0) writer.uint32(184).int32(message.campaignLocalClickCount);
		if (message.campaignLocalInteractionCount !== 0) writer.uint32(192).int32(message.campaignLocalInteractionCount);
		if (message.campaignLocalLastInteractionTimeSecsToNow !== "0") writer.uint32(200).int64(message.campaignLocalLastInteractionTimeSecsToNow);
		if (message.lockScreenWidgetAppOpenFromPush !== false) writer.uint32(208).bool(message.lockScreenWidgetAppOpenFromPush);
		if (message.communityCount !== 0) writer.uint32(216).int32(message.communityCount);
		if (message.campaignLocalFirstClickTimeSecsToNow !== "0") writer.uint32(224).int64(message.campaignLocalFirstClickTimeSecsToNow);
		if (message.campaignLocalLastClickTimeSecsToNow !== "0") writer.uint32(232).int64(message.campaignLocalLastClickTimeSecsToNow);
		if (message.campaignLocalFirstDismissTimeSecsToNow !== "0") writer.uint32(240).int64(message.campaignLocalFirstDismissTimeSecsToNow);
		if (message.campaignLocalLastDismissTimeSecsToNow !== "0") writer.uint32(248).int64(message.campaignLocalLastDismissTimeSecsToNow);
		if (message.campaignLocalFirstInteractionTimeSecsToNow !== "0") writer.uint32(256).int64(message.campaignLocalFirstInteractionTimeSecsToNow);
		if (message.serverImpressionCount !== 0) writer.uint32(264).int32(message.serverImpressionCount);
		if (message.serverClickCount !== 0) writer.uint32(272).int32(message.serverClickCount);
		if (message.serverDismissCount !== 0) writer.uint32(280).int32(message.serverDismissCount);
		if (message.serverInteractionCount !== 0) writer.uint32(288).int32(message.serverInteractionCount);
		if (message.serverFirstImpressionTimeSecsToNow !== "0") writer.uint32(296).int64(message.serverFirstImpressionTimeSecsToNow);
		if (message.serverLastImpressionTimeSecsToNow !== "0") writer.uint32(304).int64(message.serverLastImpressionTimeSecsToNow);
		if (message.serverFirstClickTimeSecsToNow !== "0") writer.uint32(312).int64(message.serverFirstClickTimeSecsToNow);
		if (message.serverLastClickTimeSecsToNow !== "0") writer.uint32(320).int64(message.serverLastClickTimeSecsToNow);
		if (message.serverFirstDismissTimeSecsToNow !== "0") writer.uint32(328).int64(message.serverFirstDismissTimeSecsToNow);
		if (message.serverLastDismissTimeSecsToNow !== "0") writer.uint32(336).int64(message.serverLastDismissTimeSecsToNow);
		if (message.serverFirstInteractionTimeSecsToNow !== "0") writer.uint32(344).int64(message.serverFirstInteractionTimeSecsToNow);
		if (message.serverLastInteractionTimeSecsToNow !== "0") writer.uint32(352).int64(message.serverLastInteractionTimeSecsToNow);
		if (message.serverContinuousDismissCount !== 0) writer.uint32(360).int32(message.serverContinuousDismissCount);
		if (message.contactPermOsGranted !== false) writer.uint32(368).bool(message.contactPermOsGranted);
		if (message.contactPermUserGranted !== false) writer.uint32(376).bool(message.contactPermUserGranted);
		if (message.contactPermOsAuthorizationStatus !== 0) writer.uint32(384).int32(message.contactPermOsAuthorizationStatus);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/bolt_provided_signals.js
function createBaseBoltSignals() {
	return {
		boltUseCase: 0,
		boltCdnExperimentationId: 0,
		boltIsContentPopular: false,
		boltClientAppState: 0
	};
}
var BoltSignals = {
	encode(message, writer = new BinaryWriter()) {
		if (message.boltUseCase !== 0) writer.uint32(8).uint32(message.boltUseCase);
		if (message.boltCdnExperimentationId !== 0) writer.uint32(16).uint32(message.boltCdnExperimentationId);
		if (message.boltIsContentPopular !== false) writer.uint32(24).bool(message.boltIsContentPopular);
		if (message.boltClientAppState !== 0) writer.uint32(32).int32(message.boltClientAppState);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/camera_provided_signals.js
var CameraDirection;
...
var CameraSignals = {
	encode(message, writer = new BinaryWriter()) {
		if (message.cameraDirection !== 0) writer.uint32(8).int32(message.cameraDirection);
		if (message.cameraContext !== 0) writer.uint32(16).int32(message.cameraContext);
		if (message.cameraApi !== 0) writer.uint32(24).int32(message.cameraApi);
		if (message.flashState !== 0) writer.uint32(32).int32(message.flashState);
		if (message.camera2Level !== 0) writer.uint32(40).int32(message.camera2Level);
		if (message.nightModeState !== 0) writer.uint32(48).int32(message.nightModeState);
		if (message.isFirstCapture !== false) writer.uint32(56).bool(message.isFirstCapture);
		if (message.pictureMode !== 0) writer.uint32(64).int32(message.pictureMode);
		if (message.captureMode !== 0) writer.uint32(72).int32(message.captureMode);
		if (message.hasLens !== false) writer.uint32(80).bool(message.hasLens);
		if (message.isLowLight !== false) writer.uint32(88).bool(message.isLowLight);
		if (message.isConcurrentCameraSupported !== false) writer.uint32(96).bool(message.isConcurrentCameraSupported);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/cognac_provided_signals.js
function createBaseCognacSignals() {
	return { cognacId: [] };
}
var CognacSignals = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.cognacId) writer.uint32(10).string(v);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/content_manager_provided_signals.js
...
var ContentManagerSignals = {
	encode(message, writer = new BinaryWriter()) {
		if (message.legacyMushroomContentType !== 0) writer.uint32(8).int32(message.legacyMushroomContentType);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/creative_tools_provided_signals.js
function createBaseCreativeToolsSignals() {
	return {
		withAnimatedOverlay: false,
		withMusic: false
	};
}
var CreativeToolsSignals = {
	encode(message, writer = new BinaryWriter()) {
		if (message.withAnimatedOverlay !== false) writer.uint32(8).bool(message.withAnimatedOverlay);
		if (message.withMusic !== false) writer.uint32(16).bool(message.withMusic);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/discover_feed_provided_signals.js
function createBaseDiscoverFeedSignals() {
	return { discoverFeedSectionCacheInfo: void 0 };
}
var DiscoverFeedSignals = {
	encode(message, writer = new BinaryWriter()) {
		if (message.discoverFeedSectionCacheInfo !== void 0) DiscoverFeedSignals_DiscoverFeedSectionCacheInfo.encode(message.discoverFeedSectionCacheInfo, writer.uint32(10).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var DiscoverFeedSignals_DiscoverFeedSectionCacheInfo = {
	encode(message, writer = new BinaryWriter()) {
		if (message.feedType !== 0) writer.uint32(8).int32(message.feedType);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/lenses_provided_signals.js
function createBaseLensesSignals() {
	return {
		lastLensActivatedTime: "0",
		lastSnappableLensActivatedTime: "0",
		isAnyEffectApplied: false
	};
}
var LensesSignals = {
	encode(message, writer = new BinaryWriter()) {
		if (message.lastLensActivatedTime !== "0") writer.uint32(8).uint64(message.lastLensActivatedTime);
		if (message.lastSnappableLensActivatedTime !== "0") writer.uint32(16).uint64(message.lastSnappableLensActivatedTime);
		if (message.isAnyEffectApplied !== false) writer.uint32(24).bool(message.isAnyEffectApplied);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/mdp_media_attribution.js
...
var MdpMediaAttribution = {
	encode(message, writer = new BinaryWriter()) {
		if (message.mediaSource !== 0) writer.uint32(8).int32(message.mediaSource);
		if (message.assetType !== 0) writer.uint32(16).int32(message.assetType);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/media_provided_signals.js
var MediaType;
(function(MediaType) {
...
var MediaSignals = {
	encode(message, writer = new BinaryWriter()) {
		if (message.mediaDurationMs !== 0) writer.uint32(8).uint32(message.mediaDurationMs);
		if (message.mediaType !== 0) writer.uint32(16).int32(message.mediaType);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/opera_provided_signals.js
function createBaseOperaSignals() {
	return {
		contentViewSource: 0,
		playbackItemType: 0,
		useVerticalNavigation: false
	};
}
var OperaSignals = {
	encode(message, writer = new BinaryWriter()) {
		if (message.contentViewSource !== 0) writer.uint32(8).int32(message.contentViewSource);
		if (message.playbackItemType !== 0) writer.uint32(16).int32(message.playbackItemType);
		if (message.useVerticalNavigation !== false) writer.uint32(24).bool(message.useVerticalNavigation);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/perception_provided_signals.js
function createBasePerceptionSignals() {
	return { scannedCategoryIds: [] };
}
var PerceptionSignals = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.scannedCategoryIds) writer.uint32(10).string(v);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/recipients_provided_signals.js
function createBaseRecipientsSignals() {
	return { userIds: [] };
}
var RecipientsSignals = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.userIds) writer.uint32(10).string(v);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/routing_provided_signals.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/snap_kit_provided_signals.js
function createBaseSnapKitSignals() {
	return { oauthClientId: "" };
}
var SnapKitSignals = {
	encode(message, writer = new BinaryWriter()) {
		if (message.oauthClientId !== "") writer.uint32(10).string(message.oauthClientId);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/upload_provided_signals.js
...
function createBaseUploadSignals() {
	return {
		chunkUploadSupportRequired: false,
		cloudfrontPop: "",
		gcdnPop: "",
		chunkUploadPreference: 0
	};
}
var UploadSignals = {
	encode(message, writer = new BinaryWriter()) {
		if (message.chunkUploadSupportRequired !== false) writer.uint32(8).bool(message.chunkUploadSupportRequired);
		if (message.cloudfrontPop !== "") writer.uint32(18).string(message.cloudfrontPop);
		if (message.gcdnPop !== "") writer.uint32(26).string(message.gcdnPop);
		if (message.chunkUploadPreference !== 0) writer.uint32(32).int32(message.chunkUploadPreference);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/feature_provided_signals.js
...
var FeatureProvidedSignals = {
	encode(message, writer = new BinaryWriter()) {
		if (message.mediaMetadata !== void 0) MediaMetadata.encode(message.mediaMetadata, writer.uint32(10).fork()).join();
		if (message.storyMetadata !== void 0) FeatureProvidedSignals_StoryMetadata.encode(message.storyMetadata, writer.uint32(18).fork()).join();
		if (message.cameraSignals !== void 0) CameraSignals.encode(message.cameraSignals, writer.uint32(26).fork()).join();
		if (message.snapSource !== 0) writer.uint32(32).int32(message.snapSource);
		if (message.contentManagerProvidedSignals !== void 0) ContentManagerSignals.encode(message.contentManagerProvidedSignals, writer.uint32(42).fork()).join();
		if (message.spectacleMetadata !== void 0) FeatureProvidedSignals_SpectacleMetadata.encode(message.spectacleMetadata, writer.uint32(50).fork()).join();
		if (message.boltSignals !== void 0) BoltSignals.encode(message.boltSignals, writer.uint32(58).fork()).join();
		if (message.lensesSignals !== void 0) LensesSignals.encode(message.lensesSignals, writer.uint32(66).fork()).join();
		if (message.creativeToolsSignals !== void 0) CreativeToolsSignals.encode(message.creativeToolsSignals, writer.uint32(74).fork()).join();
		if (message.estimatedDurationForEventMs !== 0) writer.uint32(80).uint32(message.estimatedDurationForEventMs);
		if (message.routingSignals !== void 0) RoutingSignals.encode(message.routingSignals, writer.uint32(90).fork()).join();
		if (message.mdpMediaAttribution !== void 0) MdpMediaAttribution.encode(message.mdpMediaAttribution, writer.uint32(98).fork()).join();
		if (message.size !== 0) writer.uint32(104).uint32(message.size);
		if (message.mediaSignals !== void 0) MediaSignals.encode(message.mediaSignals, writer.uint32(114).fork()).join();
		if (message.perceptionSignals !== void 0) PerceptionSignals.encode(message.perceptionSignals, writer.uint32(122).fork()).join();
		if (message.operaSignals !== void 0) OperaSignals.encode(message.operaSignals, writer.uint32(130).fork()).join();
		if (message.recipientsSignals !== void 0) RecipientsSignals.encode(message.recipientsSignals, writer.uint32(138).fork()).join();
		if (message.billboardSignals !== void 0) BillboardSignals.encode(message.billboardSignals, writer.uint32(146).fork()).join();
		if (message.cognacSignals !== void 0) CognacSignals.encode(message.cognacSignals, writer.uint32(154).fork()).join();
		if (message.hashSignals !== void 0) FeatureProvidedSignals_HashSignals.encode(message.hashSignals, writer.uint32(162).fork()).join();
		Object.entries(message.edgeSignals).forEach(([key, value]) => {
			FeatureProvidedSignals_EdgeSignalsEntry.encode({
				key,
				value
			}, writer.uint32(170).fork()).join();
		});
		if (message.snapKitSignals !== void 0) SnapKitSignals.encode(message.snapKitSignals, writer.uint32(178).fork()).join();
		if (message.uploadSignals !== void 0) UploadSignals.encode(message.uploadSignals, writer.uint32(186).fork()).join();
		for (const v of message.ruids) Ruid.encode(v, writer.uint32(194).fork()).join();
		if (message.discoverFeedSignals !== void 0) DiscoverFeedSignals.encode(message.discoverFeedSignals, writer.uint32(202).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				...
				case 21: {
					...
					if (entry21.value !== void 0) message.edgeSignals[entry21.key] = entry21.value;
					continue;
				}
				case 22:
					if (tag !== 178) break;
	...
	fromPartial(object) {
		...
		message.edgeSignals = Object.entries((_d = object.edgeSignals) !== null && _d !== void 0 ? _d : {}).reduce((acc, [key, value]) => {
			if (value !== void 0) acc[globalThis.Number(key)] = Value.fromPartial(value);
			return acc;
		}, {});
...
function createBaseFeatureProvidedSignals_StoryMetadata() {
	return {
		isPublicStory: false,
		isOfficialStory: false,
		storyType: 0
	};
}
var FeatureProvidedSignals_StoryMetadata = {
	encode(message, writer = new BinaryWriter()) {
		if (message.isPublicStory !== false) writer.uint32(8).bool(message.isPublicStory);
		if (message.isOfficialStory !== false) writer.uint32(16).bool(message.isOfficialStory);
		if (message.storyType !== 0) writer.uint32(24).int32(message.storyType);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var FeatureProvidedSignals_SpectacleMetadata = {
	encode(message, writer = new BinaryWriter()) {
		if (message.isSpectacle !== false) writer.uint32(8).bool(message.isSpectacle);
		if (message.spectaclesVersion !== 0) writer.uint32(16).int32(message.spectaclesVersion);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var FeatureProvidedSignals_HashSignals = {
	encode(message, writer = new BinaryWriter()) {
		if (message.userId.length !== 0) writer.uint32(10).bytes(message.userId);
		if (message.seed.length !== 0) writer.uint32(18).bytes(message.seed);
		if (message.namespace !== 0) writer.uint32(24).int32(message.namespace);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var FeatureProvidedSignals_EdgeSignalsEntry = {
	encode(message, writer = new BinaryWriter()) {
		if (message.key !== 0) writer.uint32(8).int32(message.key);
		if (message.value !== void 0) Value.encode(message.value, writer.uint32(18).fork()).join();
		return writer;
	},
	decode(input, length) {
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/cdp/cof/circumstance_service.js
...
var GetAbConfigValuesRequest = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.configNames) writer.uint32(10).string(v);
		writer.uint32(18).fork();
		for (const v of message.namespaces) writer.int32(v);
		writer.join();
		for (const v of message.cofConfigNames) writer.uint32(26).string(v);
		if (message.includeAllConfigs !== false) writer.uint32(32).bool(message.includeAllConfigs);
		if (message.disableExposureLogging !== false) writer.uint32(40).bool(message.disableExposureLogging);
		if (message.enableDebug !== false) writer.uint32(104).bool(message.enableDebug);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var GetAbConfigValuesResponse = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.configResults) ConfigResult.encode(v, writer.uint32(10).fork()).join();
		if (message.debugTrace !== void 0) DebugTrace.encode(message.debugTrace, writer.uint32(18).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
function createBaseConfigSetGenerationRequest() {
	return {
		targetingParameters: void 0,
		overrides: []
	};
}
var ConfigSetGenerationRequest = {
	encode(message, writer = new BinaryWriter()) {
		if (message.targetingParameters !== void 0) ConfigTargetingRequest.encode(message.targetingParameters, writer.uint32(10).fork()).join();
		for (const v of message.overrides) ConfigResult.encode(v, writer.uint32(18).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var ConfigSetGenerationResponse = {
	encode(message, writer = new BinaryWriter()) {
		if (message.targetingResponse !== void 0) ConfigTargetingResponse.encode(message.targetingResponse, writer.uint32(10).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var PinConfigRequest = {
	encode(message, writer = new BinaryWriter()) {
		if (message.configSetToken !== "") writer.uint32(10).string(message.configSetToken);
		if (message.userId !== "") writer.uint32(18).string(message.userId);
		if (message.ttlSeconds !== 0) writer.uint32(24).int32(message.ttlSeconds);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var PinConfigResponse = {
	encode(message, writer = new BinaryWriter()) {
		if (message.debug !== "") writer.uint32(10).string(message.debug);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var PinTargetingPropertiesRequest = {
	encode(message, writer = new BinaryWriter()) {
		...
		if (message.targetingProperties !== void 0) FeatureProvidedSignals.encode(message.targetingProperties, writer.uint32(18).fork()).join();
		if (message.ttlSeconds !== 0) writer.uint32(24).int32(message.ttlSeconds);
		return writer;
	},
	decode(input, length) {
...
var GetPinnedTargetingPropertiesResponse = {
	encode(message, writer = new BinaryWriter()) {
		if (message.targetingProperties !== void 0) FeatureProvidedSignals.encode(message.targetingProperties, writer.uint32(10).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var ConfigWithNamespace = {
	encode(message, writer = new BinaryWriter()) {
		if (message.configName !== "") writer.uint32(10).string(message.configName);
		if (message.namespace !== 0) writer.uint32(16).int32(message.namespace);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var GetConfigValuesByTokenRequest = {
	encode(message, writer = new BinaryWriter()) {
		...
		for (const v of message.configNames) ConfigWithNamespace.encode(v, writer.uint32(18).fork()).join();
		writer.uint32(26).fork();
		for (const v of message.namespaces) writer.int32(v);
		writer.join();
		if (message.disableExposureLogging !== false) writer.uint32(32).bool(message.disableExposureLogging);
		if (message.getAllConfigs !== false) writer.uint32(40).bool(message.getAllConfigs);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var GetCachedConfigResultSequenceIdsRequest = {
	...
	fromPartial(_) {
		return createBaseGetCachedConfigResultSequenceIdsRequest();
	}
};
...
var GetCachedConfigResultSequenceIdsResponse = {
	encode(message, writer = new BinaryWriter()) {
		if (message.serializedBitmap.length !== 0) writer.uint32(10).bytes(message.serializedBitmap);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var GetConfigResultsBySequenceIdsRequest = {
	encode(message, writer = new BinaryWriter()) {
		writer.uint32(10).fork();
		for (const v of message.sequenceIds) writer.int32(v);
		writer.join();
		return writer;
	},
	decode(input, length) {
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				case 1:
					...
					if (tag === 10) {
						const end2 = reader.uint32() + reader.pos;
						while (reader.pos < end2) message.sequenceIds.push(reader.int32());
						continue;
					}
					break;
			}
...
var ConfigResultWithSequenceId = {
	encode(message, writer = new BinaryWriter()) {
		if (message.sequenceId !== 0) writer.uint32(8).int32(message.sequenceId);
		if (message.configResult !== void 0) ConfigResult.encode(message.configResult, writer.uint32(18).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var GetConfigResultsBySequenceIdsResponse = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.configResultWithSequenceIds) ConfigResultWithSequenceId.encode(v, writer.uint32(10).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
	...
	fromPartial(object) {
		var _a;
		const message = createBaseGetConfigResultsBySequenceIdsResponse();
		message.configResultWithSequenceIds = ((_a = object.configResultWithSequenceIds) === null || _a === void 0 ? void 0 : _a.map((e) => ConfigResultWithSequenceId.fromPartial(e))) || [];
		return message;
	}
};
...
var GetBitmapByTokenResponse = {
	encode(message, writer = new BinaryWriter()) {
		if (message.idBitmap.length !== 0) writer.uint32(10).bytes(message.idBitmap);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var GetUnevaluatedConfigResultsWithAbTargetingRequest = {
	encode(message, writer = new BinaryWriter()) {
		...
		for (const v of message.namespaces) writer.int32(v);
		writer.join();
		if (message.edgeConfigClientVersion !== "") writer.uint32(18).string(message.edgeConfigClientVersion);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var AddRuidsForDebuggingRequest = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.ruids) Ruid.encode(v, writer.uint32(10).fork()).join();
		for (const v of message.configNames) writer.uint32(18).string(v);
		if (message.maxSnapshots !== 0) writer.uint32(24).int32(message.maxSnapshots);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var AddRuidsForDebuggingResponse = {
	...
	fromPartial(_) {
		return createBaseAddRuidsForDebuggingResponse();
	}
};
...
var CircumstancesServicetargetingQueryDesc = {
	...
	requestStream: false,
	responseStream: false,
	requestType: { serializeBinary() {
		return ConfigTargetingRequest.encode(this).finish();
	} },
	responseType: { deserializeBinary(data) {
...
var GrpcWebImpl = class {
	...
	unary(methodDesc, _request, metadata) {
		...
		const maybeCombinedMetadata = metadata && this.options.metadata ? new import_browser_headers_umd.BrowserHeaders(Object.assign(Object.assign({}, (_a = this.options) === null || _a === void 0 ? void 0 : _a.metadata.headersMap), metadata === null || metadata === void 0 ? void 0 : metadata.headersMap)) : metadata !== null && metadata !== void 0 ? metadata : this.options.metadata;
		return new Promise((resolve, reject) => {
			var _a;
			import_grpc_web_client_umd.grpc.unary(methodDesc, Object.assign(Object.assign({
				request,
				host: this.host,
				...
				debug: (_a = this.options.debug) !== null && _a !== void 0 ? _a : false,
				onEnd: function(response) {
					if (response.status === import_grpc_web_client_umd.grpc.Code.OK) resolve(response.message.toObject());
					else reject(new GrpcWebError(response.statusMessage, response.status, response.trailers));
				}
			}));
		});
	}
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/handlers/responseCachingHandler.js
var logger$26 = getLogger("responseCachingHandler");
...
var staleWhileRevalidateStrategy = (options) => (key, cache, network) => __awaiter(void 0, void 0, void 0, function* () {
	...
	try {
		...
		network({ isSideEffect: true }).then((response) => cache.store(key, response)).catch((error) => {
			logger$26.warn(`staleWhileRevalidateStrategy failed to retrieve and store key ${key}.`, error);
		});
		(_c = options === null || options === void 0 ? void 0 : options.onHit) === null || _c === void 0 || _c.call(options);
		return cachedResponse;
	} catch (cacheError) {
		(_d = options === null || options === void 0 ? void 0 : options.onMiss) === null || _d === void 0 || _d.call(options);
		try {
			const response = yield network();
			cache.store(key, response).catch((error) => {
				logger$26.warn(`staleWhileRevalidateStrategy failed to store key ${key}.`, error);
			});
			logger$26.debug(`staleWhileRevalidateStrategy successfully fell back to network for key ${key} after cache error.`, cacheError);
			return response;
		} catch (networkError) {
...
var createResponseCachingHandler = (cache, resolveKey, strategy) => {
	return (next) => (request, metadata) => __awaiter(void 0, void 0, void 0, function* () {
		const network = (additionalMetadata = {}) => {
			return next(request, Object.assign(Object.assign({}, metadata), additionalMetadata));
		};
		let key;
		try {
			key = resolveKey(request, metadata);
		} catch (error) {
			logger$26.warn("Cache lookup failed because the cache key could not be resolved.", error);
			return network();
		}
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/persistence/Persistence.js
var isValidKey = (key) => typeof key === "string" || typeof key === "number";
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/persistence/IndexedDBPersistence.js
...
var IndexedDBPersistence = class {
	...
	store(keyOrValue, maybeValue) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			if (!isValidKey(key) && typeof key !== "undefined") throw new TypeError(`IndexedDBPersistence failed to store a value. Invalid key type: ${typeof key}`);
	...
	transaction(mode) {
		return __awaiter(this, void 0, void 0, function* () {
			const tx = (yield this.db).transaction(this.objectStore, mode);
			return {
				tx,
				store: tx.objectStore(this.objectStore),
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/persistence/ExpiringPersistence.js
var logger$25 = getLogger("ExpiringPersistence");
var ExpiringPersistence = class {
	constructor(expiration, persistence) {
		this.expiration = expiration;
		this.persistence = persistence;
		this.removeExpired().catch((error) => {
			logger$25.warn("Failed to cleanup expired entries on startup.", error);
		});
	}
	get size() {
	...
	retrieve(key) {
		...
		return __awaiter(this, void 0, void 0, function* () {
			...
			if (value === void 0 || expiry === void 0) return void 0;
			if (Date.now() > expiry) {
				yield this.persistence.remove(key).catch((error) => {
					logger$25.warn(`Key ${key} is expired, but removing it from persistence failed.`, error);
				});
				return;
			}
			return value;
	...
	removeAll() {
		return __awaiter(this, void 0, void 0, function* () {
			return (yield this.persistence.removeAll()).map(([, v]) => v);
		});
	}
	removeExpired() {
		return __awaiter(this, void 0, void 0, function* () {
			for (const [key, [expiry]] of yield this.persistence.retrieveAll()) if (Date.now() >= expiry) yield this.persistence.remove(key).catch((error) => logger$25.warn(`Failed to remove expired key ${key}.`, error));
		});
	}
	store(keyOrValue, maybeValue) {
		const [key, value] = maybeValue === void 0 ? [void 0, keyOrValue] : [keyOrValue, maybeValue];
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/common/pageVisibility.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/blizzard/cameraKitEvents.js
var ServerEventBatch = {
	encode(message, writer = new BinaryWriter()) {
		if (message.serverUploadTs !== 0) writer.uint32(9).double(message.serverUploadTs);
		for (const v of message.serverAllUpdateEvents) ServerRequestAllUpdates.encode(v, writer.uint32(18).fork()).join();
		if (message.maxSequenceIdOnInstance !== "0") writer.uint32(24).uint64(message.maxSequenceIdOnInstance);
		for (const v of message.serverEvents) ServerEvent.encode(v, writer.uint32(34).fork()).join();
		return writer;
	},
	decode() {
...
var ServerRequestAllUpdates = {
	...
	fromPartial(_) {
		return createBaseServerRequestAllUpdates();
	}
};
...
var ServerEvent = {
	encode(message, writer = new BinaryWriter()) {
		if (message.eventName !== "") writer.uint32(10).string(message.eventName);
		if (message.serverTs !== 0) writer.uint32(25).double(message.serverTs);
		if (message.userId !== "") writer.uint32(34).string(message.userId);
		if (message.userAgent !== "") writer.uint32(42).string(message.userAgent);
		if (message.country !== "") writer.uint32(50).string(message.country);
		if (message.city !== "") writer.uint32(58).string(message.city);
		if (message.region !== "") writer.uint32(66).string(message.region);
		if (message.eventId !== "") writer.uint32(74).string(message.eventId);
		if (message.instanceId !== "") writer.uint32(82).string(message.instanceId);
		if (message.sequenceId !== "0") writer.uint32(88).uint64(message.sequenceId);
		if (message.osType !== "") writer.uint32(98).string(message.osType);
		if (message.osVersion !== "") writer.uint32(106).string(message.osVersion);
		if (message.appVersion !== "") writer.uint32(114).string(message.appVersion);
		if (message.appBuild !== "") writer.uint32(122).string(message.appBuild);
		if (message.serverUploadTs !== 0) writer.uint32(809).double(message.serverUploadTs);
		if (message.eventTime !== 0) writer.uint32(817).double(message.eventTime);
		if (message.serverReceiptTime !== 0) writer.uint32(825).double(message.serverReceiptTime);
		if (message.maxSequenceIdOnInstance !== "0") writer.uint32(832).uint64(message.maxSequenceIdOnInstance);
		if (message.userGuid !== "") writer.uint32(850).string(message.userGuid);
		if (message.collection !== 0) writer.uint32(872).int32(message.collection);
		if (message.serviceId !== "") writer.uint32(890).string(message.serviceId);
		if (message.appType !== 0) writer.uint32(912).int32(message.appType);
		if (message.spectrumInstanceId !== "") writer.uint32(930).string(message.spectrumInstanceId);
		if (message.spectrumSequenceId !== "0") writer.uint32(944).uint64(message.spectrumSequenceId);
		if (message.eventData !== void 0) ServerEventData.encode(message.eventData, writer.uint32(802).fork()).join();
		return writer;
	},
	decode() {
		throw new Error("Not implemented.");
...
var ServerEventData = {
	encode(message, writer = new BinaryWriter()) {
		if (message.cameraKitException !== void 0) CameraKitException.encode(message.cameraKitException, writer.uint32(1866).fork()).join();
		if (message.cameraKitLensSpin !== void 0) CameraKitLensSpin.encode(message.cameraKitLensSpin, writer.uint32(1874).fork()).join();
		if (message.cameraKitAssetDownload !== void 0) CameraKitAssetDownload.encode(message.cameraKitAssetDownload, writer.uint32(1882).fork()).join();
		if (message.cameraKitLensContentValidationFailed !== void 0) CameraKitLensContentValidationFailed.encode(message.cameraKitLensContentValidationFailed, writer.uint32(1922).fork()).join();
		if (message.cameraKitLensDownload !== void 0) CameraKitLensDownload.encode(message.cameraKitLensDownload, writer.uint32(1930).fork()).join();
		if (message.cameraKitAssetValidationFailed !== void 0) CameraKitAssetValidationFailed.encode(message.cameraKitAssetValidationFailed, writer.uint32(1938).fork()).join();
		if (message.cameraKitSession !== void 0) CameraKitSession$1.encode(message.cameraKitSession, writer.uint32(2130).fork()).join();
		if (message.cameraKitWebLensSwipe !== void 0) CameraKitWebLensSwipe.encode(message.cameraKitWebLensSwipe, writer.uint32(3818).fork()).join();
		if (message.cameraKitWebBenchmarkComplete !== void 0) CameraKitWebBenchmarkComplete.encode(message.cameraKitWebBenchmarkComplete, writer.uint32(3826).fork()).join();
		if (message.cameraKitLegalPrompt !== void 0) CameraKitLegalPrompt.encode(message.cameraKitLegalPrompt, writer.uint32(3874).fork()).join();
		return writer;
	},
	decode() {
		throw new Error("Not implemented.");
	...
	fromPartial(object) {
		...
		message.cameraKitSession = object.cameraKitSession !== void 0 && object.cameraKitSession !== null ? CameraKitSession$1.fromPartial(object.cameraKitSession) : void 0;
...
var CameraKitLensSpin = {
	encode(message, writer = new BinaryWriter()) {
		if (message.cameraKitEventBase !== void 0) CameraKitEventBase.encode(message.cameraKitEventBase, writer.uint32(10).fork()).join();
		if (message.lensId !== "") writer.uint32(18).string(message.lensId);
		if (message.viewTimeSec !== 0) writer.uint32(25).double(message.viewTimeSec);
		if (message.lensGroupId !== "") writer.uint32(34).string(message.lensGroupId);
		return writer;
	},
	decode() {
		throw new Error("Not implemented.");
...
var CameraKitWebLensSwipe = {
	encode(message, writer = new BinaryWriter()) {
		if (message.cameraKitEventBase !== void 0) CameraKitEventBase.encode(message.cameraKitEventBase, writer.uint32(10).fork()).join();
		if (message.performanceCluster !== "0") writer.uint32(16).uint64(message.performanceCluster);
		if (message.webglRendererInfo !== "") writer.uint32(26).string(message.webglRendererInfo);
		if (message.lensId !== "") writer.uint32(34).string(message.lensId);
		if (message.lensFrameProcessingTimeMsAvg !== 0) writer.uint32(41).double(message.lensFrameProcessingTimeMsAvg);
		if (message.lensFrameProcessingTimeMsStd !== 0) writer.uint32(49).double(message.lensFrameProcessingTimeMsStd);
		if (message.viewTimeSec !== 0) writer.uint32(57).double(message.viewTimeSec);
		if (message.recordingTimeSec !== 0) writer.uint32(65).double(message.recordingTimeSec);
		if (message.applyDelaySec !== 0) writer.uint32(73).double(message.applyDelaySec);
		if (message.avgFps !== 0) writer.uint32(81).double(message.avgFps);
		if (message.isLensFirstWithinDay !== false) writer.uint32(88).bool(message.isLensFirstWithinDay);
		if (message.isLensFirstWithinMonth !== false) writer.uint32(96).bool(message.isLensFirstWithinMonth);
		if (message.lensGroupId !== "") writer.uint32(106).string(message.lensGroupId);
		return writer;
	},
	decode() {
		throw new Error("Not implemented.");
...
var CameraKitLensDownload = {
	encode(message, writer = new BinaryWriter()) {
		if (message.cameraKitEventBase !== void 0) CameraKitEventBase.encode(message.cameraKitEventBase, writer.uint32(10).fork()).join();
		if (message.lensId !== "") writer.uint32(18).string(message.lensId);
		if (message.automaticDownload !== false) writer.uint32(24).bool(message.automaticDownload);
		if (message.downloadTimeSec !== 0) writer.uint32(33).double(message.downloadTimeSec);
		if (message.sizeByte !== "0") writer.uint32(40).uint64(message.sizeByte);
		return writer;
	},
	decode() {
		throw new Error("Not implemented.");
...
var CameraKitAssetDownload = {
	encode(message, writer = new BinaryWriter()) {
		...
		if (message.downloadTimeSec !== 0) writer.uint32(33).double(message.downloadTimeSec);
		if (message.sizeByte !== "0") writer.uint32(40).uint64(message.sizeByte);
		if (message.assetId !== "") writer.uint32(50).string(message.assetId);
		return writer;
	},
	decode() {
		throw new Error("Not implemented.");
...
var CameraKitException = {
	encode(message, writer = new BinaryWriter()) {
		if (message.cameraKitEventBase !== void 0) CameraKitEventBase.encode(message.cameraKitEventBase, writer.uint32(10).fork()).join();
		if (message.lensId !== "") writer.uint32(18).string(message.lensId);
		if (message.type !== "") writer.uint32(26).string(message.type);
...
var CameraKitSession$1 = {
	encode(message, writer = new BinaryWriter()) {
		if (message.cameraKitEventBase !== void 0) CameraKitEventBase.encode(message.cameraKitEventBase, writer.uint32(10).fork()).join();
		if (message.dailySessionBucket !== 0) writer.uint32(16).int32(message.dailySessionBucket);
		if (message.isFirstWithinMonth !== false) writer.uint32(24).bool(message.isFirstWithinMonth);
		if (message.day !== "0") writer.uint32(32).uint64(message.day);
		if (message.month !== "0") writer.uint32(40).uint64(message.month);
		if (message.year !== "0") writer.uint32(48).uint64(message.year);
		return writer;
	},
	decode() {
		throw new Error("Not implemented.");
...
var CameraKitAssetValidationFailed = {
	encode(message, writer = new BinaryWriter()) {
		if (message.cameraKitEventBase !== void 0) CameraKitEventBase.encode(message.cameraKitEventBase, writer.uint32(10).fork()).join();
		if (message.assetId !== "") writer.uint32(26).string(message.assetId);
		return writer;
	},
	decode() {
		throw new Error("Not implemented.");
...
var CameraKitWebBenchmarkComplete = {
	encode(message, writer = new BinaryWriter()) {
		if (message.cameraKitEventBase !== void 0) CameraKitEventBase.encode(message.cameraKitEventBase, writer.uint32(10).fork()).join();
		if (message.benchmarkName !== "") writer.uint32(18).string(message.benchmarkName);
		if (message.benchmarkValue !== 0) writer.uint32(25).double(message.benchmarkValue);
		if (message.performanceCluster !== "0") writer.uint32(32).uint64(message.performanceCluster);
		if (message.webglRendererInfo !== "") writer.uint32(42).string(message.webglRendererInfo);
		return writer;
	},
	decode() {
		throw new Error("Not implemented.");
...
var CameraKitLegalPrompt = {
	encode(message, writer = new BinaryWriter()) {
		if (message.cameraKitEventBase !== void 0) CameraKitEventBase.encode(message.cameraKitEventBase, writer.uint32(10).fork()).join();
		if (message.legalPromptId !== "") writer.uint32(18).string(message.legalPromptId);
		if (message.legalPromptResult !== 0) writer.uint32(24).int32(message.legalPromptResult);
		return writer;
	},
	decode() {
		throw new Error("Not implemented.");
...
var CameraKitEventBase = {
	encode(message, writer = new BinaryWriter()) {
		if (message.kitEventBase !== void 0) KitEventBase.encode(message.kitEventBase, writer.uint32(10).fork()).join();
		if (message.deviceCluster !== "0") writer.uint32(16).uint64(message.deviceCluster);
		if (message.cameraKitVersion !== "") writer.uint32(26).string(message.cameraKitVersion);
		if (message.lensCoreVersion !== "") writer.uint32(34).string(message.lensCoreVersion);
		if (message.deviceModel !== "") writer.uint32(42).string(message.deviceModel);
		if (message.cameraKitVariant !== 0) writer.uint32(48).int32(message.cameraKitVariant);
		if (message.cameraKitFlavor !== 0) writer.uint32(56).int32(message.cameraKitFlavor);
		if (message.appId !== "") writer.uint32(66).string(message.appId);
		if (message.deviceConnectivity !== 0) writer.uint32(72).int32(message.deviceConnectivity);
		if (message.sessionId !== "") writer.uint32(82).string(message.sessionId);
		if (message.appVendorUuid !== "") writer.uint32(90).string(message.appVendorUuid);
		if (message.rankingRequestId !== "") writer.uint32(98).string(message.rankingRequestId);
		if (message.cameraKitEnvironment !== 0) writer.uint32(104).int32(message.cameraKitEnvironment);
		if (message.partnerUuid !== "") writer.uint32(114).string(message.partnerUuid);
		return writer;
	},
	decode() {
		throw new Error("Not implemented.");
...
var KitEventBase = {
	encode(message, writer = new BinaryWriter()) {
		if (message.oauthClientId !== "") writer.uint32(10).string(message.oauthClientId);
		if (message.locale !== "") writer.uint32(18).string(message.locale);
		if (message.kitUserAgent !== "") writer.uint32(26).string(message.kitUserAgent);
		if (message.ipAddress !== "") writer.uint32(34).string(message.ipAddress);
		if (message.osMinorVersion !== "") writer.uint32(42).string(message.osMinorVersion);
		if (message.kitVariant !== 0) writer.uint32(48).int32(message.kitVariant);
		if (message.kitVariantVersion !== "") writer.uint32(58).string(message.kitVariantVersion);
		if (message.kitClientTimestampMillis !== "0") writer.uint32(64).uint64(message.kitClientTimestampMillis);
		if (message.clientSequenceId !== "0") writer.uint32(72).uint64(message.clientSequenceId);
		if (message.maxClientSequenceIdOnInstance !== "0") writer.uint32(80).uint64(message.maxClientSequenceIdOnInstance);
		if (message.targetArchitecture !== "") writer.uint32(90).string(message.targetArchitecture);
		if (message.runningWithDebuggerAttached !== false) writer.uint32(96).bool(message.runningWithDebuggerAttached);
		if (message.runningInTests !== false) writer.uint32(104).bool(message.runningInTests);
		if (message.runningInSimulator !== false) writer.uint32(112).bool(message.runningInSimulator);
		if (message.isAppPrerelease !== false) writer.uint32(120).bool(message.isAppPrerelease);
		if (message.kitAppId !== "") writer.uint32(130).string(message.kitAppId);
		if (message.kitSessionId !== "") writer.uint32(138).string(message.kitSessionId);
		if (message.kitPluginType !== 0) writer.uint32(144).int32(message.kitPluginType);
		if (message.isFromReactNativePlugin !== false) writer.uint32(152).bool(message.isFromReactNativePlugin);
		return writer;
	},
	decode() {
		throw new Error("Not implemented.");
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/handlers/mappingHandler.js
var createMappingHandler = (map, pageVisibility, maxMapConcurrency = Number.POSITIVE_INFINITY) => {
	const buffer = [];
	...
	const processRequest = (request) => __awaiter(void 0, void 0, void 0, function* () {
		try {
			...
			if (mapped instanceof Promise) request.next(yield mapped);
		...
		while (buffer.length > 0 && mapConcurrency < maxMapConcurrency) processRequest(buffer.shift());
	});
	if (pageVisibility) pageVisibility.onPageHidden(() => {
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/handlers/batchingHandler.js
var createBatchingHandler = ({ batchReduce, isBatchComplete, maxBatchAge, pageVisibility }) => {
	...
	const batchAndSend = (next, request, metadata) => {
		...
		if (!batch) return;
		const complete = batch instanceof Promise ? batch.then((b) => next(b, metadata)).catch(() => {}) : next(batch, metadata).catch(() => {});
		currentBatch = void 0;
	...
	return (next) => (request, metadata) => __awaiter(void 0, void 0, void 0, function* () {
		...
		if (currentBatch === void 0) {
			...
			if (maxBatchAge !== void 0) batchTimeout = setTimeout(sendBatch, maxBatchAge);
			if (pageVisibility) clearOnHidden = pageVisibility.onPageHidden(sendBatch);
		}
		return reducingHandler(() => __awaiter(void 0, void 0, void 0, function* () {
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/handlers/rateLimitingHandler.js
...
var createRateLimitingHandler = (duration, pageVisibility) => {
	...
	const mappingHandler = createMappingHandler((request) => __awaiter(void 0, void 0, void 0, function* () {
		if (mostRecentSendTime !== void 0) {
			...
			if (millisUntilNextSend > 0) yield delay(millisUntilNextSend);
		}
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/operational/Count.js
function isCountMetric(value) {
	var _a;
	return ((_a = value.metric) === null || _a === void 0 ? void 0 : _a.$case) === "count";
}
var Count = class Count extends Metric {
	...
	toOperationalMetric() {
		return [{
			...
			metric: {
				$case: "count",
				count: `${Math.ceil(this.count)}`
			}
		}];
	}
};
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/clients/metricsClient.js
...
var metricsClientFactory = Injectable("metricsClient", [
	externalMetricsSubjectFactory.token,
	grpcHandlerFactory.token,
	pageVisibilityFactory.token
], (externalMetricsSubjectFactory, grpcHandler, pageVisibility) => {
	const metrics = new MetricsClient(createTsProtoClient(MetricsDefinition, grpcHandler), pageVisibility);
	externalMetricsSubjectFactory.pipe(filter((metric) => validExternalMetrics.test(metric.name))).subscribe({ next: (metric) => {
		metrics.setOperationalMetrics(metric);
	} });
	return metrics;
});
var logger$24 = getLogger("cofHandler");
var cofHandlerFactory = Injectable("cofHandler", [
	configurationToken,
	requestStateEventTargetFactory.token,
	metricsClientFactory.token
], (config, requestStateEventTarget, metrics) => {
	...
	return new HandlerChainBuilder((request, _a) => __awaiter(void 0, void 0, void 0, function* () {
		...
		const client = new CircumstancesServiceClientImpl(new GrpcWebImpl(`https://${config.apiHostname}`, {}));
		return new Promise((resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
			var _b;
			if (signal) signal.addEventListener("abort", () => reject(/* @__PURE__ */ new Error("COF request aborted by handler chain.")));
			const cachedResponse = yield cofCache.retrieve(getCacheKey(request)).catch((e) => {
				logger$24.warn("Unable to get COF response from cache.", e);
				return {
			...
			const { requestId } = dispatchRequestStarted(requestStateEventTarget, { dimensions });
			try {
				const response = yield client.targetingQuery(Object.assign(Object.assign({}, request), {
					configResultsEtag: cachedResponse === null || cachedResponse === void 0 ? void 0 : cachedResponse.configResultsEtag,
					deltaSync: !!(cachedResponse === null || cachedResponse === void 0 ? void 0 : cachedResponse.configResultsEtag)
				}), new import_browser_headers_umd.BrowserHeaders(Object.assign({
	...
	})).map(createRetryingHandler({ retryPredicate: (r) => r instanceof Error })).map(createTimeoutHandler({ timeout: 20 * 1e3 })).map(createResponseCachingHandler(cofCache, getCacheKey, staleWhileRevalidateStrategy({ onMiss: () => {
		metrics.setOperationalMetrics(Count.count("cache_miss", 1, { request_type: "cof" }));
	} }))).handler;
});
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/remote-configuration/remoteConfiguration.js
var defaultTargetingRequest = { namespaces: [
	Namespace.LENS_CORE,
	Namespace.CAMERA_KIT_CORE,
	Namespace.LENS_CORE_CONFIG
] };
var RemoteConfiguration = class {
	constructor(lensPerformance, cofHandler, grpcClient) {
		this.configById = from(Promise.resolve(lensPerformance).then((lensPerformance) => {
			return (lensPerformance === null || lensPerformance === void 0 ? void 0 : lensPerformance.cluster) === 0 ? void 0 : lensPerformance === null || lensPerformance === void 0 ? void 0 : lensPerformance.cluster;
		})).pipe(mergeMap((lensClusterOrig4) => from(cofHandler(Object.assign(Object.assign({}, defaultTargetingRequest), { lensClusterOrig4 })))), map((result) => {
		...
		this.initializationConfig = from(grpcClient.getInitializationConfig({})).pipe(map((result) => {
			if (result.ok) {
				...
				else throw new Error(`Failed to load initialization config. gRPC response successful, but message was null. gRPC status: ${response.statusMessage}`);
			}
			throw new Error(`Failed to load initialization config. gRPC status message: ${result.unwrapErr().statusMessage}`);
		}), shareReplay(1));
	}
	get(configId) {
		return this.configById.pipe(map((config) => {
			var _a;
			return (_a = config.get(configId)) !== null && _a !== void 0 ? _a : [];
		}));
	}
	getInitializationConfig() {
	...
	getGpuIndexConfig() {
		...
		return this.get(lensClusterConfigName).pipe(map((configResults) => {
			var _a, _b;
			if (configResults.length === 0) throw new Error(`Cannot find '${lensClusterConfigName}' config.`);
	...
	getNamespace(namespace) {
		return this.configById.pipe(map((configs) => {
			return Array.from(configs.values()).filter((values) => values.some((c) => c.namespace === namespace)).flatMap((results) => results);
		}));
	}
};
var remoteConfigurationFactory = Injectable("remoteConfiguration", [
	configurationToken,
	cofHandlerFactory.token,
	grpcHandlerFactory.token
], (config, cofHandler, grpcHandler) => {
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens/assets/deviceDependentAssetLoader.js
var logger$23 = getLogger("deviceDependentAssetLoader");
	...
	return function deviceDependentAssetLoader({ assetDescriptor: { assetId }, lowPriority }) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			return firstValueFrom(remoteConfiguration.get(assetId).pipe(catchError((error) => {
				...
				if (configs.length === 0) throw loadingFailed(`No COF config found corresponding to that assetId.`);
				const [{ value }] = configs;
				...
				logger$23.info(`COF request for ${assetId}. url: ${assetConfig.url}. checksum: ${assetConfig.checksum}.`);
				return assetConfig;
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens/assets/staticAssetLoader.js
var staticAssetLoaderFactory = Injectable("staticAssetLoader", [defaultFetchHandlerFactory.token], (fetchHandler) => {
	...
	return ({ assetDescriptor: { assetId }, assetManifest, lowPriority }) => __awaiter(void 0, void 0, void 0, function* () {
		...
		if (!assetUrl) throw new Error(`Cannot load lens asset ${assetId}. Static asset URL could not be found.`);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens-core-module/lensCoreError.js
var lensCoreErrorName = Object.fromEntries(Object.entries({
...
function wrapLensCoreError(unknownError, isFrameError) {
	...
	const error = new Error(lcError.message.split("\n")[0], { cause: lcError.otherExceptions || ((_a = lcError.cause) === null || _a === void 0 ? void 0 : _a.metadata) ? {
		otherExceptions: lcError.otherExceptions,
		metadata: (_b = lcError.cause) === null || _b === void 0 ? void 0 : _b.metadata
	} : void 0 });
	error.name = `LensCore${(_e = lensCoreErrorName[(_d = (_c = lcError.cause) === null || _c === void 0 ? void 0 : _c.type) === null || _d === void 0 ? void 0 : _d.value]) !== null && _e !== void 0 ? _e : "Unknown"}Error`;
	error.isFrameError = isFrameError;
	if (lcError.stack) {
		const [_, ...stackLines] = lcError.stack.split("\n");
		if (error.stack) stackLines.unshift(error.stack.split("\n")[0]);
		error.stack = stackLines.join("\n");
	}
	return error;
}
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens-core-module/lensCore.js
...
var createLensCore = (lensCoreModule) => {
	...
	const customMethods = {
		initialize(input) {
			return new Promise((onSuccess, onFailure) => {
				var _a;
				return lensCoreModule.initialize(Object.assign(Object.assign({}, input), {
					exceptionHandler: getFrameErrorWrapper((_a = input.exceptionHandler) !== null && _a !== void 0 ? _a : (() => {})),
					onSuccess,
		...
		playCanvas(input) {
			return new Promise((onReady, onFailure) => {
				lensCoreModule.playCanvas(Object.assign(Object.assign({}, input), {
					onReady,
					onFailure: getRegularErrorWrapper(onFailure)
				}));
			});
	...
	return new Proxy(lensCoreModule, { get: (target, property, receiver) => {
		if (property === errorsFieldName) return errorsObservable;
		if (property in customMethods) return withTryCatch(customMethods[property], errors);
		const targetProperty = Reflect.get(target, property, receiver);
		if (!targetProperty) return targetProperty;
		if (property in promisifiableMethods) return withTryCatch(function(input) {
			return new Promise((onSuccess, onFailure) => targetProperty(Object.assign(Object.assign({}, input), {
				onSuccess,
				onFailure: getRegularErrorWrapper(onFailure)
			})));
		}, errors);
		if (typeof targetProperty === "function") if ("values" in targetProperty) return targetProperty;
		else return withTryCatch(targetProperty, errors);
		return targetProperty;
	} });
};
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/common/loadScript.js
function loadScript(scriptUri) {
	return new Promise((resolve, reject) => {
		const scriptElement = document.createElement("script");
		scriptElement.src = scriptUri;
		scriptElement.async = true;
		merge(fromEvent(scriptElement, "load").pipe(tap(() => resolve(scriptElement))), fromEvent(scriptElement, "error").pipe(tap((e) => reject(e)))).pipe(take(1)).subscribe();
		document.body.appendChild(scriptElement);
	});
}
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/handlers/customLensCoreHandler.js
var createCustomLensCoreHandler = () => {
	return (next) => (input, init) => {
		var _a;
		if ((isString(input) ? input : (_a = input === null || input === void 0 ? void 0 : input.url) !== null && _a !== void 0 ? _a : "").startsWith("https://lens-core-wasm.sc-corp.net/")) return next(input, Object.assign(Object.assign({}, init), { credentials: "include" }));
		return next(input, init);
	};
};
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/trusted-types/trustedTypesHandler.js
...
function createHandler(trustedTypesPolicyName) {
	const trustedUrls = /* @__PURE__ */ new Set();
	const trustedTypes = globalThis.trustedTypes;
	const policy = trustedTypes === null || trustedTypes === void 0 ? void 0 : trustedTypes.createPolicy(trustedTypesPolicyName, { createScriptURL: (url) => {
...
var relaxedSimd = async () => WebAssembly.validate(new Uint8Array([
	0,
	97,
	115,
	109,
...
var simd = async () => WebAssembly.validate(new Uint8Array([
	0,
	97,
	115,
	109,
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/platform/platformCapabilities.js
...
var getPlatformCapabilities = memoize(function getPlatformCapabilities() {
	return __awaiter(this, void 0, void 0, function* () {
		return {
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens-core-module/loader/bootstrapURLs.js
var wasmAssets = ["LensCoreWebAssembly.js", "LensCoreWebAssembly.wasm"];
function getBuildFlavor({ simd, relaxedSimd }, versionNumber) {
	if (parseInt(versionNumber) >= 282 && relaxedSimd) return "rel-relaxed-simd-neh";
	else if (simd) return "rel-simd-neh";
	else return "rel-neh";
}
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens-core-module/loader/lensCoreFactory.js
var logger$22 = getLogger("lensCoreFactory");
var findMatch = (regex, strings) => strings.find((s) => regex.test(s));
var LENS_CORE_JS_REQUEST_TYPE = "lens_core_js";
var LENS_CORE_WASM_REQUEST_TYPE = "lens_core_wasm";
var lensCoreFactory = Injectable("lensCore", [
	defaultFetchHandlerFactory.token,
	configurationToken,
	requestStateEventTargetFactory.token
], (handler, { lensCoreOverrideUrls, wasmEndpointOverride, trustedTypesPolicyName }, requestStateEventTarget) => __awaiter(void 0, void 0, void 0, function* () {
	var _a, _b;
	let lensCoreJS;
	let lensCoreWASM;
	...
	if (customBuild) lensCoreHandlerChainBuilder = lensCoreHandlerChainBuilder.map(createCustomLensCoreHandler());
	...
	if (lensCoreOverrideUrls) {
		...
		const assetURLs = yield getRequiredBootstrapURLs(wasmEndpointOverride !== null && wasmEndpointOverride !== void 0 ? wasmEndpointOverride : void 0);
		lensCoreJS = (_a = findMatch(/\.js/, assetURLs)) !== null && _a !== void 0 ? _a : "";
		lensCoreWASM = (_b = findMatch(/\.wasm/, assetURLs)) !== null && _b !== void 0 ? _b : "";
		if (!lensCoreJS || !lensCoreWASM) throw new Error(`Cannot fetch required LensCore assets. Either the JS or WASM filename is missing from this list: ${assetURLs}.`);
		const glueScript = yield lensCoreHandler([lensCoreJS, {
			requestType: LENS_CORE_JS_REQUEST_TYPE,
	...
	const lensCore = yield new Promise((resolve, reject) => {
		...
		const moduleInit = globalThis.createLensesModule(initialModule = {
			...
			instantiateWasm: (importObject, receiveInstance) => {
				WebAssembly.instantiateStreaming(lensCoreHandler([lensCoreWASM, {
					requestType: LENS_CORE_WASM_REQUEST_TYPE,
					customBuild: `${customBuild}`
	...
	if (lensCoreWasmVersions_default.version != `${lensCore.getCoreVersion()}`) logger$22.warn(`Loaded LensCore version (${lensCore.getCoreVersion()}) differs from expected one (${lensCoreWasmVersions_default.version})`);
	return createLensCore(lensCore);
}));
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/common/validate.js
...
function getTypeName(value) {
	...
	if (baseType === "function" && Function.prototype.toString.call(nonNullValue).startsWith("class")) return "class";
...
function validate(...guards) {
	return function validator(target, context) {
		return function(...args) {
			for (const [index, guard] of guards.entries()) if (!guard(args[index])) {
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/logger/errorLoggingDecorator.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/common/assertions.js
function assertUnreachable(_) {
	throw new Error("Reached unreachable code at runtime.");
}
function assert(condition, error = "Assertion failed") {
	if (!condition) if (error instanceof Error) throw error;
	else throw new Error(error);
}
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens/Lens.js
...
function toPublicLens({ id, groupId, name, content, vendorData, cameraFacingPreference, lensCreator, scannable, featureMetadata }) {
	...
	return {
		...
		lensCreator,
		snapcode: scannable ? {
			imageUrl: scannable.snapcodeImageUrl,
			deepLink: scannable.snapcodeDeeplink
		} : void 0,
		featureMetadata
	};
}
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/metricsEventTarget.js
var metricsEventTargetFactory = Injectable("metricsEventTarget", () => new TypedEventTarget());
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens/assets/LensAssetRepository.js
var logger$21 = getLogger("LensAssetRepository");
function getCacheKey(asset) {
	return `${asset.assetId}_${asset.assetType.value}`;
}
function mapManfiestItemToAssetType(lensCore, type) {
	switch (type) {
		case LensAssetManifestItem_Type.ASSET: return lensCore.AssetType.Static;
		case LensAssetManifestItem_Type.DEVICE_DEPENDENT_ASSET_UNSET:
		case LensAssetManifestItem_Type.UNRECOGNIZED: return lensCore.AssetType.DeviceDependent;
...
var LensAssetRepository = class {
	...
	cacheAssets(assetManifest, lens, assetTimings = [LensAssetManifestItem_RequestTiming.REQUIRED], lowPriority = false) {
		return __awaiter(this, void 0, void 0, function* () {
			const assetTimingsToPreload = new Set([LensAssetManifestItem_RequestTiming.PRELOAD_UNSET, ...assetTimings]);
			...
			if (assetDescriptors.length) return this.cacheAssetsByDescriptor(assetDescriptors, lens, assetManifest, lowPriority);
		});
	}
	loadAsset(request) {
		var _a, _b;
		return __awaiter(this, void 0, void 0, function* () {
			...
			try {
				if (!assetLoader) throw new Error(`Cannot get asset ${assetId}. Asset type ${safeAssetTypeName} is not supported.`);
				...
				this.lensCore.provideRemoteAssetsResponse({
					...
					onFailure: (lensCoreError) => {
						if (/validation failed/.test(lensCoreError.message)) this.metrics.dispatchEvent(new TypedCustomEvent("assetValidationFailed", {
							name: "assetValidationFailed",
							assetId
						}));
						logger$21.warn(`Failed to provide lens asset ${assetId}.`, lensCoreError);
					}
				});
			} catch (error) {
				const wrappedError = new Error(`Failed to load lens asset ${assetId}.`, { cause: error });
				dispatchRequestErrored(this.requestStateEventTarget, {
					requestId,
					dimensions,
					error: wrappedError
				});
	...
	cacheAssetsByDescriptor(assetDescriptors, lens, assetManifest, lowPriority) {
		return __awaiter(this, void 0, void 0, function* () {
			yield Promise.all(assetDescriptors.filter((assetDescriptors) => !this.cachedAssetKeys.has(getCacheKey(assetDescriptors))).map((assetDescriptor) => __awaiter(this, void 0, void 0, function* () {
				var _a;
				try {
					...
					logger$21.warn(`Failed to cache asset ${assetId} of type ${assetTypeName !== null && assetTypeName !== void 0 ? assetTypeName : assetType.value}.`, error);
				}
			})));
		});
	}
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/camera_kit/v3/export.js
...
var ExtensionRequestContext = {
	encode(message, writer = new BinaryWriter()) {
		if (message.userAgent !== "") writer.uint32(10).string(message.userAgent);
		if (message.locale !== "") writer.uint32(18).string(message.locale);
		return writer;
	},
	decode(input, length) {
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens/LensSource.js
...
function loadLensesFromSources(sources, groupId, lensId) {
	return __awaiter(this, void 0, void 0, function* () {
		...
		if (!source) throw new Error(`Cannot load lens ${lensId ? `${lensId} from ` : ""}group ${groupId}. No LensSource claimed ownership of that lens group.`);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens/LensRepository.js
var logger$20 = getLogger("LensRepository");
var assetTimingMap = {
	...
	return _a = class LensRepository {
		...
		loadLens(lensId, groupId) {
			return __awaiter(this, void 0, void 0, function* () {
				...
				if (!lens) throw new Error(`Cannot load lens. No lens with id ${lensId} was found in lens group ${groupId}.`);
		...
		loadLensGroups(groupIds) {
			return __awaiter(this, void 0, void 0, function* () {
				return (yield Promise.all(groupIds.map((groupId) => __awaiter(this, void 0, void 0, function* () {
					try {
						...
						logger$20.error(new Error(`Failed to load lens group ${groupId}.`, { cause: error }));
						return error;
					}
			...
			return __awaiter(this, void 0, void 0, function* () {
				...
				yield Promise.all(lenses.map((lens) => __awaiter(this, void 0, void 0, function* () {
					try {
						...
						logger$20.warn(`Failed to cache lens ${lens.id}.`, error);
					}
				})));
			});
		}
		...
		getLensContent(lens, lowPriority = false) {
			...
			return __awaiter(this, void 0, void 0, function* () {
				...
				if (!content) throw new Error(`Cannot find metadata for lens ${lens.id}.`);
				const cachedLensBuffer = this.binariesCache.get(lens.id);
				if (cachedLensBuffer) return {
					lensBuffer: cachedLensBuffer,
					lensChecksum: content.lnsSha256
				};
				const [lensBuffer] = yield this.lensFetchHandler([new Request(content.lnsUrlBolt, withRequestPriority({ cache: "force-cache" }, lowPriority)), {
		...
		_loadLens_decorators = [validate(isSafeString, isSafeString), errorLoggingDecorator(logger$20)];
		_loadLensGroups_decorators = [validate(isSafeStringArray), errorLoggingDecorator(logger$20)];
		_cacheLensContent_decorators = [validate(isLensArray, isAssetTimingArrayOrUndefined), errorLoggingDecorator(logger$20)];
		__esDecorate(_a, null, _loadLens_decorators, {
			kind: "method",
			name: "loadLens",
			static: false,
			private: false,
			access: {
				has: (obj) => "loadLens" in obj,
				get: (obj) => obj.loadLens
			}
		}, null, _instanceExtraInitializers);
		__esDecorate(_a, null, _loadLensGroups_decorators, {
			kind: "method",
			name: "loadLensGroups",
			static: false,
			private: false,
			access: {
				has: (obj) => "loadLensGroups" in obj,
				get: (obj) => obj.loadLensGroups
			}
		}, null, _instanceExtraInitializers);
		__esDecorate(_a, null, _cacheLensContent_decorators, {
			kind: "method",
			name: "cacheLensContent",
			static: false,
			private: false,
			access: {
				has: (obj) => "cacheLensContent" in obj,
				get: (obj) => obj.cacheLensContent
			}
		}, null, _instanceExtraInitializers);
	})(), _a;
})();
...
//#endregion
//#region node_modules/.pnpm/@snap+state-management@1.13.0_rxjs@7.8.2/node_modules/@snap/state-management/dist/types.js
var fromEntries = (entries) => Object.fromEntries(entries);
//#endregion
//#region node_modules/.pnpm/@snap+state-management@1.13.0_rxjs@7.8.2/node_modules/@snap/state-management/dist/actions.js
/**
...
var defineActions = (...actions) => {
	return fromEntries(actions.map((s) => [s.name, s]));
};
/**
...
//#endregion
//#region node_modules/.pnpm/@snap+state-management@1.13.0_rxjs@7.8.2/node_modules/@snap/state-management/dist/state-machine.js
/**
...
var StateMachine = class {
	...
	*/
	constructor(actions, states, initialState, reducer) {
		...
		this.actionsSubject.pipe(withLatestFrom(this.state), mergeMap(([a, s]) => NEVER.pipe(startWith([a, s]), reducer, raceWith(of(s)), tap((newState) => {
			if (newState !== s) this.state.next(newState);
		}), map((newState) => [a, newState]), take(1)))).subscribe(this.eventsSubject);
	}
...
//#endregion
//#region node_modules/.pnpm/@snap+state-management@1.13.0_rxjs@7.8.2/node_modules/@snap/state-management/dist/states.js
/**
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/lenses/launch_params.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/lenses/geopoint.js
...
var Geopoint = {
	encode(message, writer = new BinaryWriter()) {
		if (message.latitude !== 0) writer.uint32(9).double(message.latitude);
		if (message.longitude !== 0) writer.uint32(17).double(message.longitude);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/lenses/geocircle.js
...
var Geocircle = {
	encode(message, writer = new BinaryWriter()) {
		if (message.radius !== 0) writer.uint32(9).double(message.radius);
		if (message.center !== void 0) Geopoint.encode(message.center, writer.uint32(18).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/lenses/lures.js
...
var Lures = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.locations) Geocircle.encode(v, writer.uint32(10).fork()).join();
		if (message.activeLure !== void 0) Geocircle.encode(message.activeLure, writer.uint32(18).fork()).join();
		if (message.closestLure !== void 0) Geocircle.encode(message.closestLure, writer.uint32(26).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/lenses/persistent_store.js
function createBasePersistentStore() {
	return { store: new Uint8Array(0) };
}
var PersistentStore = {
	encode(message, writer = new BinaryWriter()) {
		if (message.store.length !== 0) writer.uint32(10).bytes(message.store);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/lenses/snappable.js
...
function encryptionData_EncryptionSchemeFromJSON(object) {
	switch (object) {
		case 0:
		case "NOT_APPLICABLE": return EncryptionData_EncryptionScheme.NOT_APPLICABLE;
		case 1:
		case "SPOOKEY": return EncryptionData_EncryptionScheme.SPOOKEY;
		case 2:
		case "REGISTRY": return EncryptionData_EncryptionScheme.REGISTRY;
		case 3:
		case "FIDELIUS": return EncryptionData_EncryptionScheme.FIDELIUS;
		default: return EncryptionData_EncryptionScheme.UNRECOGNIZED;
	}
}
function encryptionData_EncryptionSchemeToNumber(object) {
	switch (object) {
		case EncryptionData_EncryptionScheme.NOT_APPLICABLE: return 0;
		case EncryptionData_EncryptionScheme.SPOOKEY: return 1;
		case EncryptionData_EncryptionScheme.REGISTRY: return 2;
		case EncryptionData_EncryptionScheme.FIDELIUS: return 3;
...
var EncryptionData = {
	encode(message, writer = new BinaryWriter()) {
		...
		Object.entries(message.ivStore).forEach(([key, value]) => {
			EncryptionData_IvStoreEntry.encode({
				key,
				value
			}, writer.uint32(18).fork()).join();
		});
		if (message.isKeyServerEncrypted !== false) writer.uint32(24).bool(message.isKeyServerEncrypted);
		if (message.isKeyE2eEncrypted !== false) writer.uint32(32).bool(message.isKeyE2eEncrypted);
		if (message.encryptionScheme !== EncryptionData_EncryptionScheme.NOT_APPLICABLE) writer.uint32(40).int32(encryptionData_EncryptionSchemeToNumber(message.encryptionScheme));
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				...
				case 2: {
					...
					if (entry2.value !== void 0) message.ivStore[entry2.key] = entry2.value;
					continue;
				}
				case 3:
					if (tag !== 24) break;
...
function createBasePlayerData() {
	return {
		store: {},
		serializedStore: new Uint8Array(0)
	};
}
var PlayerData = {
	encode(message, writer = new BinaryWriter()) {
		Object.entries(message.store).forEach(([key, value]) => {
			PlayerData_StoreEntry.encode({
				key,
				value
			}, writer.uint32(10).fork()).join();
		});
		if (message.serializedStore.length !== 0) writer.uint32(18).bytes(message.serializedStore);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				case 1: {
					...
					if (entry1.value !== void 0) message.store[entry1.key] = entry1.value;
					continue;
				}
				case 2:
					if (tag !== 18) break;
...
var SessionData = {
	encode(message, writer = new BinaryWriter()) {
		Object.entries(message.playerData).forEach(([key, value]) => {
			SessionData_PlayerDataEntry.encode({
				key,
				value
			}, writer.uint32(10).fork()).join();
		});
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				case 1: {
					...
					if (entry1.value !== void 0) message.playerData[entry1.key] = entry1.value;
					continue;
				}
			}
			if ((tag & 7) === 4 || tag === 0) break;
	...
	fromPartial(object) {
		...
		message.playerData = Object.entries((_a = object.playerData) !== null && _a !== void 0 ? _a : {}).reduce((acc, [key, value]) => {
			if (value !== void 0) acc[key] = PlayerData.fromPartial(value);
			return acc;
		}, {});
		return message;
	}
...
var SessionData_PlayerDataEntry = {
	encode(message, writer = new BinaryWriter()) {
		if (message.key !== "") writer.uint32(10).string(message.key);
		if (message.value !== void 0) PlayerData.encode(message.value, writer.uint32(18).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var Snappable = {
	encode(message, writer = new BinaryWriter()) {
		...
		if (message.playerId !== "") writer.uint32(26).string(message.playerId);
		if (message.sessionData !== void 0) SessionData.encode(message.sessionData, writer.uint32(34).fork()).join();
		if (message.createdTime !== "0") writer.uint32(40).int64(message.createdTime);
		if (message.expireTime !== "0") writer.uint32(48).int64(message.expireTime);
		if (message.version !== 0) writer.uint32(56).int32(message.version);
		if (message.encryptionData !== void 0) EncryptionData.encode(message.encryptionData, writer.uint32(66).fork()).join();
		if (message.sessionDepth !== 0) writer.uint32(72).int32(message.sessionDepth);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/lenses/user_data.js
var UserData_Zodiac;
...
function userData_ZodiacFromJSON(object) {
	switch (object) {
		case 0:
		case "Invalid": return UserData_Zodiac.Invalid;
		case 1:
		case "Aquarius": return UserData_Zodiac.Aquarius;
		case 2:
		case "Aries": return UserData_Zodiac.Aries;
		case 3:
		case "Cancer": return UserData_Zodiac.Cancer;
		case 4:
		case "Capricorn": return UserData_Zodiac.Capricorn;
		case 5:
		case "Gemini": return UserData_Zodiac.Gemini;
		case 6:
		case "Leo": return UserData_Zodiac.Leo;
		case 7:
		case "Libra": return UserData_Zodiac.Libra;
		case 8:
		case "Pisces": return UserData_Zodiac.Pisces;
		case 9:
		case "Sagittarius": return UserData_Zodiac.Sagittarius;
		case 10:
		case "Scorpio": return UserData_Zodiac.Scorpio;
		case 11:
		case "Taurus": return UserData_Zodiac.Taurus;
		case 12:
		case "Virgo": return UserData_Zodiac.Virgo;
		default: return UserData_Zodiac.UNRECOGNIZED;
	}
}
function userData_ZodiacToNumber(object) {
	switch (object) {
		case UserData_Zodiac.Invalid: return 0;
		case UserData_Zodiac.Aquarius: return 1;
		case UserData_Zodiac.Aries: return 2;
		case UserData_Zodiac.Cancer: return 3;
		case UserData_Zodiac.Capricorn: return 4;
		case UserData_Zodiac.Gemini: return 5;
		case UserData_Zodiac.Leo: return 6;
		case UserData_Zodiac.Libra: return 7;
		case UserData_Zodiac.Pisces: return 8;
		case UserData_Zodiac.Sagittarius: return 9;
		case UserData_Zodiac.Scorpio: return 10;
		case UserData_Zodiac.Taurus: return 11;
		case UserData_Zodiac.Virgo: return 12;
...
var BitmojiUserInfo = {
	encode(message, writer = new BinaryWriter()) {
		if (message.avatarId !== "") writer.uint32(10).string(message.avatarId);
		if (message.selfieId !== "") writer.uint32(18).string(message.selfieId);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var FriendUserInfo = {
	encode(message, writer = new BinaryWriter()) {
		if (message.friendshipStart !== void 0) Timestamp.encode(toTimestamp$1(message.friendshipStart), writer.uint32(10).fork()).join();
		if (message.lastInteraction !== void 0) Timestamp.encode(toTimestamp$1(message.lastInteraction), writer.uint32(18).fork()).join();
		if (message.streak !== 0) writer.uint32(24).uint32(message.streak);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				case 1:
					if (tag !== 10) break;
					message.friendshipStart = fromTimestamp$1(Timestamp.decode(reader, reader.uint32()));
					continue;
				case 2:
					if (tag !== 18) break;
					message.lastInteraction = fromTimestamp$1(Timestamp.decode(reader, reader.uint32()));
					continue;
				case 3:
					if (tag !== 24) break;
...
var UserData = {
	encode(message, writer = new BinaryWriter()) {
		...
		if (message.birthdate !== "") writer.uint32(26).string(message.birthdate);
		if (message.displayName !== "") writer.uint32(34).string(message.displayName);
		if (message.countrycode !== "") writer.uint32(42).string(message.countrycode);
		if (message.score !== "0") writer.uint32(56).uint64(message.score);
		if (message.bitmojiInfo !== void 0) BitmojiUserInfo.encode(message.bitmojiInfo, writer.uint32(66).fork()).join();
		if (message.friendInfo !== void 0) FriendUserInfo.encode(message.friendInfo, writer.uint32(74).fork()).join();
		if (message.zodiac !== UserData_Zodiac.Invalid) writer.uint32(80).int32(userData_ZodiacToNumber(message.zodiac));
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/lenses/launchdata.js
...
function launchData_EntryPointFromJSON(object) {
	switch (object) {
		case 0:
		case "LIVE_CAMERA": return LaunchData_EntryPoint.LIVE_CAMERA;
		case 1:
		case "STORY_REPLY": return LaunchData_EntryPoint.STORY_REPLY;
		case 2:
		case "CHAT_REPLY": return LaunchData_EntryPoint.CHAT_REPLY;
		case 3:
		case "RESTART": return LaunchData_EntryPoint.RESTART;
		case 4:
		case "PREVIEW_CANCEL": return LaunchData_EntryPoint.PREVIEW_CANCEL;
		case 5:
		case "MAP": return LaunchData_EntryPoint.MAP;
		case 6:
		case "BITMOJI_STICKERS": return LaunchData_EntryPoint.BITMOJI_STICKERS;
		case 7:
		case "POST_CAPTURE_PREVIEW": return LaunchData_EntryPoint.POST_CAPTURE_PREVIEW;
		case 8:
		case "POST_CAPTURE_TRANSCODING": return LaunchData_EntryPoint.POST_CAPTURE_TRANSCODING;
		default: return LaunchData_EntryPoint.UNRECOGNIZED;
	}
}
function launchData_EntryPointToNumber(object) {
	switch (object) {
		case LaunchData_EntryPoint.LIVE_CAMERA: return 0;
		case LaunchData_EntryPoint.STORY_REPLY: return 1;
		case LaunchData_EntryPoint.CHAT_REPLY: return 2;
		case LaunchData_EntryPoint.RESTART: return 3;
		case LaunchData_EntryPoint.PREVIEW_CANCEL: return 4;
		case LaunchData_EntryPoint.MAP: return 5;
		case LaunchData_EntryPoint.BITMOJI_STICKERS: return 6;
		case LaunchData_EntryPoint.POST_CAPTURE_PREVIEW: return 7;
		case LaunchData_EntryPoint.POST_CAPTURE_TRANSCODING: return 8;
...
function launchData_ApiDescriptorFromJSON(object) {
	switch (object) {
		case 0:
		case "NONE": return LaunchData_ApiDescriptor.NONE;
		case 1:
		case "EXPERIMENTAL": return LaunchData_ApiDescriptor.EXPERIMENTAL;
		default: return LaunchData_ApiDescriptor.UNRECOGNIZED;
	}
}
function launchData_ApiDescriptorToNumber(object) {
	switch (object) {
		case LaunchData_ApiDescriptor.NONE: return 0;
		case LaunchData_ApiDescriptor.EXPERIMENTAL: return 1;
...
function launchData_RenderConfigFromJSON(object) {
	switch (object) {
		case 0:
		case "DEFAULT": return LaunchData_RenderConfig.DEFAULT;
		case 1:
		case "REPLAY_STREAM": return LaunchData_RenderConfig.REPLAY_STREAM;
		default: return LaunchData_RenderConfig.UNRECOGNIZED;
	}
}
function launchData_RenderConfigToNumber(object) {
	switch (object) {
		case LaunchData_RenderConfig.DEFAULT: return 0;
		case LaunchData_RenderConfig.REPLAY_STREAM: return 1;
...
function launchData_ActivationSourceFromJSON(object) {
	switch (object) {
		case 0:
		case "ACTIVATION_SOURCE_UNSET": return LaunchData_ActivationSource.ACTIVATION_SOURCE_UNSET;
		case 1:
		case "ACTIVATION_SOURCE_DEFAULT": return LaunchData_ActivationSource.ACTIVATION_SOURCE_DEFAULT;
		case 2:
		case "CREATIVE": return LaunchData_ActivationSource.CREATIVE;
		case 3:
		case "SCAN": return LaunchData_ActivationSource.SCAN;
		case 4:
		case "SCAN_HISTORY": return LaunchData_ActivationSource.SCAN_HISTORY;
		case 5:
		case "CHAT_FEED_PSA": return LaunchData_ActivationSource.CHAT_FEED_PSA;
		case 6:
		case "GROWTH_NOTIFICATION": return LaunchData_ActivationSource.GROWTH_NOTIFICATION;
		case 7:
		case "MASS_SNAP": return LaunchData_ActivationSource.MASS_SNAP;
		case 8:
		case "SMART_CTA": return LaunchData_ActivationSource.SMART_CTA;
		case 9:
		case "MASS_CHAT": return LaunchData_ActivationSource.MASS_CHAT;
		case 10:
		case "BILLBOARD_FHP": return LaunchData_ActivationSource.BILLBOARD_FHP;
		case 11:
		case "LENS_ACTIVITY_CENTER": return LaunchData_ActivationSource.LENS_ACTIVITY_CENTER;
		case 12:
		case "AR_BAR": return LaunchData_ActivationSource.AR_BAR;
		default: return LaunchData_ActivationSource.UNRECOGNIZED;
	}
}
function launchData_ActivationSourceToNumber(object) {
	switch (object) {
		case LaunchData_ActivationSource.ACTIVATION_SOURCE_UNSET: return 0;
		case LaunchData_ActivationSource.ACTIVATION_SOURCE_DEFAULT: return 1;
		case LaunchData_ActivationSource.CREATIVE: return 2;
		case LaunchData_ActivationSource.SCAN: return 3;
		case LaunchData_ActivationSource.SCAN_HISTORY: return 4;
		case LaunchData_ActivationSource.CHAT_FEED_PSA: return 5;
		case LaunchData_ActivationSource.GROWTH_NOTIFICATION: return 6;
		case LaunchData_ActivationSource.MASS_SNAP: return 7;
		case LaunchData_ActivationSource.SMART_CTA: return 8;
		case LaunchData_ActivationSource.MASS_CHAT: return 9;
		case LaunchData_ActivationSource.BILLBOARD_FHP: return 10;
		case LaunchData_ActivationSource.LENS_ACTIVITY_CENTER: return 11;
		case LaunchData_ActivationSource.AR_BAR: return 12;
...
var LaunchData = {
	encode(message, writer = new BinaryWriter()) {
		if (message.snappable !== void 0) Snappable.encode(message.snappable, writer.uint32(10).fork()).join();
		if (message.lures !== void 0) Lures.encode(message.lures, writer.uint32(18).fork()).join();
		if (message.userId !== "") writer.uint32(26).string(message.userId);
		if (message.entryPoint !== LaunchData_EntryPoint.LIVE_CAMERA) writer.uint32(32).int32(launchData_EntryPointToNumber(message.entryPoint));
		if (message.userData !== void 0) UserData.encode(message.userData, writer.uint32(42).fork()).join();
		if (message.persistentStore !== void 0) PersistentStore.encode(message.persistentStore, writer.uint32(50).fork()).join();
		if (message.launchParams !== void 0) LaunchParams.encode(message.launchParams, writer.uint32(58).fork()).join();
		writer.uint32(66).fork();
		for (const v of message.apiDescriptors) writer.int32(launchData_ApiDescriptorToNumber(v));
		writer.join();
		if (message.renderConfig !== LaunchData_RenderConfig.DEFAULT) writer.uint32(72).int32(launchData_RenderConfigToNumber(message.renderConfig));
		if (message.activationSource !== LaunchData_ActivationSource.ACTIVATION_SOURCE_UNSET) writer.uint32(80).int32(launchData_ActivationSourceToNumber(message.activationSource));
		if (message.overridenLaunchTime !== void 0) Timestamp.encode(toTimestamp(message.overridenLaunchTime), writer.uint32(90).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				...
				case 8:
					...
					if (tag === 66) {
						const end2 = reader.uint32() + reader.pos;
						while (reader.pos < end2) message.apiDescriptors.push(launchData_ApiDescriptorFromJSON(reader.int32()));
						continue;
					}
					break;
				case 9:
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens/LensLaunchData.js
var zodiacMap = {
...
var encodeLensLaunchData = (launchData, persistentStore) => {
	var _a;
	return LaunchData.encode(LaunchData.fromPartial(Object.assign(Object.assign({}, launchData), {
		userData: launchData.userData ? Object.assign(Object.assign({}, launchData.userData), { zodiac: ((_a = launchData.userData) === null || _a === void 0 ? void 0 : _a.zodiac) ? zodiacMap[launchData.userData.zodiac] : void 0 }) : void 0,
		launchParams: launchData.launchParams ? encodeLensLaunchParams(launchData.launchParams) : void 0,
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/transforms/Transform2D.js
var Transform2D = class {
	constructor(matrix) {
		this.matrix = matrix;
	}
};
Transform2D.MirrorX = new Transform2D([
	-1,
	0,
	0,
	0,
...
Transform2D.MirrorY = new Transform2D([
	1,
	0,
	0,
	0,
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/media-sources/CameraKitSource.js
var defaultDeviceInfo = {
	cameraType: "user",
	fpsLimit: Number.POSITIVE_INFINITY
};
var createNotAttachedError = (message) => /* @__PURE__ */ new Error(`${message}. This CameraKitSource is not attached to a CameraKitSession.`);
...
var CameraKitSource = (() => {
	...
	return _a = class CameraKitSource {
		...
		attach(lensCore, frameEvents, reportError) {
			return __awaiter(this, void 0, void 0, function* () {
				if (this.lensCore) throw new Error("This CameraKitSource has already been attached to this CameraKitSession and cannot be reattached, even after detachment.");
				this.lensCore = lensCore;
				...
				if (this.subscriber.onAttach) yield this.subscriber.onAttach(this, lensCore, reportError, frameEvents);
			});
		}
		copy(deviceInfo = {}) {
		...
		setRenderSize(width, height) {
			...
			return this.lensCore.setRenderSize({
				mode: "explicit",
				target
			});
		}
		...
		__esDecorate(_a, null, _setRenderSize_decorators, {
			kind: "method",
			name: "setRenderSize",
			static: false,
			private: false,
			access: {
				has: (obj) => "setRenderSize" in obj,
				get: (obj) => obj.setRenderSize
			}
		}, null, _instanceExtraInitializers);
		__esDecorate(_a, null, _setTransform_decorators, {
			kind: "method",
			name: "setTransform",
			static: false,
			private: false,
			access: {
				has: (obj) => "setTransform" in obj,
				get: (obj) => obj.setTransform
			}
		}, null, _instanceExtraInitializers);
	})(), _a;
})();
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/observable-operators/debounceTimeAfter.js
	...
	return (source) => {
		return new Observable((subscriber) => {
			...
			return source.pipe(tap((value) => {
				iterationCount++;
				if (iterationCount <= amount) subscriber.next({
					type: "initial",
					value
				});
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/media-sources/MediaStreamSource.js
var defaultOptions$1 = Object.assign(Object.assign({}, defaultDeviceInfo), {
...
function handleAudioProcessingErrors(errors, reportError) {
	return errors.pipe(debounceTimeAfter(1, 1e3), map((event) => {
		if (event.type === "initial") reportError(new Error("The first audio processing error before debouncing.", { cause: event.value }));
		else if (event.type === "debounced") {
			const errorMessages = [...new Set(event.values.map(stringifyErrorMessage))].join("\n");
			reportError(new Error(`Debounced ${event.values.length} audio processing errors.`, { cause: new Error(errorMessages) }));
		}
	})).subscribe();
}
function createMediaStreamSource(stream, options = {}) {
	...
	const optionsWithDefaults = Object.assign(Object.assign(Object.assign({}, defaultOptions$1), copyDefinedProperties(options)), { cameraType: (_a = options.cameraType) !== null && _a !== void 0 ? _a : detectedCameraType });
	...
	if (enableSourceAudio) {
		const microphoneRecorderBlob = new Blob([`
        ...
        registerProcessor('microphone-worklet', MicrophoneWorkletProcessor);`], { type: "application/javascript" });
	...
	return new CameraKitSource({ media: stream }, {
		onAttach: (source, lensCore, reportError) => __awaiter(this, void 0, void 0, function* () {
			...
			if (enableSourceAudio) {
				...
				yield lensCore.setAudioParameters({ parameters: {
					numChannels: 2,
					sampleRate
				} });
				try {
					closeWorklet(worklet);
					audioSource === null || audioSource === void 0 || audioSource.disconnect();
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/media-sources/VideoSource.js
var defaultOptions = Object.assign(Object.assign({}, defaultDeviceInfo), { trackingData: /* @__PURE__ */ new ArrayBuffer(0) });
function createVideoSource(video, options = {}) {
	...
	return new CameraKitSource({
		media: video,
		replayTrackingData: trackingData.byteLength > 0 ? { buffer: trackingData } : void 0
	}, {}, options);
}
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/logger/logEntries.js
var logEntriesFactory = Injectable("logEntries", () => resetLogger().asObservable());
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/session/LensPerformanceMeasurement.js
...
var LensPerformanceMeasurement = class {
	...
	computeRunningStats(processingTimeMs) {
		...
		if (this.priorFrameCompletedTime === void 0) this.priorFrameCompletedTime = performance.now();
		else {
			const frameDurationSec = (performance.now() - this.priorFrameCompletedTime) / 1e3;
			if (frameDurationSec < frameDurationThreshold) this.state.avgFps = (this.state.avgFps + 1 / frameDurationSec) / 2;
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/session/LensPerformanceMetrics.js
var logger$19 = getLogger("RenderingMetrics");
	...
	return _a = class LensPerformanceMetrics {
		constructor(frameEvents) {
			...
			this.subscription = frameEvents.subscribe({
				next: (frameEvent) => {
					try {
						for (const measurement of this.measurementInstances.values()) measurement.update(frameEvent.processingTimeMs);
					} catch (error) {
						logger$19.error(error);
					}
				},
				error: (error) => {
					logger$19.error(error);
		...
		_beginMeasurement_decorators = [errorLoggingDecorator(logger$19)];
		__esDecorate(_a, null, _beginMeasurement_decorators, {
			kind: "method",
			name: "beginMeasurement",
			static: false,
			private: false,
			access: {
				has: (obj) => "beginMeasurement" in obj,
				get: (obj) => obj.beginMeasurement
			}
		}, null, _instanceExtraInitializers);
	})(), _a;
})();
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens/LensPersistenceStore.js
var logger$18 = getLogger("LensPersistenceStore");
var lensPersistenceStoreFactory = Injectable("lensPersistenceStore", [lensCoreFactory.token], (lensCore) => {
	...
	lensCore.registerSavePersistentStoreCallback((id, data) => __awaiter(void 0, void 0, void 0, function* () {
		try {
			...
			logger$18.error(persistentStoreError(`Error occurred while storing data for lens ${id}.`, error));
		}
	}));
	return db;
});
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/common/localization.js
...
var legalPromptMessage = makeTemplate([
	"privacyPolicyUrl",
	"termsOfServiceUrl",
	"learnMoreUrl"
]);
...
function getSupportedLocale() {
	...
	if (langCode && langCode in allStrings) return langCode;
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/common/dialog.js
var stylesCss = `
dialog {
    display: flex;
...
function showDialog(options) {
	return new Promise((res) => {
		...
		prompt.innerHTML = `
			...
			key: "dismiss",
			text: (_b = options.dismissButtonText) !== null && _b !== void 0 ? _b : "Dismiss"
		})}
            ${getTitleHtml(options.title)}
            ${getBodyHtml(options.body)}
            ${getButtonsHtml((_c = options.buttons) !== null && _c !== void 0 ? _c : [])}
        `;
		merge(...Array.from(prompt.querySelectorAll("button")).map((b) => fromEvent(b, "click").pipe(map(() => b.dataset.key))), fromEvent(prompt, "cancel").pipe(map(() => "dismiss"))).pipe(take(1)).subscribe({
			next: res,
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/common/hash.js
var computeHash = (str) => {
	let h1 = 3735928559;
	let h2 = 1103547991;
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/legal/legalPrompt.js
var snapIcon = `
...
var legalPromptFactory = Injectable("legalPrompt", () => {
	return function legalPrompt(privacyPolicy, termsOfService, learnMore, childrenProtectionActRestricted) {
		...
		return {
			...
			show() {
				return __awaiter(this, void 0, void 0, function* () {
					if (!childrenProtectionActRestricted) return showTosDialog(legalTitle, legalTitleText, legalMessage);
					while (true) {
						...
						switch (adultOrChild) {
							case "child": {
								...
								switch (findGuardian) {
									...
									case "guardian": return showTosDialog(legalTitle, legalTitleText, legalMessage);
									case "dismiss": return false;
									default: assertUnreachable(findGuardian);
								}
							}
							case "adult": return showTosDialog(legalTitle, legalTitleText, legalMessage);
							case "dismiss": return false;
							default: assertUnreachable(adultOrChild);
						}
					}
				});
			}
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/legal/legalState.js
var logger$17 = getLogger("LegalState");
var tosContentHashExpiry = 720 * 60;
...
var createLegalState = () => {
	...
	return new StateMachine(defineActions(defineAction("requestLegalPrompt")(), defineAction("accept")(), defineAction("reject")()), states, states.unknown(), (actions) => {
...
var legalStateFactory = Injectable("legalState", [remoteConfigurationFactory.token, legalPromptFactory.token], (remoteConfig, legalPrompt) => {
	...
	const getLastAcceptedTosContentHash = () => from(persistance.retrieve(tosContentHashKey).catch((error) => logger$17.warn(error)));
	const setLastAcceptedTosContentHash = (hash) => persistance.store(tosContentHashKey, hash).catch((error) => logger$17.warn(error));
	...
	legalState.events.pipe(inStates("unknown"), forActions("requestLegalPrompt"), switchMap(() => forkJoin({
		...
		var _a;
		if ((_a = initConfig.legalPrompt) === null || _a === void 0 ? void 0 : _a.disabled) return of(legalState.actions.accept("disabled"));
		if (cofConfig.disabled) return of(legalState.actions.accept("disabled"));
	...
	}), dispatch(legalState)).subscribe({ error: logger$17.error });
	return legalState;
});
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/observable-operators/unsubscribed.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens/fetchWatermarkLens.js
var logger$16 = getLogger("fetchWatermarkLens");
...
var fetchWatermarkLens = Injectable("fetchWatermarkLens", [remoteConfigurationFactory.token, lensRepositoryFactory.token], (remoteConfiguration, lensRepository) => {
	remoteConfiguration.getInitializationConfig().pipe(take(1)).subscribe({
		...
		error: logger$16.error
	});
});
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens/userDataAccessResolver.js
var userDataAccessResolverFactory = Injectable("userDataAccessResolver", () => () => "unrestricted");
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/session/lensState.js
var logger$15 = getLogger("LensState");
var createLensState = () => {
...
var lensStateFactory = Injectable("lensState", [
	...
	lensState.events.pipe(forActions("applyLens"), exhaustMap(([a]) => of(legalState.actions.requestLegalPrompt()).pipe(dispatch(legalState), inStates("accepted", "rejected"), take(1), map(([, { name }]) => {
		...
		return forkJoin({
			watermarkInput: remoteConfig.getInitializationConfig().pipe(mergeMap((config) => {
				if (!config.watermarkEnabled) return of(void 0);
				return from(lensRepository.loadLens("", watermarksLensGroup)).pipe(mergeMap((watermark) => from(lensRepository.getLensContent(watermark)).pipe(map(({ lensBuffer, lensChecksum }) => {
					return {
						...
						lensChecksum,
						launchData: /* @__PURE__ */ new ArrayBuffer(0)
					};
				}))));
			})),
			lensInput: of(a.data).pipe(mergeMap(({ lens, launchData }) => {
				return from(lensPersistence.retrieve(lens.id).catch(() => void 0)).pipe(map((persistentStore) => ({
					lens,
					launchData,
					persistentStore
				})));
				...
				if (!lensDetails) throw new Error(`Cannot apply lens ${lens.id}. It has not been loaded by the Lens repository. Use CameraKit.lensRepository.loadLens (or loadLensGroups) to load lens metadata before calling CameraKitSession.applyLens.`);
				const { content, isThirdParty } = lensDetails;
				if (!content) throw new Error(`Cannot apply lens ${lens.id}. Metadata retrieved for this lens does not include the lens content URL.`);
				return {
					lens,
					launchData: encodeLensLaunchData(launchData !== null && launchData !== void 0 ? launchData : {}, persistentStore !== null && persistentStore !== void 0 ? persistentStore : /* @__PURE__ */ new ArrayBuffer(0)),
					content,
					isThirdParty
				};
				...
				return from(Promise.all([
					...
					isThirdParty ? getUserDataAccess(lens) : void 0,
					content.assetManifest.length > 0 ? lensAssetRepository.cacheAssets(content.assetManifest, lens).finally(() => networkTimer.measure("assets")) : Promise.resolve()
				])).pipe(tap(() => {
			...
			lensCore.replaceLenses({ lenses: [Object.assign(Object.assign({}, lensInput), {
			...
			}), ...watermarkInput ? [watermarkInput] : []] }).then(() => {
				...
				subscriber.next(lensState.actions.applyLensFailed({
					error,
					lens
				}));
				subscriber.complete();
			...
			applyTimer.measure("failure");
			applyTimer.stopAndReport(metrics);
			return of(lensState.actions.applyLensFailed({
				error,
				lens
			}));
		}), unsubscribed(() => {
	...
	}), dispatch(lensState)).subscribe({ error: logger$15.error });
	...
	lensState.events.pipe(inStates("applyingLens"), forActions("removeLens"), switchMap(([a]) => lensState.events.pipe(inStates("lensApplied"), takeUntil(lensState.events.pipe(forActions("applyLens"))), map(() => a))), dispatch(lensState)).subscribe({ error: logger$15.error });
	lensState.events.subscribe(([a, s]) => {
		const data = extractLoggableData(a);
		logger$15.debug(`Action: "${a.name}", state: "${s.name}"${data ? ", data: " + JSON.stringify(data) : ""}`);
	});
	return lensState;
});
function extractLoggableData(action) {
	switch (action.name) {
		case "applyLens": return { lensId: action.data.lens.id };
		case "applyLensFailed": return {
			lensId: action.data.lens.id,
		...
		case "turnedOff": return { lensId: action.data.id };
		case "removeLens":
		case "removeLensComplete": return;
		case "removeLensFailed": return { error: action.data.message };
		default: assertUnreachable(action);
	}
}
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/session/sessionState.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/session/LensKeyboard.js
var logger$14 = getLogger("LensKeyboard");
		...
		this.uriHandler = {
			...
			handleRequest: (request, reply) => {
				if (request.uri === "app://textInput/requestKeyboard") {
					...
					this.notifyClient();
				} else logger$14.error(/* @__PURE__ */ new Error(`Unhandled lens keyboard request '${request.uri}'.`));
			}
		};
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/session/CameraKitSessionEvents.js
...
function isPublicLensError(value) {
	...
	switch (maybeSessionErrorName) {
		...
		case "LensImagePickerError": return true;
		default: return isReachable(maybeSessionErrorName);
	}
}
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/session/frameEvents.js
var logger$13 = getLogger("FrameEvents");
var frameEventsFactory = Injectable("frameEvents", [lensCoreFactory.token], (lensCore) => {
	const frameEventsSubject = new Subject();
	lensCore.setOnFrameProcessedCallback({ onFrameProcessed: (frameEvent) => {
		frameEventsSubject.next(frameEvent);
	} }).catch((error) => logger$13.error(`Failed registering setOnFrameProcessedCallback with error: ${error.message}`));
	return frameEventsSubject.asObservable();
});
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/session/screenRegions.js
var REGION_TYPE_MAPPING = {
...
function prepareScreenRegionsUpdate(newRegions) {
	...
	for (const [regionType, region] of Object.entries(newRegions)) {
		...
		mappedRegions[lensCoreRegionType] = { normalizedRect: {
			origin: {
				x: region.x,
				y: region.y
			},
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/session/CameraKitSession.js
var logger$12 = getLogger("CameraKitSession");
	...
	return _a = class CameraKitSession {
		constructor(innerKeyboard, lensCore, sessionState, lensState, logEntries, pageVisibility, frameEvents) {
			...
			this.subscriptions = [
				...
				logEntries.pipe(filter((entry) => entry.level === "error"), map((entry) => entry.messages.find((e) => e instanceof Error)), filter(isPublicLensError)).subscribe((error) => {
					const state = lensState.getState();
					if (!isState(state, "noLensApplied")) this.events.dispatchEvent(new TypedCustomEvent("error", {
						error,
						lens: state.data
					}));
					else logger$12.warn("Lens error occurred even though there is no active lens.", error);
				})
			];
		}
		applyLens(lens, launchData) {
			return __awaiter(this, void 0, void 0, function* () {
				const action = this.lensState.actions.applyLens({
					lens,
					launchData
				});
		...
		setSource(source, options = {}) {
			return __awaiter(this, void 0, void 0, function* () {
				...
				yield cameraKitSource.attach(this.lensCore, this.frameEvents, (error) => {
					logger$12.error(cameraKitSourceError("Error occurred during source attachment.", error));
				});
				this.source = cameraKitSource;
				if (priorPlayingState.live) yield this.play("live");
				if (priorPlayingState.capture) yield this.play("capture");
		...
		setScreenRegions(regions) {
			return __awaiter(this, void 0, void 0, function* () {
				yield this.lensCore.setScreenRegions({ regions: prepareScreenRegionsUpdate(regions) });
			});
		}
		destroy() {
			return __awaiter(this, void 0, void 0, function* () {
				try {
					...
					logger$12.warn("An error occurred in LensCore during the session termination process.", error);
				}
			...
			return __awaiter(this, void 0, void 0, function* () {
				if (this.source) try {
					yield this.source.detach((error) => {
						logger$12.error(cameraKitSourceError("Error occurred during source detachment.", error));
					});
				} catch (error) {
					logger$12.error(cameraKitSourceError(`Detaching prior source of type ${getTypeName(this.source)} failed.`, error));
				}
			});
		}
	}, (() => {
		_applyLens_decorators = [validate(isLens, isLensLaunchDataOrUndefined), errorLoggingDecorator(logger$12)];
		_removeLens_decorators = [errorLoggingDecorator(logger$12)];
		_play_decorators = [validate(isRenderTargetOrUndefined), errorLoggingDecorator(logger$12)];
		_pause_decorators = [validate(isRenderTargetOrUndefined), errorLoggingDecorator(logger$12)];
		_mute_decorators = [errorLoggingDecorator(logger$12)];
		_unmute_decorators = [errorLoggingDecorator(logger$12)];
		_setSource_decorators = [validate(isAllowedSource, isPartialCameraKitDeviceOptionsOrUndefined), errorLoggingDecorator(logger$12)];
		_setFPSLimit_decorators = [validate(isValidNumber), errorLoggingDecorator(logger$12)];
		_setScreenRegions_decorators = [validate(isScreenRegions), errorLoggingDecorator(logger$12)];
		_destroy_decorators = [errorLoggingDecorator(logger$12)];
		__esDecorate(_a, null, _applyLens_decorators, {
			kind: "method",
			name: "applyLens",
			static: false,
			private: false,
			access: {
				has: (obj) => "applyLens" in obj,
				get: (obj) => obj.applyLens
			}
		}, null, _instanceExtraInitializers);
		__esDecorate(_a, null, _removeLens_decorators, {
			kind: "method",
			name: "removeLens",
			static: false,
			private: false,
			access: {
				has: (obj) => "removeLens" in obj,
				get: (obj) => obj.removeLens
			}
		}, null, _instanceExtraInitializers);
		...
		__esDecorate(_a, null, _pause_decorators, {
			kind: "method",
			name: "pause",
			static: false,
			private: false,
		...
		__esDecorate(_a, null, _mute_decorators, {
			kind: "method",
			name: "mute",
			static: false,
			private: false,
		...
		__esDecorate(_a, null, _unmute_decorators, {
			kind: "method",
			name: "unmute",
			static: false,
			private: false,
			access: {
				has: (obj) => "unmute" in obj,
				get: (obj) => obj.unmute
			}
		}, null, _instanceExtraInitializers);
		__esDecorate(_a, null, _setSource_decorators, {
			kind: "method",
			name: "setSource",
			static: false,
			private: false,
			access: {
				has: (obj) => "setSource" in obj,
				get: (obj) => obj.setSource
			}
		}, null, _instanceExtraInitializers);
		__esDecorate(_a, null, _setFPSLimit_decorators, {
			kind: "method",
			name: "setFPSLimit",
			static: false,
			private: false,
			access: {
				has: (obj) => "setFPSLimit" in obj,
				get: (obj) => obj.setFPSLimit
			}
		}, null, _instanceExtraInitializers);
		__esDecorate(_a, null, _setScreenRegions_decorators, {
			kind: "method",
			name: "setScreenRegions",
			static: false,
			private: false,
			access: {
				has: (obj) => "setScreenRegions" in obj,
				get: (obj) => obj.setScreenRegions
			}
		}, null, _instanceExtraInitializers);
		__esDecorate(_a, null, _destroy_decorators, {
			kind: "method",
			name: "destroy",
			static: false,
			private: false,
			access: {
				has: (obj) => "destroy" in obj,
				get: (obj) => obj.destroy
			}
		}, null, _instanceExtraInitializers);
	})(), _a;
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens/assets/LensAssetsProvider.js
var logger$11 = getLogger("LensAssetProvider");
...
var registerLensAssetsProvider = Injectable("registerLensAssetsProvider", [
	...
	lensCore.setRemoteAssetsProvider((assetDescriptor) => __awaiter(void 0, void 0, void 0, function* () {
		...
		try {
			if (((_a = consecutiveErrorsPerAsset.get(assetId)) !== null && _a !== void 0 ? _a : 0) > maxConsecutiveErrors) throw new Error(`Maximum consecutive asset load errors reached for asset ${assetId}`);
			const calculatedLensId = lensId !== null && lensId !== void 0 ? lensId : (_b = lensState.getState().data) === null || _b === void 0 ? void 0 : _b.id;
			const lens = calculatedLensId ? lensRepository.getLensMetadata(calculatedLensId) : void 0;
			...
			if (consecutiveErrors <= maxConsecutiveErrors) logger$11.error(lensAssetError(`Error occurred while handling lens asset ${assetId} request.`, error));
			else logger$11.warn(`Maximum consecutive asset load errors reached for asset ${assetId}`);
		}
	}));
});
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/camera_kit/v3/remote_api_spec.js
...
var GetRemoteApiSpecsRequest = {
	...
	fromPartial(_) {
		return createBaseGetRemoteApiSpecsRequest();
	}
};
...
var GetRemoteApiSpecsResponse = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.remoteApiSpecs) RemoteApiSpec.encode(v, writer.uint32(10).fork()).join();
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var RemoteApiSpec = {
	encode(message, writer = new BinaryWriter()) {
		if (message.id !== "") writer.uint32(10).string(message.id);
		if (message.host !== "") writer.uint32(18).string(message.host);
		for (const v of message.endpoints) RemoteEndpoint.encode(v, writer.uint32(26).fork()).join();
		if (message.tlsRequired !== false) writer.uint32(32).bool(message.tlsRequired);
		if (message.maxRequestSizeBytes !== 0) writer.uint32(40).int32(message.maxRequestSizeBytes);
		if (message.maxResponseSizeBytes !== 0) writer.uint32(48).int32(message.maxResponseSizeBytes);
		if (message.maxResponseTimeMillis !== 0) writer.uint32(56).int32(message.maxResponseTimeMillis);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
var RemoteEndpoint = {
	encode(message, writer = new BinaryWriter()) {
		...
		writer.uint32(18).fork();
		for (const v of message.methods) writer.int32(v);
		writer.join();
		for (const v of message.parameters) RemoteParameter.encode(v, writer.uint32(26).fork()).join();
		if (message.refId !== "") writer.uint32(34).string(message.refId);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				...
				case 2:
					...
					if (tag === 18) {
						const end2 = reader.uint32() + reader.pos;
						while (reader.pos < end2) message.methods.push(reader.int32());
						continue;
					}
					break;
				case 3:
...
var RemoteParameter = {
	encode(message, writer = new BinaryWriter()) {
		if (message.name !== "") writer.uint32(10).string(message.name);
		if (message.location !== 0) writer.uint32(16).int32(message.location);
		if (message.optional !== false) writer.uint32(24).bool(message.optional);
		if (message.externalName !== "") writer.uint32(34).string(message.externalName);
		if (message.defaultValue !== "") writer.uint32(42).string(message.defaultValue);
		if (message.constant !== false) writer.uint32(48).bool(message.constant);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/clients/remoteApiSpecsClient.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/uri-handlers/uriRequestProcessor.js
function callCancellationHandler(cancellationHandlers, ...keys) {
	var _a;
	for (const key of keys) {
		(_a = cancellationHandlers.get(key)) === null || _a === void 0 || _a();
...
function createUriRequestProcessor({ uri, lensState, sessionState, createLensRequestState, processRequest, processInternalError }) {
	...
	merge(lensState.events.pipe(forActions("turnedOff"), tap(([action]) => handleLensApplicationEnd(lensRequestState, action.data.id))), uriRequests.pipe(map(({ request, lens, reply }) => {
	...
	}), tap(processRequest)), uriCancelRequests.pipe(tap((uriRequest) => {
		var _a;
		const cancellationHandlers = (_a = lensRequestState.get(uriRequest.lens.id)) === null || _a === void 0 ? void 0 : _a.cancellationHandlers;
		if (cancellationHandlers) callCancellationHandler(cancellationHandlers, uriRequest.request.requestId);
	}))).pipe(catchError((error, sourcePipe) => {
		processInternalError(error);
		return sourcePipe;
	}), retry(), takeUntil(sessionState.events.pipe(forActions("destroy")))).subscribe({ complete: () => handleLensApplicationEnd(lensRequestState, ...lensRequestState.keys()) });
	return {
		uri,
...
var requestValidationError = namedError("RequestValidationError");
function validateRequest(request, specs) {
	var _a;
	const url = new URL(request.uri);
	for (const spec of specs) {
		if (url.host !== spec.host || url.protocol !== (spec.tlsRequired ? "https:" : "http:")) continue;
...
function validatePath(path, parameters) {
	...
	for (const param of parameters) {
		...
		if (param.constant) if (paramNameComponent === param.name && paramValueComponent === param.defaultValue) paramIndex += 2;
		else throw requestValidationError(`Expected constant parameter '${param.name}' with value '${param.defaultValue}' at position ${paramIndex}, but found '${paramNameComponent}' and '${paramValueComponent}'.`);
		else if (param.optional) {
			if (paramNameComponent === param.name) if (paramValueComponent !== void 0) paramIndex += 2;
			else paramIndex += 1;
		} else if (paramNameComponent === param.name && paramValueComponent !== void 0) paramIndex += 2;
		else throw requestValidationError(`Expected parameter '${param.name}' with a value at position ${paramIndex}, but found '${paramNameComponent}' and '${paramValueComponent}'.`);
	}
...
function validateHeaders(headers, parameters) {
	for (const param of parameters) {
		...
		if (param.constant) {
			if (headerValue !== param.defaultValue) throw requestValidationError(`Expected constant header '${param.name}' with value '${param.defaultValue}', but found '${headerValue !== null && headerValue !== void 0 ? headerValue : "undefined"}'.`);
		} else if (param.optional) {} else if (headerValue == void 0 || isEmptyString(headerValue)) throw requestValidationError(`Required header '${param.name}' is missing or empty.`);
	}
}
function validateQuery(queryParams, parameters) {
	for (const param of parameters) {
		...
		if (param.constant) {
			if (paramValue !== param.defaultValue) throw requestValidationError(`Expected constant query parameter '${param.name}' with value '${param.defaultValue}', but found '${paramValue !== null && paramValue !== void 0 ? paramValue : "undefined"}'.`);
		} else if (param.optional) {} else if (paramValue == void 0 || isEmptyString(paramValue)) throw requestValidationError(`Required query parameter '${param.name}' is missing or empty.`);
	}
}
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/uri-handlers/internal-handlers/httpUriHandler.js
var logger$10 = getLogger("lensHttpHandler");
	...
	return createUriRequestProcessor({
		...
		processRequest({ request, lens, reply, setCancellationHandler }) {
			return __awaiter(this, void 0, void 0, function* () {
				...
				handleHttpUriRequest(request, lens, reply, setCancellationHandler, yield allowlistPromise, customLentFetchHandler);
			});
		},
		processInternalError(error) {
			logger$10.error(error);
		}
	});
}
		...
		try {
			...
			if (customLensHttpHandler) {
				...
				try {
					...
					if (!isPromise(responsePromise)) throw lensHttpHandlerError(`The '${handlerName}' callback provided to Camera Kit must return a Promise.`);
...
function getErrorResponse(errorType, message) {
	return {
		...
		metadata: { "x-camera-kit-error-type": errorType },
		data: new TextEncoder().encode(message)
	};
}
function mapLensToFetchRequest({ uri, method, metadata, data }) {
	const headers = new Headers(metadata);
	for (const header of requestHeadersToExclude) headers.delete(header);
	return {
		url: uri,
		init: {
...
function mapFetchToLensResponse(response) {
	...
	return __awaiter(this, void 0, void 0, function* () {
		...
		for (const allowedHeader of allowedResponseHeaders) {
			...
			if (value) metadata[allowedHeader.toLowerCase()] = value;
		}
		return {
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/uri-handlers/UriHandlers.js
var SEPARATOR = "://";
function extractSchemeAndRoute(uri) {
	const separatorIndex = uri.indexOf(SEPARATOR);
	return {
		scheme: uri.slice(0, separatorIndex),
		route: uri.slice(separatorIndex + 3)
	};
}
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/camera_kit/v3/features/remote_api_info.js
function createBaseRemoteApiInfo() {
	return { apiSpecIds: [] };
}
var RemoteApiInfo = {
	encode(message, writer = new BinaryWriter()) {
		for (const v of message.apiSpecIds) writer.uint32(10).string(v);
		return writer;
	},
	decode(input, length) {
		const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/generated-proto/pb_schema/lenses/remote_api/remote_api_service.js
...
function responseCodeToNumber(object) {
	switch (object) {
		case ResponseCode.RESPONSE_CODE_UNSET: return 0;
		case ResponseCode.SUCCESS: return 1;
		case ResponseCode.REDIRECTED: return 2;
		case ResponseCode.BAD_REQUEST: return 3;
		case ResponseCode.ACCESS_DENIED: return 4;
		case ResponseCode.NOT_FOUND: return 5;
		case ResponseCode.TIMEOUT: return 6;
		case ResponseCode.REQUEST_TOO_LARGE: return 7;
		case ResponseCode.SERVER_ERROR: return 8;
		case ResponseCode.CANCELLED: return 9;
		case ResponseCode.PROXY_ERROR: return 10;
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/uri-handlers/internal-handlers/remoteApiUriHandler.js
var logger$9 = getLogger("RemoteApiServices");
...
function getFirstRequestHandler(services, remoteApiRequest, lens) {
	for (const service of services) try {
		...
		logger$9.warn("Client's Remote API request handler factory threw an error.", error);
	}
}
function createRemoteApiUriHandler(registeredServices, sessionState, lensState, lensRepository, metrics) {
	...
	return createUriRequestProcessor({
		...
		createLensRequestState(_request, lens) {
			var _a, _b;
			return { supportedSpecIds: new Set([...((_b = (_a = lensRepository.getLensMetadata(lens.id)) === null || _a === void 0 ? void 0 : _a.featureMetadata) !== null && _b !== void 0 ? _b : []).filter((feature) => feature.typeUrl === remoteApiInfoProtobufTypeUrl).flatMap((any) => RemoteApiInfo.decode(any.value).apiSpecIds), remoteApiUploadImageSpecId]) };
		},
		processRequest({ request, reply, lens, setCancellationHandler, state }) {
			...
			const reportSingleCount = (name) => {
				metrics.setOperationalMetrics(Count.count(joinMetricNames([
					"lens",
					"remote-api",
					name
				]), 1, dimensions));
			...
			if (!(state === null || state === void 0 ? void 0 : state.supportedSpecIds.has(specId))) return;
			...
			try {
				...
				logger$9.warn("Client's Remote API request handler threw an error.", error);
			}
			if (typeof cancellationHandler === "function") setCancellationHandler(() => {
				try {
					cancellationHandler();
				} catch (error) {
					logger$9.warn("Client's Remote API request cancellation handler threw an error.", error);
				}
			});
		},
		processInternalError(error) {
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/uri-handlers/uriHandlersRegister.js
var logger$8 = getLogger("uriHandlersRegister");
var registerUriHandlers = Injectable("registerUriHandlers", [
	...
	if (!isUriHandlers(userHandlers)) throw new Error("Expected an array of UriHandler objects");
	const allHandlers = [
	...
	for (const { uri, handleRequest, cancelRequest } of allHandlers) {
		...
		for (const { scheme, route } of uris.map(extractSchemeAndRoute)) lensCore.registerUriListener(scheme, route, {
			handleRequest: (request) => {
				const reply = (response) => {
					if (!isUriResponse(response)) throw new Error("Expected UriResponse object");
					lensCore.provideUriResponse(request.identifier, response);
				};
				const state = lensState.getState();
				if (isState(state, "noLensApplied")) {
					logger$8.warn(`Got a URI request for ${request.uri}, but there is no active lens. The request will not be processed.`);
					return;
				}
			...
			cancelRequest: (request) => {
				if (cancelRequest) {
					const state = lensState.getState();
					if (isState(state, "noLensApplied")) {
						logger$8.warn(`Got a URI cancel request for ${request.uri}, but there is no active lens. The cancel request will not be processed.`);
						return;
					}
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/reporters/reportGlobalException.js
var logMethods = entries(logLevelMap).map(([level]) => level);
var maxBufferedEntries = 15;
var contextSeparator = "\n\n----------------- Context -----------------\n\n";
var methodLength = logMethods.reduce((max, method) => Math.max(max, method.length), 0);
...
function reportExceptionToBlizzard(logEntries, metricsEventTarget, metrics, lensState) {
	logEntries.pipe(scan$1(({ entries }, newEntry) => {
		const lastEntry = entries[entries.length - 1];
		if (lastEntry && lastEntry.entry.messages.join() === newEntry.messages.join() && lastEntry.entry.level === newEntry.level) {
			lastEntry.count += 1;
	...
	}), filter(({ recent }) => recent.level === "error"), map(({ entries, recent }) => ({
		context: entries,
		error: recent.messages.find((e) => e instanceof Error)
	})), filter(({ error }) => !!error)).subscribe(({ error, context }) => {
...
var reportGlobalException = Injectable("reportGlobalException", [
	logEntriesFactory.token,
	metricsEventTargetFactory.token,
	metricsClientFactory.token
], (logEntries, metricsEventTarget, metrics) => {
	...
	return { attachLensContext: (lensState) => {
		cancellationSubject.next();
		reportExceptionToBlizzard(logEntries, metricsEventTarget, metrics, lensState);
	} };
});
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/reporters/reportSessionException.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/operational/Histogram.js
var Histogram = class Histogram extends Metric {
	...
	toOperationalMetric() {
		...
		return this.levels.map((level) => ({
			...
			metric: {
				$case: "histogram",
				histogram: `${Math.ceil(level)}`
			}
		}));
	}
};
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/reporters/reportBenchmarks.js
var reportableBenchmarks = ["gflops"];
var reportBenchmarks = Injectable("reportBenchmarks", [
	metricsEventTargetFactory.token,
	metricsClientFactory.token,
	configurationToken
], (metricsEventTarget, metrics, config) => __awaiter(void 0, void 0, void 0, function* () {
	if (config.lensPerformance === void 0) return;
	...
	for (const benchmark of lensPerformance.benchmarks) {
		...
		const benchmarkComplete = Object.assign(Object.assign({}, baseBenchmark), {
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/events/scan.js
var scan = (seedState) => (source, eventTypes, accumulator) => {
	let state = seedState;
	...
	sink.addEventListener = new Proxy(sink.addEventListener, { apply: (target, thisArg, args) => {
	...
	sink.removeEventListener = new Proxy(sink.removeEventListener, { apply: (target, thisArg, args) => {
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/reporters/reportHttpMetrics.js
...
var getAdditionalDimensions = (dimensions) => {
	switch (dimensions.requestType) {
		case "cof": return [["delta", dimensions.delta]];
		case GRPC_CALL_REQUEST_TYPE: return [["method", dimensions.methodName]];
		case LENS_CORE_JS_REQUEST_TYPE:
		case LENS_CORE_WASM_REQUEST_TYPE: return [["custom", dimensions.customBuild]];
		default: return [];
	}
};
var getContentType = (dimensions) => {
	if (dimensions.requestType === "asset") return dimensions.assetType;
...
var getSizeKb = (event) => {
	switch (event.type) {
		...
		case "completed": return event.detail.sizeByte / 1024;
...
var getStatus = (event) => {
	switch (event.type) {
		case "started":
		case "errored": return "0";
		case "completed": return event.detail.status.toString();
		default: assertUnreachable(event);
	}
};
	...
	scan({
		...
		switch (event.type) {
			...
			case "errored":
				...
				for (const [key, value] of getAdditionalDimensions(dimensions)) operationalDimensions[key] = value;
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/reporters/reportLegalState.js
var reportLegalState = Injectable("reportLegalState", [
	legalStateFactory.token,
	metricsEventTargetFactory.token,
	metricsClientFactory.token
], (legalState, metricsEventTarget, metrics) => {
	...
	}))).subscribe({ next: (legalPromptEvent) => {
		metricsEventTarget.dispatchEvent(new TypedCustomEvent("legalPrompt", legalPromptEvent));
		metrics.setOperationalMetrics(Count.count("legal_prompt_interaction", 1, { accepted: (legalPromptEvent.legalPromptResult === CameraKitLegalPromptResult.CAMERA_KIT_LEGAL_PROMPT_ACCEPTED).toString() }));
	} });
});
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/reporters/reportLensAndAssetDownload.js
...
var reportLensAndAssetDownload = Injectable("reportLensAndAssetDownload", [
	...
	metricsClientFactory.token,
	requestStateEventTargetFactory.token
], (metricsEventTarget, metrics, requestStateEventTarget) => {
	scan({
		name: "inProgress",
		inProgress: /* @__PURE__ */ new Map()
	})(requestStateEventTarget, [
		...
		switch (event.type) {
			...
			case "completed":
				...
				switch (dimensions.requestType) {
					...
					default: assertUnreachable(dimensions);
				}
			case "errored":
				if (!inProgress.get(requestId)) return state;
				inProgress.delete(requestId);
		...
		if (state.event.detail.name === "exception") metrics.setOperationalMetrics(Count.count("handled_exception", 1, { type: state.event.detail.type }));
	});
});
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/reporters/reportLensValidationFailed.js
var reportLensValidationFailed = Injectable("reportLensValidationFailed", [lensStateFactory.token, metricsEventTargetFactory.token], (lensState, metricsEventTarget) => {
	lensState.events.pipe(forActions("applyLensFailed"), filter(([a]) => a.data.error.name === "LensContentValidationError")).subscribe({ next: ([{ data }]) => {
		const { lens } = data;
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/common/date.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/reporters/reportLensView.js
var viewTimeThresholdSec$1 = .1;
...
var reportLensView = Injectable("reportLensView", [
	...
	const lensViewPersistence = new ExpiringPersistence(() => 1440 * 60 * 60, new IndexedDBPersistence({ databaseName: "recentLensViews" }));
	merge(lensState.events.pipe(forActions("downloadComplete"), map(([a]) => a.data)), lensState.events.pipe(inStates("lensApplied"), switchMap(([, s]) => sessionState.events.pipe(forActions("resume"), takeUntil(lensState.events.pipe(forActions("removeLens"))), map(() => s.data))))).pipe(map((lens) => [
		getTimeMs(),
		lens.id,
		lens.groupId
	]), mergeMap(([applyLensStartTime, lensId, lensGroupId]) => {
		const alreadyOn = isState(lensState.getState(), "lensApplied");
		const applyDelay = alreadyOn ? of(0) : lensState.events.pipe(forActions("resourcesLoaded"), filter(([a]) => a.data.id === lensId), take(1), map(() => (getTimeMs() - applyLensStartTime) / 1e3));
		...
		return applyDelay.pipe(combineLatestWith(viewMetrics, from(isFirstTimeWithinPeriods(lensId, lensViewPersistence))), takeUntil(lensState.events.pipe(forActions("turnedOn"), filter(([a]) => a.data.id !== lensId))), take(1), map(([applyDelaySec, viewMetrics, isFirstTimeResults]) => Object.assign(Object.assign({
			applyDelaySec,
			lensId,
			lensGroupId
		}, viewMetrics), isFirstTimeResults)));
	})).subscribe({ next: ({ applyDelaySec, lensId, lensGroupId, viewTimeSec, avgFps, lensFrameProcessingTimeMsAvg, lensFrameProcessingTimeMsStd, lensFrameProcessingTimeMsMedian, lensFrameProcessingN, isLensFirstWithinDay, isLensFirstWithinMonth }) => __awaiter(void 0, void 0, void 0, function* () {
		if (viewTimeSec < viewTimeThresholdSec$1) return;
		const lensView = {
			name: "lensView",
			applyDelaySec,
			avgFps,
		...
		if (lensFrameProcessingN >= 30) metrics.setOperationalMetrics(Histogram.level("lens_view_frame-processing-time", lensFrameProcessingTimeMsMedian, { performance_cluster: performanceCluster.toString() }));
	}) });
}));
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/reporters/reportLensWait.js
var viewTimeThresholdSec = .1;
var reportLensWait = Injectable("reportLensWait", [lensStateFactory.token, metricsEventTargetFactory.token], (lensState, metricsEventTarget) => {
	lensState.events.pipe(forActions("applyLens"), mergeMap(([a]) => {
	...
	})).subscribe({ next: ([viewTimeSec, lensId, lensGroupId]) => {
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/reporters/reportUserSession.js
var reportUserSession = Injectable("reportUserSession", [metricsEventTargetFactory.token], (metricsEventTarget) => __awaiter(void 0, void 0, void 0, function* () {
	...
	const { day, month, year } = dayFormatter.formatToParts(date).reduce((acc, { type, value }) => Object.assign(Object.assign({}, acc), { [type]: parseInt(value) }), {});
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/reporters/reportPlatformCapabilities.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/reporters/reporters.js
...
//#endregion
//#region node_modules/.pnpm/browser-fs-access@0.35.0/node_modules/browser-fs-access/dist/index.modern.js
var e = (() => {
...
var d = async (e = {}) => {
	...
	if ("stream" in e) return await e.stream().pipeTo(c), s;
	...
	}, o = e[0].legacySetup && e[0].legacySetup(a, () => o(n), r), s = () => {
	...
	"body" in e && (r = await async function(e, t) {
		const n = e.getReader(), r = new ReadableStream({ start: (e) => async function t() {
			return n.read().then(({ done: n, value: r }) => {
				if (!n) return e.enqueue(r), t();
				e.close();
			});
		}() }), a = await new Response(r).blob();
		return n.releaseLock(), new Blob([a], { type: t });
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens-client-interface/exif.js
var JPEG_ID = 65496;
...
function extractJpegOrientationTag(data) {
	...
	if (view.getUint16(0, false) !== JPEG_ID) return void 0;
	const length = view.byteLength;
	let offset = 2;
	while (offset < length) {
		if (view.getUint16(offset + 2, false) <= 8) return void 0;
		let marker = view.getUint16(offset, false);
		offset += 2;
		if (marker === APP1_MARKER) {
			if (view.getUint32(offset += 2, false) !== EXIF_ID) return;
			let little = view.getUint16(offset += 6, false) === LITTLE_ENDIAN;
			offset += view.getUint32(offset + 4, little);
			let tags = view.getUint16(offset, little);
			offset += 2;
			for (let i = 0; i < tags; i++) if (view.getUint16(offset + i * 12, little) === ORIENTATION_TAG_OFFSET) return view.getUint16(offset + i * 12 + 8, little);
		} else if ((marker & 65280) !== 65280) break;
		else offset += view.getUint16(offset, false);
	}
}
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens-client-interface/imagePicker.js
var logger$7 = getLogger("lensClientInterfaceImagePicker");
var mimeTypes = {
	image: [
		"image/avif",
		"image/bmp",
...
function* enumerateSupportedVideoTypes(types) {
	...
	for (const type of types) if ((testVideoElement === null || testVideoElement === void 0 ? void 0 : testVideoElement.canPlayType(type)) || false) yield type;
}
...
function getMimeType({ ImageEnabled, VideoEnabled }) {
	...
	if (types.length === 0) throw new Error("Unknown media type requested.");
	return types;
}
function getOrientation(data, lensCore) {
	...
	try {
		...
		logger$7.info("Error occurred while reading EXIF orientation tag.", error);
...
function pickClientImage(clientInterfaceData, lensCore, filePicker) {
	return __awaiter(this, void 0, void 0, function* () {
		...
		logger$7.debug(`Opening file dialog for MIME types: ${mimeTypes}`);
		const file = yield filePicker({ mimeTypes }, n);
		if (Array.isArray(file)) throw new Error("Multiple files are not supported.");
		logger$7.debug(`Selected file MIME type: ${file.type}`);
		const data = yield readFileAsArrayBuffer(file);
		if (file.type.startsWith("image/")) lensCore.provideExternalImage({
			data,
		...
		else lensCore.provideExternalVideo({
			data,
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens-client-interface/filePicker.js
var filePickerFactory = Injectable("filePicker", () => (options, openDefaultPicker) => openDefaultPicker(options));
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens-client-interface/lensClientInterface.js
var logger$6 = getLogger("lensClientInterface");
var registerLensClientInterfaceHandler = Injectable("registerLensClientInterfaceHandler", [lensCoreFactory.token, filePickerFactory.token], (lensCore, filePicker) => {
	if (!lensCore.setClientInterfaceRequestHandler) {
		logger$6.warn("Current LensCore version doesn't support lens client interface requests");
		return;
	}
	lensCore.setClientInterfaceRequestHandler(({ data, interfaceControl, interfaceAction }) => __awaiter(void 0, void 0, void 0, function* () {
		if (interfaceControl === lensCore.InterfaceControl.ImagePicker && interfaceAction === lensCore.InterfaceAction.Show) yield pickClientImage(data, lensCore, filePicker).catch((error) => {
			logger$6.error(lensImagePickerError("Error occurred while attempting to select an image file for the lens request.", error));
		});
	}));
});
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/remote-configuration/preloadConfiguration.js
var logger$5 = getLogger("setPreloadedConfiguration");
var setPreloadedConfiguration = Injectable("setPreloadedConfiguration", [lensCoreFactory.token, remoteConfigurationFactory.token], (lensCore, remoteConfiguration) => {
	remoteConfiguration.getNamespace(Namespace.LENS_CORE_CONFIG).pipe(take(1)).subscribe({
		next: (configs) => {
			const inputs = configs.map(({ configId, value }) => ({
				configId,
				value
			}));
			lensCore.setPreloadedConfiguration(inputs);
		},
		error: logger$5.error
	});
});
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/geo/geoDataProvider.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/geo/registerGeoDataProvider.js
var logger$4 = getLogger("registerGeoDataProvider");
var registerGeoDataProvider = Injectable("registerGeoDataProvider", [lensCoreFactory.token, geoDataProviderFactory.token], (lensCore, getGeoData) => {
	if (!lensCore.setGeoDataProvider) {
		logger$4.warn("setGeoDataProvider is not defined.");
		return;
	}
	lensCore.setGeoDataProvider(() => {
		const geoData = getGeoData();
		if (geoData) {
			if (!isGeoData(geoData)) throw new Error("Expected GeoData object.");
			lensCore.provideGeoData({ geoData });
		}
	});
});
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/CameraKit.js
var logger$3 = getLogger("CameraKit");
...
var CameraKit = (() => {
	...
	return _a = class CameraKit {
		...
		createSession({ liveRenderTarget, renderWhileTabHidden } = {}) {
			return __awaiter(this, void 0, void 0, function* () {
				const exceptionHandler = (error) => {
					if (error.name === "LensCoreAbortError") logger$3.error(lensAbortError("Unrecoverable error occurred during lens execution. The CameraKitSession will be destroyed.", error));
					else logger$3.error(lensExecutionError("Error occurred during lens execution. The lens cannot be rendered and will be removed from the CameraKitSession.", error));
				};
				...
				if (this.lensCore.setGpuIndex) try {
					yield this.lensCore.setGpuIndex({ gpuIndex: yield firstValueFrom(this.remoteConfig.getGpuIndexConfig()) });
				} catch (cause) {
					logger$3.error(new Error("Cannot set GPU index.", { cause }));
				}
				if (config.fonts.length > 0) this.lensCore.setSystemFonts({ fonts: config.fonts.map((font) => Object.assign(Object.assign({}, font), { data: font.data.slice(0) })) });
				yield this.lensCore.setRenderLoopMode({ mode: renderWhileTabHidden ? this.lensCore.RenderLoopMode.SetTimeout : this.lensCore.RenderLoopMode.RequestAnimationFrame });
				const session = this.container.copy().provides(sessionStateFactory).provides(lensStateFactory).provides(lensKeyboardFactory).provides(frameEventsFactory).provides(cameraKitSessionFactory).run(registerLensAssetsProvider).run(registerLensClientInterfaceHandler).run(registerGeoDataProvider).run(setPreloadedConfiguration).run(reportSessionScopedMetrics).run(registerUriHandlers).get(cameraKitSessionFactory.token);
		...
		_createSession_decorators = [errorLoggingDecorator(logger$3)];
		_destroy_decorators = [errorLoggingDecorator(logger$3)];
		__esDecorate(_a, null, _createSession_decorators, {
			kind: "method",
			name: "createSession",
			static: false,
			private: false,
			access: {
				has: (obj) => "createSession" in obj,
				get: (obj) => obj.createSession
			}
		}, null, _instanceExtraInitializers);
		__esDecorate(_a, null, _destroy_decorators, {
			kind: "method",
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/platform/assertPlatformSupported.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/metrics/businessEventsReporter.js
var logger$2 = getLogger("BusinessEventsReporter");
var connectivityTypeMapping = {
	cellular: CameraKitConnectivityType.CAMERA_KIT_CONNECTIVITY_TYPE_MOBILE,
	bluetooth: CameraKitConnectivityType.CAMERA_KIT_CONNECTIVITY_TYPE_BLUETOOTH,
	wifi: CameraKitConnectivityType.CAMERA_KIT_CONNECTIVITY_TYPE_WIFI,
	unknown: CameraKitConnectivityType.CAMERA_KIT_CONNECTIVITY_TYPE_UNKNOWN,
	none: CameraKitConnectivityType.CAMERA_KIT_CONNECTIVITY_TYPE_UNREACHABLE
};
...
function listenAndReport(metricsEventTarget, metricsClient, eventHandlers, appVendorAndPartnerUuid) {
	const sessionId = v4();
	logger$2.info(`Session ID: ${sessionId}`);
	let sequenceId = 1;
	const makeBlizzardEvent = (event, appVendorUuid, partnerUuid) => {
		...
		const deviceConnectivity = (_a = connectivityTypeMapping[connectionType]) !== null && _a !== void 0 ? _a : CameraKitConnectivityType.CAMERA_KIT_CONNECTIVITY_TYPE_UNKNOWN;
		return Object.assign(Object.assign({}, event), { cameraKitEventBase: CameraKitEventBase.fromPartial({
			...
			cameraKitFlavor: CameraKitFlavor.CAMERA_KIT_FLAVOR_DEBUG,
			appId: origin,
			deviceConnectivity,
			sessionId,
			appVendorUuid,
	...
	merge(...entries(eventHandlers).map(([eventType, createEventData]) => fromEvent(metricsEventTarget, eventType).pipe(map((event) => ({
		event,
		createEventData
	}))))).pipe(combineLatestWith(appVendorAndPartnerUuid)).subscribe(([{ event, createEventData }, { appVendorUuid, partnerUuid }]) => {
...
function getAppVendorAndPartnerUuid(configuration, remoteConfiguration) {
	...
	return remoteConfiguration.getInitializationConfig().pipe(take(1), switchMap(({ appVendorUuidOptIn }) => {
		...
		if (appVendorUuidOptIn) return from(getOrGenerateVendorUuid(vendorAnalyticsPersistence)).pipe(map((appVendorUuid) => ({
			appVendorUuid,
			partnerUuid
		})));
		return of({
			appVendorUuid: void 0,
			partnerUuid
		});
	}), catchError((error) => {
		logger$2.warn(`Failed to retrieve or generate vendor UUID.`, error);
		return of({
			appVendorUuid: void 0,
			partnerUuid: configuration.analyticsId
		});
	}));
}
var businessEventsReporterFactory = Injectable("businessEventsReporter", [
	...
	listenAndReport(metricsEventTarget, metricsClient, {
		assetDownload: (event) => ["CAMERA_KIT_ASSET_DOWNLOAD", { cameraKitAssetDownload: CameraKitAssetDownload.fromPartial(event) }],
		assetValidationFailed: (event) => ["CAMERA_KIT_ASSET_VALIDATION_FAILED", { cameraKitAssetValidationFailed: CameraKitAssetValidationFailed.fromPartial(event) }],
		benchmarkComplete: (event) => ["CAMERA_KIT_WEB_BENCHMARK_COMPLETE", { cameraKitWebBenchmarkComplete: CameraKitWebBenchmarkComplete.fromPartial(event) }],
		exception: (event) => ["CAMERA_KIT_EXCEPTION", { cameraKitException: CameraKitException.fromPartial(event) }],
		legalPrompt: (event) => ["CAMERA_KIT_LEGAL_PROMPT", { cameraKitLegalPrompt: CameraKitLegalPrompt.fromPartial(event) }],
		lensDownload: (event) => ["CAMERA_KIT_LENS_DOWNLOAD", { cameraKitLensDownload: CameraKitLensDownload.fromPartial(event) }],
		lensView: (event) => ["CAMERA_KIT_WEB_LENS_SWIPE", { cameraKitWebLensSwipe: CameraKitWebLensSwipe.fromPartial(event) }],
		lensWait: (event) => ["CAMERA_KIT_LENS_SPIN", { cameraKitLensSpin: CameraKitLensSpin.fromPartial(event) }],
		lensContentValidationFailed: (event) => ["CAMERA_KIT_LENS_CONTENT_VALIDATION_FAILED", { cameraKitLensContentValidationFailed: CameraKitLensContentValidationFailed.fromPartial(event) }],
		session: (event) => ["CAMERA_KIT_SESSION", { cameraKitSession: CameraKitSession$1.fromPartial(event) }]
	}, getAppVendorAndPartnerUuid(configuration, remoteConfiguration));
});
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/logger/registerLogEntriesSubscriber.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/clients/lensesClient.js
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/lens/cameraKitLensSource.js
var logger$1 = getLogger("CameraKitLensSource");
var hasAnyValue = (c) => {
	var _a, _b;
	return ((_b = (_a = c.value) === null || _a === void 0 ? void 0 : _a.anyValue) === null || _b === void 0 ? void 0 : _b.value) instanceof Uint8Array;
};
...
var cameraKitLensSourceFactory = Injectable(lensSourcesFactory.token, [
	lensSourcesFactory.token,
	lensesClientFactory.token,
	remoteConfigurationFactory.token
], (lensSources, lensesClient, remoteConfig) => [
	{
	...
	...lensSources,
	{
		...
		loadLens(lensId, groupId) {
			var _a;
			return __awaiter(this, void 0, void 0, function* () {
				const result = yield lensesClient.getGroupLens({
					lensId,
					groupId
				});
				if (!result.ok) {
				...
				const response = result.unwrap();
				if (!((_a = response.message) === null || _a === void 0 ? void 0 : _a.lens)) throw new Error(`Cannot load lens ${lensId} from group ${groupId}. The response did not contain a lens.\n\t${JSON.stringify(result)} for requestId ${response.headers.get("x-request-id")}`);
		...
		loadLensGroup(groupId) {
			...
			return __awaiter(this, void 0, void 0, function* () {
				...
				const response = result.unwrap();
				if (!((_a = response.message) === null || _a === void 0 ? void 0 : _a.lenses)) throw new Error(`Cannot load lens group ${groupId}. The response contained no lenses \n\t${JSON.stringify(response)} for requestId ${response.headers.get("x-request-id")}`);
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/bootstrapCameraKit.js
var logger = getLogger("bootstrapCameraKit");
var nonWrappableErrors = ["ConfigurationError", "PlatformNotSupportedError"];
function shouldWrapError(error) {
	if (error instanceof Error) return !nonWrappableErrors.some((name) => error.name === name);
	return true;
}
function bootstrapCameraKit(configuration, provide) {
	return __awaiter(this, void 0, void 0, function* () {
		...
		try {
			...
			if (shouldWrapError(error)) error = bootstrapError("Error occurred during Camera Kit bootstrapping.", error);
			logger.error(error);
			throw error;
		}
	});
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/extensions/extensionRequestContext.js
var getExtensionRequestContext = memoize(function getExtensionRequestContext() {
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/media-sources/FunctionSource.js
var getYUVImageData = (canvas, lensCore) => __awaiter(void 0, void 0, void 0, function* () {
	const { width, height } = canvas;
	const outputBuffer = /* @__PURE__ */ new ArrayBuffer(width * height * 3 / 2);
	yield lensCore.imageToYuvBuffer({
		image: canvas,
		width,
		height,
		outputBuffer
...
var getImageBitmap = (imageData, format) => __awaiter(void 0, void 0, void 0, function* () {
	switch (format) {
		case "nv12":
		case "yuv":
			if (!window.VideoFrame) return Promise.reject(/* @__PURE__ */ new Error(`Cannot process frame. ImageData in ${format} is not supported by this browser.`));
		...
		case "rgb": return createImageBitmap(imageData);
	}
});
var createFunctionSource = (sourceFunction, options = {}) => {
	...
	return new CameraKitSource({ useManualFrameProcessing: true }, {
		onAttach: (source, lensCore, reportError) => {
			...
			const processFrame = (source, lensCore, reportError) => requestAnimationFrame(() => __awaiter(void 0, void 0, void 0, function* () {
				...
				try {
						...
						const frameOutput = new Promise((resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
							...
							lensCore.processFrame({
								...
								switch (format !== null && format !== void 0 ? format : "rgb") {
									case "nv12":
									case "yuv":
										const [live, capture] = yield Promise.all([getYUVImageData(output.live, lensCore), getYUVImageData(output.capture, lensCore)]).catch((error) => {
											reject(error);
					...
					reportError(new Error(`Failure to process frame, which was not handled by the provided MediaSourceFunction ${(_a = sourceFunction.name) !== null && _a !== void 0 ? _a : "anonymous"}.`, { cause: error }));
				}
			}));
...
//#endregion
//#region node_modules/.pnpm/@snap+camera-kit@1.13.0_google-protobuf@3.21.4/node_modules/@snap/camera-kit/dist/media-sources/ImageSource.js
function createImageSource(image, options = {}) {
	return new CameraKitSource({ media: image }, {}, options);
}
//#endregion
export { Any, CameraKit, CameraKitSession, CameraKitSource, ConcatInjectable, Container, Count, Histogram, Injectable, InjectableCompat, LensPerformanceMeasurement, LensPerformanceMetrics, LensRepository, Lens_CameraFacing, Metric, PartialContainer, Timer, Transform2D, TypedCustomEvent, TypedEventTarget, argumentValidationError, benchmarkError, bootstrapCameraKit, bootstrapError, cacheKeyNotFoundError, cameraKitSourceError, configurationError, configurationToken, createExtension, createFunctionSource, createImageSource, createMediaStreamSource, createVideoSource, defaultFetchHandlerFactory, estimateLensPerformance, extensionRequestContext, externalMetricsSubjectFactory, filePickerFactory, geoDataProviderFactory, getExtensionRequestContext, legalError, lensAbortError, lensAssetError, lensContentValidationError, lensError, lensExecutionError, lensImagePickerError, lensSourcesFactory, namedError, persistentStoreError, platformNotSupportedError, remoteApiServicesFactory, remoteMediaAssetLoaderFactory, toPublicLens, uriHandlersFactory, userDataAccessResolverFactory, webGLError };
